...
const knownExistingBlogTitle =
  process.env.ALTFT_EXISTING_BLOG_TITLE ||
  "Ultimate France Travel Guide for First-Time Visitors";
...
test.describe("admin blog authoring UX", () => {
  ...
  test("admin shell keeps mobile navigation and theme mode reachable", async ({
    page,
  }) => {
  ...
  test("save draft preview explains the final gate before writing", async ({
    page,
  }) => {
    const quality = createPageQualityGate(page);

    await page.goto(`${adminUrl}/altftool/blogs/add-blogs`, {
      waitUntil: "domcontentloaded",
    });
    await expect(
    ...
    await expect(
      page.getByRole("heading", { name: "Save draft preview" }),
    ).toBeVisible();
    await expect(page.getByText("Final slug", { exact: true })).toBeVisible();
    await expect(
      page.getByText("Changed fields", { exact: true }),
    ).toBeVisible();
    await expect(page.getByText("Publish gate", { exact: true })).toBeVisible();
    await expect(
      page.getByText("Duplicate guard", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Confirm draft" }),
    ).toBeVisible();

    await quality.expectClean("admin blog draft preview");
  });

  test("duplicate guard blocks an existing title before save confirmation", async ({
    page,
  }) => {
    const quality = createPageQualityGate(page);
    ...
    await expect(
      page.getByText("Duplicate slug:", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Duplicate title:", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Confirm draft" }),
    ).toBeDisabled();
  ...
  test("content health jump highlights the missing editor section", async ({
    page,
  }) => {
    const quality = createPageQualityGate(page);
    ...
    await expect(page.locator("#blog-section-post-details")).toHaveClass(
      /ring-primary/,
    );
    await quality.expectClean("admin blog content health jump");
  });

  test("quality center exposes content health bulk controls", async ({
    page,
  }) => {
    const quality = createPageQualityGate(page);

    await page.goto(`${adminUrl}/altftool/blogs/quality`, {
      waitUntil: "domcontentloaded",
    });
    await expect(
      page.getByRole("heading", { name: "Quality Center" }),
    ).toBeVisible({ timeout: 30_000 });
    await expect(page.getByText("Publish guard")).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.getByText("Schema preview")).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.getByText("Public runtime QA")).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.getByText("Content health")).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.getByText("Internal link opportunities")).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.getByText("Blog link health")).toBeVisible({
      timeout: 30_000,
    });
    await expect(
      page.getByRole("button", { name: /Fix top 10 issues|Fix \d+ selected/ }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Preview top internal links" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Preview apply" }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Preview schema" }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("button", {
        name: /Preview all link fixes|Preview \d+ selected link fixes/,
      }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", {
        name: /Preview safe cleanup|Preview \d+ selected/,
      }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Run runtime QA" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Links" }).first(),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", { name: "Preview weak anchor fixes" }),
    ).toBeVisible();
