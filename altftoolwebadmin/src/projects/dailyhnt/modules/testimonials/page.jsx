      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">DailyHnt Testimonials</h1>
              <p className="text-sm text-gray-500">Manage the client testimonials shown on the DailyHnt site.</p>
            </div>
          </div>
    ...
    <div className="overflow-hidden rounded-2xl border border-[#262626] bg-[#111111]">
      <div className="p-6">
        <div className="text-center">
          <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-[#c6f135]">Client Word</p>
          <h3 className="mx-auto mt-3 max-w-md text-2xl font-medium leading-tight text-[#ededed]">
            Ask the teams who shipped with us.
          </h3>
        </div>

        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
          {activeItems.length ? activeItems.map((item) => (
            <figure key={item.id} className="flex w-full flex-col gap-4 border border-[#262626] bg-[#0a0a0a] p-5">
              <div className="flex gap-1" aria-label={`${item.rating} out of 5 stars`}>
                {Array.from({ length: 5 }).map((_, index) => (
                  <Star
                    key={index}
                    className="h-4 w-4"
                    style={{ color: index < (Number(item.rating) || 0) ? "#ffb020" : "#262626" }}
                    fill={index < (Number(item.rating) || 0) ? "#ffb020" : "none"}
                  />
                ))}
              </div>
              <blockquote className="flex-1 text-sm leading-relaxed text-[#ededed]">
                &ldquo;{item.quote}&rdquo;
              </blockquote>
              <figcaption className="border-t border-[#262626] pt-4 font-mono text-[11px] uppercase tracking-[0.08em] text-[#8a8a8a]">
                {item.name}
                {item.role ? ` — ${item.role}` : ""}
            ...
            <div className="col-span-full border border-[#262626] bg-[#0a0a0a] p-8 text-center text-sm text-[#8a8a8a]">
