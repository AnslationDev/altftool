...
import { Edit3, Eye, EyeOff, Image as ImageIcon, Loader2, Plus, Search, Trash2, Upload, Users, X } from "lucide-react";
...
const DEPARTMENTS = ["Leadership", "Engineering", "Design", "Marketing"];
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">DailyHnt Team</h1>
              <p className="text-sm text-gray-500">Manage the team members shown on the DailyHnt site.</p>
            </div>
          </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_170px]">
              <label className="relative">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} className={`${inputClass} pl-10`} placeholder="Search by name, role, or department" />
              </label>
...
function TeamPreview({ members }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#262626] bg-[#0a0a0a]">
      <div className="p-6">
        <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-[#c6f135]">Team</p>
        <h3 className="mt-3 max-w-md text-2xl font-medium leading-tight text-[#ededed]">
          The engineers and strategists behind the work.
        </h3>

        <div className="mt-8 grid grid-cols-1 gap-px border border-[#262626] bg-[#262626] sm:grid-cols-2">
          {activeMembers.length ? activeMembers.map((member) => (
            <div key={member.id} className="group overflow-hidden border border-[#262626] bg-[#0a0a0a]">
              <div className="relative aspect-[4/5] overflow-hidden bg-[#111111]">
                {member.image ? (
                  <img src={member.image} alt={member.name} className="h-full w-full object-cover" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center">
                    <ImageIcon className="h-10 w-10 text-[#262626]" />
                  </div>
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent" />
                {member.role ? (
                  <span className="absolute left-3 top-3 rounded-full border border-white/20 bg-black/50 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.12em] text-white backdrop-blur">
                    {member.role}
                  </span>
                ) : null}
              </div>
              <div className="p-4">
                <h4 className="text-lg font-medium text-[#ededed]">{member.name}</h4>
                {member.department ? (
                  <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.15em] text-[#8a8a8a]">{member.department}</p>
                ) : null}
                <div className="mt-4 font-mono text-[10px] uppercase tracking-[0.15em] text-[#c6f135]">
                  View Profile →
                </div>
              </div>
            </div>
          )) : (
            <div className="col-span-full border border-[#262626] bg-[#111111] p-8 text-center text-sm text-[#8a8a8a]">
...
function MemberModal({ mode, member, members, onClose }) {
  ...
  async function save() {
    const nextErrors = {};
    if (!form.image) nextErrors.image = "Image is required.";
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[220px_1fr]">
          ...
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Slug" error={errors.slug}><input value={form.slug} onChange={(event) => setField("slug", event.target.value)} className={inputClass} placeholder="jane-doe" /></Field>
            ...
            <Field label="Skills (one per line)"><textarea value={form.skills} onChange={(event) => setField("skills", event.target.value)} rows={4} className={textareaClass} placeholder={"React\nNode.js\nDesign Systems"} /></Field>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="GitHub URL"><input value={form.social.github} onChange={(event) => setSocial("github", event.target.value)} className={inputClass} /></Field>
              <Field label="Dribbble URL"><input value={form.social.dribbble} onChange={(event) => setSocial("dribbble", event.target.value)} className={inputClass} /></Field>
            </div>
            <Field label="Status">
