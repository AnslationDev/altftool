      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">DailyHnt Services</h1>
              <p className="text-sm text-gray-500">Manage the services catalog, hero copy, benefits, workflow, and FAQs.</p>
            </div>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
              ...
              <div className="min-w-[720px]">
                ...
                {loading ? (
                  ...
                  <div key={item.id} className="grid grid-cols-[1fr_1fr_110px_100px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                    ...
                    <span className="w-fit rounded-lg bg-gray-100 px-2 py-1 text-xs font-semibold text-gray-600">{item.icon}</span>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="ml-auto text-xs font-medium text-gray-400">DailyHnt · Services</span>
            </div>
            <ServicesPreview services={previewServices} loading={loading} />
          </section>
        </div>
      </div>
...
function ServicesPreview({ services, loading }) {
  return (
    <div className="overflow-hidden rounded-xl border border-[#262626] bg-[#0a0a0a] p-5">
      <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#c6f135]">Services</p>
      <h3 className="mt-2 max-w-md text-lg font-semibold leading-snug text-[#ededed]">
        Full-stack technical marketing, end to end.
      </h3>

      {loading ? (
        <div className="mt-5 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-[#262626] bg-[#262626]">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="h-32 animate-pulse bg-[#111111]" />
          ))}
        </div>
      ) : services.length ? (
        <div className="mt-5 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-[#262626] bg-[#262626]">
          {services.slice(0, 6).map((service) => {
            const Icon = PREVIEW_ICONS[service.icon] || Grid3x3;
            return (
              <div key={service.id || service.slug} className="flex min-h-32 flex-col gap-4 bg-[#0a0a0a] p-4 transition-colors hover:bg-[#111111]">
                <div className="flex items-start justify-between">
                  <span className="font-mono text-[11px] text-[#8a8a8a]">{service.number || "—"}</span>
                  <Icon className="h-5 w-5 text-[#c6f135]" />
                </div>
                <div className="flex flex-1 flex-col gap-1.5">
                  <h4 className="text-sm font-medium leading-snug text-[#ededed]">{service.title || "Untitled service"}</h4>
                  <p className="line-clamp-3 text-xs leading-relaxed text-[#8a8a8a]">{service.shortDescription}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-5 rounded-lg border border-dashed border-[#262626] p-8 text-center">
          <p className="font-mono text-xs uppercase tracking-[0.15em] text-[#8a8a8a]">No active services to preview</p>
        </div>
      )}
    </div>
  );
      ...
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Slug" error={errors.slug}><input value={form.slug} onChange={(event) => setField("slug", event.target.value.toLowerCase())} className={inputClass} placeholder="web-development" /></Field>
            ...
            <Field label="Number"><input value={form.number} onChange={(event) => setField("number", event.target.value)} className={inputClass} placeholder="01" /></Field>
            <Field label="Icon">
          ...
          <Repeater
            ...
            rows={form.workflow}
            onAdd={() => addRow("workflow", { step: form.workflow.length + 1, title: "", description: "" })}
            onRemove={(index) => removeRow("workflow", index)}
            render={(row, index) => (
              <div className="grid gap-2 sm:grid-cols-[70px_minmax(0,0.9fr)_minmax(0,1.4fr)]">
                <input type="number" value={row.step ?? index + 1} onChange={(event) => updateRow("workflow", index, { step: event.target.value })} className={inputClass} placeholder="Step" />
          ...
          <div className="rounded-xl border border-gray-200 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Headline"><input value={form.pricingCta.headline} onChange={(event) => setPricing("headline", event.target.value)} className={inputClass} /></Field>
