      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Infovasta Blog</h1>
...
function BlogPageSettingsCard() {
  const [settings, setSettings] = useState(DEFAULT_BLOG_PAGE);
  const [savedSettings, setSavedSettings] = useState(DEFAULT_BLOG_PAGE);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    const unsub = subscribeBlogPage(
      (data) => {
        setSettings(data);
      ...
      () => {
        emitAlert({ type: "error", message: "Failed to load blog page settings." });
        setLoading(false);
      },
    );
    return () => unsub();
  ...
  async function save() {
    setSaving(true);
    try {
      await saveBlogPage(settings);
      emitAlert({ type: "success", message: "Blog page settings saved." });
    } catch (error) {
      emitAlert({ type: "error", message: error?.message || "Failed to save blog page settings." });
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-base font-bold text-gray-900">Blog page hero, meta &amp; detail copy</h2>
        </div>
        <div className="flex items-center gap-3">
      ...
      {loading ? (
        ...
        <div className="mt-5 space-y-6">
          ...
          <div>
            <p className="mb-3 text-xs font-bold uppercase tracking-widest text-gray-400">Listing Copy</p>
            <div className="grid gap-4 sm:grid-cols-3">
              <Field label="Search Placeholder"><input value={settings.searchPlaceholder || ""} onChange={(event) => setField("searchPlaceholder", event.target.value)} className={inputClass} /></Field>
              <Field label="Empty State Text"><input value={settings.emptyText || ""} onChange={(event) => setField("emptyText", event.target.value)} className={inputClass} /></Field>
              <Field label={'"All" Filter Label'}><input value={settings.allLabel || ""} onChange={(event) => setField("allLabel", event.target.value)} className={inputClass} /></Field>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Back Label"><input value={getPath(settings, "detail.backLabel") || ""} onChange={(event) => setField("detail.backLabel", event.target.value)} className={inputClass} /></Field>
              <Field label="More Articles Heading"><input value={getPath(settings, "detail.moreHeading") || ""} onChange={(event) => setField("detail.moreHeading", event.target.value)} className={inputClass} /></Field>
            </div>
          </div>
        </div>
      )}
...
function ArticleModal({ mode, article, articles, onClose }) {
  ...
  const [form, setForm] = useState(() => ({
    ...
    body: Array.isArray(article?.body) && article.body.length ? article.body : [""],
    order: Number(article?.order) || nextOrder,
    width: Number(article?.width) || 0,
    height: Number(article?.height) || 0,
    large: Boolean(article?.large),
    active: article?.active !== false,
  }));
  const [tagsText, setTagsText] = useState(linesToText(article?.tags));
  ...
  function updateBodyRow(index, value) {
    setForm((prev) => ({ ...prev, body: prev.body.map((row, i) => (i === index ? value : row)) }));
  }
      ...
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[260px_1fr]">
          <div>
            ...
            <div className="mt-3">
              <Field label="Featured / Large Card">
                <button onClick={() => setField("large", !form.large)} type="button" className={`flex h-11 w-full items-center justify-between rounded-xl border px-3 text-sm font-semibold ${form.large ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-gray-200 bg-gray-50 text-gray-500"}`}>
                  <span>{form.large ? "Large" : "Standard"}</span>
                  <span className={`h-2.5 w-2.5 rounded-full ${form.large ? "bg-emerald-500" : "bg-gray-400"}`} />
                </button>
              </Field>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Slug" error={errors.slug}><input value={form.slug} onChange={(event) => { setSlugTouched(true); setField("slug", createSlug(event.target.value)); }} className={inputClass} placeholder="auto-generated-from-title" /></Field>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
              <Field label="Tag"><input value={form.tag} onChange={(event) => setField("tag", event.target.value)} className={inputClass} placeholder="Featured" /></Field>
              <Field label="Category"><input value={form.category} onChange={(event) => setField("category", event.target.value)} className={inputClass} placeholder="Company News" /></Field>
              <Field label="Author"><input value={form.author} onChange={(event) => setField("author", event.target.value)} className={inputClass} /></Field>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="mt-5">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-gray-500">Body (one paragraph per row)</span>
            <button onClick={addBodyRow} type="button" className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50">
          ...
          <div className="space-y-2">
            {form.body.map((paragraph, index) => (
              <div key={index} className="grid grid-cols-[1fr_40px] gap-2">
                <textarea value={paragraph} onChange={(event) => updateBodyRow(index, event.target.value)} rows={3} className={textareaClass} placeholder={`Paragraph ${index + 1}`} />
                <button onClick={() => removeBodyRow(index)} type="button" className="flex items-center justify-center rounded-xl border border-red-200 text-red-500 hover:bg-red-50">
        ...
        <div className="mt-5">
          <Field label="Tags (one per line)"><textarea value={tagsText} onChange={(event) => setTags(event.target.value)} rows={4} className={textareaClass} /></Field>
        </div>
