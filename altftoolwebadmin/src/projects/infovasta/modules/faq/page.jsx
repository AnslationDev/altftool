...
import { Edit3, Eye, EyeOff, HelpCircle, Loader2, Plus, Search, Trash2, X } from "lucide-react";
...
export default function FaqPage() {
  ...
  useEffect(() => {
    const unsub = subscribeFaqs(
      (data) => {
        setItems(data);
        setLoading(false);
      },
      () => {
        emitAlert({ type: "error", message: "Failed to load FAQs." });
        setLoading(false);
      },
    );
    return () => unsub();
  ...
  async function toggleItem(item) {
    try {
      await toggleFaqStatus(item.id, item.active === false);
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Infovasta FAQ</h1>
              <p className="text-sm text-gray-500">Manage the frequently asked questions shown on the Infovasta site.</p>
            </div>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <StatCard label="Total Questions" value={loading ? "-" : items.length} />
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <h2 className="mt-1 text-base font-bold text-gray-900">{items.length} questions</h2>
              </div>
            </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_170px]">
              <label className="relative">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} className={`${inputClass} pl-10`} placeholder="Search by question or answer" />
              </label>
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-gray-200">
              <div className="min-w-[620px]">
                <div className="grid grid-cols-[1.3fr_1.5fr_80px_120px] bg-gray-50 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  <span>Question</span><span>Answer</span><span>Status</span><span>Actions</span>
                </div>
                {loading ? (
                  <div className="border-t border-gray-100 p-8 text-center text-sm font-semibold text-gray-500">Loading FAQs…</div>
                ) : filtered.length ? filtered.map((item) => (
                  <div key={item.id} className="grid grid-cols-[1.3fr_1.5fr_80px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                    <p className="truncate font-bold text-gray-900">{item.q}</p>
                    <p className="truncate text-xs font-semibold text-gray-500">{item.a}</p>
                  ...
                  <div className="border-t border-gray-100 p-8 text-center text-sm font-semibold text-gray-500">No FAQs found.</div>
                )}
              </div>
            </div>
          </section>
    ...
    <div className="overflow-hidden rounded-2xl border border-[#e7e5e4] bg-[#faf8f4]">
      <div className="p-6">
        <div className="text-center">
          <p className="text-[11px] uppercase tracking-[0.2em] text-[#78716c]">Questions, answered</p>
        ...
        <div className="mt-8">
          {activeItems.length ? (
            <div className="divide-y divide-[#e7e5e4] rounded-lg border border-[#e7e5e4] bg-white">
              {activeItems.map((item, index) => {
                const isOpen = index === 0;
                return (
                  <div key={item.id}>
                    <div className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left">
                      <span className="text-base font-medium text-[#1c1917]">{item.q}</span>
                      <span aria-hidden="true" className="shrink-0 text-lg text-[#78716c]">
                        {isOpen ? "−" : "+"}
                      </span>
                    </div>
                    {isOpen ? (
                      <p className="px-5 pb-5 text-sm leading-relaxed text-[#78716c]">{item.a}</p>
                    ) : null}
                  </div>
                );
              })}
            ...
            <div className="rounded-lg border border-[#e7e5e4] bg-white p-8 text-center text-sm text-[#78716c]">
              No active FAQs yet.
            </div>
          )}
        </div>
...
function FaqModal({ mode, item, items, onClose }) {
  ...
  async function save() {
    const nextErrors = {};
    if (!form.q.trim()) nextErrors.q = "Question is required.";
    if (!form.a.trim()) nextErrors.a = "Answer is required.";
      ...
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 text-lg font-bold text-gray-900">FAQ details</h3>
          </div>
        ...
        <div className="mt-5 space-y-4">
          <Field label="Question" error={errors.q}><input value={form.q} onChange={(event) => setField("q", event.target.value)} className={inputClass} /></Field>
          <Field label="Answer" error={errors.a}><textarea value={form.a} onChange={(event) => setField("a", event.target.value)} rows={5} className={textareaClass} /></Field>
