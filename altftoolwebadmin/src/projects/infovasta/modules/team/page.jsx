...
export default function TeamPage() {
  const [pageContent, setPageContent] = useState(DEFAULT_TEAM_PAGE_CONTENT);
  const [pageDraft, setPageDraft] = useState(DEFAULT_TEAM_PAGE_CONTENT);
  ...
  useEffect(() => {
    const unsubPage = subscribeTeamPageContent(
      (data) => {
        setPageContent(data);
        setPageDraft(data);
      },
      () => emitAlert({ type: "error", message: "Failed to load team page content." }),
    );
  ...
  async function handleSavePageContent() {
    setPageSaving(true);
    try {
      await saveTeamPageContent(pageDraft);
      emitAlert({ type: "success", message: "Team page content saved." });
    } catch (error) {
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Infovasta Team</h1>
              <p className="text-sm text-gray-500">Manage the team list page copy, detail-page template, and team members.</p>
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-5">
            ...
            <h2 className="mt-1 text-base font-bold text-gray-900">Team list &amp; detail page content</h2>
          </div>
          ...
          <div className="mt-5 rounded-xl border border-gray-200 p-4">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Field label="Experience Suffix"><input value={pageDraft.detail?.experienceSuffix || ""} onChange={(event) => updatePageDraft("detail", "experienceSuffix", event.target.value)} className={inputClass} placeholder="years experience" /></Field>
              <Field label="Skills Heading"><input value={pageDraft.detail?.skillsHeading || ""} onChange={(event) => updatePageDraft("detail", "skillsHeading", event.target.value)} className={inputClass} /></Field>
              <Field label="Back Label"><input value={pageDraft.detail?.backLabel || ""} onChange={(event) => updatePageDraft("detail", "backLabel", event.target.value)} className={inputClass} /></Field>
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 overflow-x-auto rounded-xl border border-gray-200">
            <div className="min-w-[720px]">
              <div className="grid grid-cols-[70px_1fr_1fr_80px_120px] bg-gray-50 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                <span>Image</span><span>Name</span><span>Role</span><span>Status</span><span>Actions</span>
              </div>
              {loading ? (
                ...
                <div key={member.id} className="grid grid-cols-[70px_1fr_1fr_80px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                  <div className="h-12 w-12 overflow-hidden rounded-xl border border-gray-200 bg-gray-50">
                    {member.image ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center text-xs font-bold text-gray-400" style={hslPreviewStyle(member.color) || {}}>
                        {member.initials || <ImageIcon className="h-5 w-5 text-gray-300" />}
                      </div>
                    )}
                  </div>
...
function MemberModal({ mode, member, members, onClose }) {
  ...
  const [form, setForm] = useState(() => ({
    ...
    socials: { ...EMPTY_MEMBER.socials, ...(member?.socials || {}) },
  ...
  function updateSkillRow(index, value) {
    setForm((prev) => ({ ...prev, skills: prev.skills.map((row, i) => (i === index ? value : row)) }));
  }
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[220px_1fr]">
          <div>
            <div className="flex aspect-[0.82] items-center justify-center overflow-hidden rounded-2xl border border-gray-200 bg-gray-50">
              {form.image ? (
                <img src={form.image} alt="Member preview" className="h-full w-full object-cover object-top" />
              ) : (
                <div className="flex h-full w-full items-center justify-center text-2xl font-bold text-gray-400" style={hslPreviewStyle(form.color) || {}}>
                  {form.initials || <ImageIcon className="h-8 w-8 text-gray-300" />}
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Initials"><input value={form.initials} onChange={(event) => setField("initials", event.target.value)} className={inputClass} placeholder="JD" /></Field>
              ...
              <Field label="Experience"><input value={form.experience} onChange={(event) => setField("experience", event.target.value)} className={inputClass} placeholder="8+ years" /></Field>
            ...
            <div className="rounded-xl border border-gray-200 p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <button onClick={addSkillRow} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50">
                  <Plus className="h-3.5 w-3.5" /> Add Row
                </button>
              </div>
              <div className="mt-3 space-y-3">
                {form.skills.length ? form.skills.map((row, index) => (
                  <div key={index} className="flex items-start gap-2">
                    <input value={row} onChange={(event) => updateSkillRow(index, event.target.value)} className={`${inputClass} flex-1`} placeholder="Brand Strategy" />
                    <button onClick={() => removeSkillRow(index)} className="rounded-lg border border-red-200 p-2 text-red-500 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                )) : (
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <Field label="Twitter URL"><input value={form.socials.twitter} onChange={(event) => setSocial("twitter", event.target.value)} className={inputClass} /></Field>
              <Field label="LinkedIn URL"><input value={form.socials.linkedin} onChange={(event) => setSocial("linkedin", event.target.value)} className={inputClass} /></Field>
              <Field label="Facebook URL"><input value={form.socials.facebook} onChange={(event) => setSocial("facebook", event.target.value)} className={inputClass} /></Field>
            </div>
            <Field label="Status">
