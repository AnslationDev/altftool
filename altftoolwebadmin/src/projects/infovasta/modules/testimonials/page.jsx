      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Infovasta Testimonials</h1>
              <p className="text-sm text-gray-500">Manage the client testimonials shown on the Infovasta site.</p>
            </div>
          </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_170px]">
              <label className="relative">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} className={`${inputClass} pl-10`} placeholder="Search by name or company" />
              </label>
                ...
                {loading ? (
                  ...
                  <div key={item.id} className="grid grid-cols-[1fr_1fr_100px_80px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs font-semibold text-gray-500">{item.initials}</p>
                    </div>
...
function TestimonialModal({ mode, item, items, onClose }) {
  ...
  async function save() {
    ...
    if (!form.review.trim()) nextErrors.review = "Review is required.";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/55 p-4 backdrop-blur-sm">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          <Field label="Review" error={errors.review}><textarea value={form.review} onChange={(event) => setField("review", event.target.value)} rows={4} className={textareaClass} /></Field>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Initials"><input value={form.initials} onChange={(event) => setField("initials", event.target.value)} className={inputClass} placeholder="e.g. AB" /></Field>
