...
import { Edit3, Eye, EyeOff, Loader2, Plus, Search, Sparkles, Trash2, X } from "lucide-react";
...
export default function WhyChooseUsPage() {
  ...
  useEffect(() => {
    const unsub = subscribeWhyChooseUs(
      (data) => {
        setItems(data);
        setLoading(false);
      },
      () => {
        emitAlert({ type: "error", message: "Failed to load Why Choose Us items." });
        setLoading(false);
      },
    );
    return () => unsub();
  ...
  async function toggleItem(item) {
    try {
      await toggleWhyChooseUsStatus(item.id, item.active === false);
  ...
  async function confirmDelete() {
    ...
    try {
      await deleteWhyChooseUs(deleteTarget.id);
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Infovasta Why Choose Us</h1>
              <p className="text-sm text-gray-500">Manage the "Why Choose Us" highlights shown on the Infovasta homepage.</p>
            </div>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-gray-400">Item Management</p>
                <h2 className="mt-1 text-base font-bold text-gray-900">{items.length} items</h2>
              </div>
            </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_170px]">
              <label className="relative">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} className={`${inputClass} pl-10`} placeholder="Search by title or text" />
              </label>
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-gray-200">
              <div className="min-w-[680px]">
                <div className="grid grid-cols-[1fr_1fr_100px_80px_120px] bg-gray-50 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  <span>Title</span><span>Icon</span><span>Color</span><span>Status</span><span>Actions</span>
                </div>
                {loading ? (
                  <div className="border-t border-gray-100 p-8 text-center text-sm font-semibold text-gray-500">Loading items…</div>
                  ...
                  <div key={item.id} className="grid grid-cols-[1fr_1fr_100px_80px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                    ...
                    <p className="truncate text-xs font-semibold text-gray-500">{item.icon}</p>
                    <span
                      className="h-6 w-6 rounded-full border border-gray-200"
                      style={{ backgroundColor: item.color ? `hsl(${item.color})` : "transparent" }}
                      title={item.color}
                    />
                  ...
                  <div className="border-t border-gray-100 p-8 text-center text-sm font-semibold text-gray-500">No items found.</div>
                )}
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <WhyChooseUsPreview items={items} />
          </section>
        </div>
      </div>

      {modalState ? <WhyChooseUsModal mode={modalState.mode} item={modalState.item} items={items} onClose={() => setModalState(null)} /> : null}
      {deleteTarget ? (
        <DeleteConfirmModal
          title="Delete item"
...
function WhyChooseUsPreview({ items }) {
    ...
    <div className="overflow-hidden rounded-2xl border border-[#e7e5e4] bg-[#faf8f4]">
      <div className="p-6">
        <div className="text-center">
          <p className="text-[11px] uppercase tracking-[0.2em] text-[#78716c]">Why choose us</p>
          ...
          {activeItems.length ? activeItems.map((item) => (
            <div key={item.id} className="flex flex-col gap-3 rounded-lg border border-[#e7e5e4] bg-white p-5">
              <span
                ...
                style={{ backgroundColor: item.color ? `hsl(${item.color})` : "#78716c" }}
              >
...
function WhyChooseUsModal({ mode, item, items, onClose }) {
  ...
  async function save() {
    ...
    try {
      if (mode === "edit") {
        await updateWhyChooseUs(item.id, form);
        emitAlert({ type: "success", message: "Item updated." });
      } else {
        await createWhyChooseUs(form);
        emitAlert({ type: "success", message: "Item added." });
      }
      onClose();
    } catch (error) {
      emitAlert({ type: "error", message: error?.message || "Failed to save item." });
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/55 p-4 backdrop-blur-sm">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-gray-400">{mode === "edit" ? "Edit Item" : "Add Item"}</p>
            <h3 className="mt-1 text-lg font-bold text-gray-900">Why Choose Us details</h3>
          </div>
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="Icon (Ionicons name)"><input value={form.icon} onChange={(event) => setField("icon", event.target.value)} className={inputClass} placeholder="e.g. flash-outline" /></Field>
            <Field label="Color (HSL triplet)"><input value={form.color} onChange={(event) => setField("color", event.target.value)} className={inputClass} placeholder="e.g. 174, 77%, 50%" /></Field>
