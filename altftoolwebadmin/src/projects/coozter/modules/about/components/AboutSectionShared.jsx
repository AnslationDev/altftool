...
export const SECTION_FIELDS = {
  ...
  workModelSection: [
    ["eyebrowText", "Eyebrow text"],
    ["visualAltText", "Visual alt text", "textarea"],
    ["centerIconKey", "Center icon key"],
  ],
  teamSection: [
    ["eyebrowText", "Eyebrow text"],
    ["headingText", "Heading text", "textarea"],
    ["descriptionText", "Description", "textarea"],
    ["modalCompanyLabelSuffix", "Modal company label suffix"],
        ...
        ([arrayKey, config]) => (
          <ArrayEditor
            ...
            onUpdate={(index, field, value) =>
              onArrayUpdate(arrayKey, index, field, value)
            }
          />
        ),
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--muted)]">
            Add, edit, delete, reorder, and hide repeated items.
          </p>
        </div>
        <button
          type="button"
              ...
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      onUpdate(index, "isActive", row.isActive === false)
                    }
                    className="rounded-md border border-[var(--border)] px-2.5 py-1.5 text-xs font-bold text-[var(--muted)]"
                  >
                  ...
                  <button
                    ...
                    className="rounded-md border border-[color-mix(in_srgb,var(--danger)_35%,var(--border))] p-1.5 text-[var(--danger)]"
                  >
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {config.fields.map(([field, label, type]) => (
                  <Field
                    ...
                    label={label}
                    error={
                      errors[`${sectionKey}.${arrayKey}.${index}.${field}`]
                    }
                    wide={type === "textarea" || type === "list"}
                  >
                    <ValueInput
                      value={row[field] || ""}
                      type={type}
                ...
                <Field
                  label="Sort order"
                  error={errors[`${sectionKey}.${arrayKey}.${index}.sortOrder`]}
                >
                  <input
                    type="number"
                    min="0"
                    ...
                    onChange={(event) =>
                      onUpdate(index, "sortOrder", event.target.value)
                    }
...
export function LoadingFields() {
  return (
    <div className="h-40 animate-pulse rounded-lg bg-[var(--surface-soft)]" />
  );
}
