...
export const SECTION_FIELDS = {
  ...
  growthEngineSection: [
    ["headingText", "Heading text", "textarea"],
    ["descriptionText", "Description", "textarea"],
    ["visualAltText", "Visual alt text"],
    ["centerLabelLineOne", "Center label line one"],
    ["centerLabelLineTwo", "Center label line two"],
  ],
  outcomesSection: [
    ["headingText", "Heading text", "textarea"],
    ["descriptionText", "Description", "textarea"],
  ...
  faqSection: [["headingText", "Heading text"]],
};

export const ARRAY_FIELDS = {
  heroSection: {
    partnerBadges: { label: "Partner Badges", fields: [["name", "Name"], ["label", "Label"]] },
  },
  serviceCategoriesSection: {
    serviceGroups: {
      label: "Service Groups",
      fields: [["title", "Title"], ["items", "Items", "list"], ["buttonLabel", "Button label"], ["buttonUrl", "Button URL"]],
    },
  },
  growthEngineSection: {
    cards: { label: "Growth Cards", fields: [["title", "Title"], ["descriptionText", "Description"]] },
  },
  outcomesSection: {
    achievementBlocks: {
      label: "Achievement Blocks",
      fields: [["iconKey", "Icon key"], ["title", "Title"], ["descriptionText", "Description", "textarea"]],
    },
    metrics: { label: "Metrics", fields: [["value", "Value"], ["label", "Label"]] },
  },
  processSection: {
    steps: { label: "Steps", fields: [["title", "Title"], ["descriptionText", "Description", "textarea"]] },
  },
  faqSection: {
    faqs: { label: "FAQs", fields: [["question", "Question", "textarea"], ["answer", "Answer", "textarea"]] },
  },
};
      ...
      {(SECTION_FIELDS[sectionKey] || []).length ? (
        <div className="grid gap-4 md:grid-cols-2">
          {(SECTION_FIELDS[sectionKey] || []).map(([field, label, type]) => (
            <Field key={field} label={label} error={errors[`${sectionKey}.${field}`]} wide={type === "textarea" || type === "image"}>
              <ValueInput
                value={section[field] || ""}
                type={type}
