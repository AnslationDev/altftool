...
import { LoadingFields, PreviewPanel, buttonClass } from "./components/ServicesSectionShared";
import HeroSectionTab from "./hero-section/page";
import ServiceCategoriesSectionTab from "./service-categories-section/page";
import GrowthEngineSectionTab from "./growth-engine-section/page";
import OutcomesSectionTab from "./outcomes-section/page";
import ProcessSectionTab from "./process-section/page";
import FaqSectionTab from "./faq-section/page";

const SECTION_COMPONENTS = {
  heroSection: HeroSectionTab,
  serviceCategoriesSection: ServiceCategoriesSectionTab,
  growthEngineSection: GrowthEngineSectionTab,
  outcomesSection: OutcomesSectionTab,
  processSection: ProcessSectionTab,
  faqSection: FaqSectionTab,
};

export default function CoozterServicesAdminPage() {
  ...
  useEffect(() => {
    return subscribeServicesPageContent(
      (next) => {
        setContent(next);
        setLoading(false);
      },
      (error) => {
        setLoading(false);
        emitAlert({ type: "error", message: error?.message || "Failed to load Coozter services content." });
      },
    );
  }, []);
  ...
  async function handleSave() {
    ...
    const nextErrors = validateServicesPageContent(content);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length) {
      emitAlert({ type: "error", message: "Please fix required services page fields before saving." });
      return;
    }
    ...
    setSaving(true);
    try {
      await saveServicesPageContent(content);
      emitAlert({ type: "success", message: "Coozter services content saved." });
    } catch (error) {
      emitAlert({ type: "error", message: error?.message || "Failed to save services content." });
    } finally {
  ...
  async function handleSeedDefaults() {
    ...
    setSeeding(true);
    try {
      await seedServicesPageContent();
      setErrors({});
      emitAlert({ type: "success", message: "Default Coozter services content seeded." });
    } catch (error) {
  ...
  async function handleUpload(file, target) {
    ...
    setUploadProgress(0);
    try {
      const uploaded = await uploadServicesAsset({
        file,
      ...
      if (target.length === 1) setSectionField(target[0], uploaded.url);
      else updateArrayItem(target[0], target[1], target[2], uploaded.url);
    ...
    <main className="min-h-screen bg-[var(--background)] p-4 text-[var(--foreground)] sm:p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <h1 className="mt-1 text-2xl font-bold text-[var(--foreground)]">Services Page Content</h1>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setContent(DEFAULT_SERVICES_PAGE_CONTENT)} className={buttonClass("secondary")}>
