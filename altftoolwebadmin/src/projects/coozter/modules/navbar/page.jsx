          ...
          {activeItems.length ? (
            activeItems.map((item) => (
              <div key={item.id} className="flex items-center gap-1 whitespace-nowrap">
                <span>{item.label || item.title || "Menu item"}</span>
                {item.children?.length ? <ChevronDown className="h-4 w-4" /> : null}
              </div>
            ))
          ) : (
            <span className="text-white/60">No visible menu items</span>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      ...
      >
        ...
        <div className="mt-5 space-y-4">
          <Field label="Menu label" error={errors.label}>
            <input
              value={form.label || ""}
          ...
          <Field label="Menu URL" error={errors.url}>
            <input
              value={form.url || ""}
...
export default function CoozterNavbarAdminPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return sortedItems.filter((item) => {
      ...
      const matchesStatus =
        statusFilter === "all" ||
        (statusFilter === "visible" ? item.visible !== false : item.visible === false);
  ...
  const handleLogoUpload = async (file) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      emitAlert({ type: "error", message: "Logo image must be 5MB or smaller." });
      return;
    }
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      ...
      emitAlert({ type: "success", message: `"${deleteTarget.label || deleteTarget.title}" deleted.` });
  ...
  const handleToggleStatus = async (item) => {
    try {
      await toggleNavbarItemStatus(item.id, item.visible === false);
      emitAlert({
        type: "success",
        message: `${item.label || item.title} set to ${item.visible === false ? "visible" : "hidden"}.`,
      });
    } catch {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <StatCard label="Visible Items" value={loading ? "-" : visibleCount} tone="green" />
          <StatCard label="Hidden Items" value={loading ? "-" : hiddenCount} tone="amber" />
        </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <Field label="Logo alt text" error={settingsErrors.logoAlt}>
                <input
                        ...
                        <tr key={item.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3">
                            <button type="button" onClick={() => handleToggleStatus(item)}>
                              <StatusBadge active={item.visible !== false} />
                            </button>
                          </td>
                          <td className="px-4 py-3">
      ...
      {deleteTarget ? (
        <DeleteConfirmModal
          ...
          message={`Delete "${deleteTarget.label || deleteTarget.title}" from the Coozter navbar?`}
          confirmText="Delete"
