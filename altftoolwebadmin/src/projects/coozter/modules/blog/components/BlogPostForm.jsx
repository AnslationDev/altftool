...
export default function BlogPostForm({ mode = "create", post = null }) {
  ...
  function setField(field, value) {
    setForm((prev) => {
      ...
      if (field === "title" && !prev.seo.metaTitle) next.seo = { ...next.seo, metaTitle: `${value} | Coozter` };
      if (field === "excerpt" && !prev.seo.metaDescription) next.seo = { ...next.seo, metaDescription: value };
      if (field === "slug" && !next.seo.canonicalUrl) next.seo = { ...next.seo, canonicalUrl: `/blogs/${createSlug(value)}` };
  ...
  async function save() {
    ...
    try {
      ...
      emitAlert({ type: "error", message: error?.message || "Failed to save blog post." });
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Link href="/coozter/blog" className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-2 text-[var(--muted)]">
              <ArrowLeft className="h-4 w-4" />
            </Link>
            <div>
              <h1 className="text-2xl font-bold">{mode === "edit" ? "Edit Blog Post" : "Add Blog Post"}</h1>
              <p className="text-sm text-[var(--muted)]">projects/coozter/blogs/{mode === "edit" ? post?.id : "{blogId}"}</p>
            </div>
          </div>
          <button type="button" onClick={save} disabled={saving || uploading} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[var(--primary)] px-4 text-sm font-semibold text-[var(--primary-foreground)]">
          ...
          <section className="space-y-5">
            <Panel title="Post Fields">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Slug" error={errors.slug}><input value={form.slug} onChange={(event) => { setSlugEdited(true); setField("slug", createSlug(event.target.value)); }} className={inputClass} /></Field>
                <Field label="Category" error={errors.category}><input value={form.category} onChange={(event) => setField("category", event.target.value)} className={inputClass} /></Field>
                <Field label="Author" error={errors.author}><input value={form.author} onChange={(event) => setField("author", event.target.value)} className={inputClass} /></Field>
                <Field label="Read time" error={errors.readTime}><input value={form.readTime} onChange={(event) => setField("readTime", event.target.value)} className={inputClass} /></Field>
                <Field label="Display date" error={errors.displayDate}><input value={form.displayDate} onChange={(event) => setField("displayDate", event.target.value)} className={inputClass} /></Field>
                <Field label="Date" error={errors.date}><input type="date" value={form.date} onChange={(event) => setField("date", event.target.value)} className={inputClass} /></Field>
                <Field label="Order"><input type="number" min="0" value={form.order} onChange={(event) => setField("order", event.target.value)} className={inputClass} /></Field>
              </div>
            ...
            <Panel title="SEO">
              <Field label="Meta title" error={errors["seo.metaTitle"]}><input value={form.seo.metaTitle} onChange={(event) => setSeoField("metaTitle", event.target.value)} className={inputClass} /></Field>
              <Field label="Meta description" error={errors["seo.metaDescription"]}><textarea rows={3} value={form.seo.metaDescription} onChange={(event) => setSeoField("metaDescription", event.target.value)} className={textareaClass} /></Field>
              <Field label="Canonical URL" error={errors["seo.canonicalUrl"]}><input value={form.seo.canonicalUrl} onChange={(event) => setSeoField("canonicalUrl", event.target.value)} className={inputClass} /></Field>
            </Panel>
          </section>
          ...
          <aside className="space-y-5">
            <Panel title="Status">
              ...
              <label className="flex items-center gap-2 text-sm font-semibold text-[var(--muted)]">
                <input type="checkbox" checked={form.published} onChange={(event) => setField("status", event.target.checked ? "published" : "draft")} />
                Published
              </label>
            </Panel>
            ...
            <Panel title="Image">
              <div className="grid h-44 place-items-center overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface-soft)]">
                {form.imageUrl ? <img src={form.imageUrl} alt={form.imageAlt || ""} className="h-full w-full object-cover" /> : <ImageIcon className="h-8 w-8 text-[var(--muted)]" />}
              </div>
              <Field label="Image URL" error={errors.imageUrl}><input value={form.imageUrl} onChange={(event) => setField("imageUrl", event.target.value)} className={inputClass} /></Field>
              ...
              {uploading ? <div className="h-2 overflow-hidden rounded-full bg-[var(--border)]"><div className="h-full bg-[var(--primary)]" style={{ width: `${uploadProgress}%` }} /></div> : null}
              <Field label="Image alt" error={errors.imageAlt}><input value={form.imageAlt} onChange={(event) => setField("imageAlt", event.target.value)} className={inputClass} /></Field>
              <Field label="Image fallback text"><input value={form.image} onChange={(event) => setField("image", event.target.value)} className={inputClass} /></Field>
            </Panel>
          </aside>
        </div>
      </div>
