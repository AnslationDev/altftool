  ...
  async function confirmDelete() {
    ...
    try {
      ...
      if (deleteTarget.imagePath) {
        try {
          await deleteBlogImage(deleteTarget.imagePath);
        } catch {
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Offerris Blog</h1>
              <p className="text-sm text-gray-500">Manage the journal list page and articles.</p>
            </div>
          </div>
      ...
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[260px_1fr]">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Category"><input value={form.category} onChange={(event) => setField("category", event.target.value)} className={inputClass} placeholder="Web Design" /></Field>
              <Field label="Author" hint="team member slug"><input value={form.author} onChange={(event) => setField("author", event.target.value)} className={inputClass} placeholder="maya-okonkwo" /></Field>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <Field label="Reading Time"><input value={form.readingTime} onChange={(event) => setField("readingTime", event.target.value)} className={inputClass} placeholder="6 min read" /></Field>
            ...
            <Field label="Cover Gradient" hint="tailwind gradient classes e.g. from-violet-600 via-fuchsia-500 to-cyan-400">
              <input value={form.coverGradient} onChange={(event) => setField("coverGradient", event.target.value)} className={inputClass} placeholder="from-violet-600 via-fuchsia-500 to-cyan-400" />
            </Field>
            ...
            <Field label="Tags (one per line)"><textarea value={form.tags} onChange={(event) => setField("tags", event.target.value)} rows={3} className={textareaClass} placeholder={"Performance\nWeb Design"} /></Field>
          </div>
        </div>

        <div className="mt-5">
          <Field label="Content" hint="Separate paragraphs with a blank line — each blank-line-separated block is stored as one paragraph.">
            <textarea
              value={form.content}
