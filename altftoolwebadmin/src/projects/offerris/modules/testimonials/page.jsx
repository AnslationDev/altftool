      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Offerris Testimonials</h1>
              <p className="text-sm text-gray-500">Manage the client testimonials shown on the Offerris site.</p>
            </div>
          </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-gray-200">
              <div className="min-w-[680px]">
                <div className="grid grid-cols-[1fr_1fr_1fr_70px_80px_120px] bg-gray-50 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  <span>Name</span><span>Role</span><span>Company</span><span>Rating</span><span>Status</span><span>Actions</span>
                </div>
                {loading ? (
                  ...
                  <div key={item.id} className="grid grid-cols-[1fr_1fr_1fr_70px_80px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                    ...
                    <p className="text-xs font-semibold text-amber-500">{item.rating || 5}/5</p>
    ...
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-gray-50">
      <div className="p-6">
        <div className="text-center">
          <p className="text-[11px] uppercase tracking-[0.2em] text-gray-500">Client Voices</p>
          <h3 className="mx-auto mt-3 max-w-md text-2xl font-semibold leading-tight text-gray-900">
            Brands running at Offeris speed.
          </h3>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
          {activeItems.length ? activeItems.map((item) => (
            <figure key={item.id} className="flex w-full flex-col gap-4 rounded-lg border border-gray-200 bg-white p-5">
              <p className="text-xs font-semibold tracking-widest text-amber-500">
      ...
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Role"><input value={form.role} onChange={(event) => setField("role", event.target.value)} className={inputClass} placeholder="VP Marketing" /></Field>
            <Field label="Company"><input value={form.company} onChange={(event) => setField("company", event.target.value)} className={inputClass} placeholder="Northwind Supply Co." /></Field>
            <Field label="Rating (1-5)"><input type="number" min={1} max={5} value={form.rating} onChange={(event) => setField("rating", event.target.value)} className={inputClass} /></Field>
