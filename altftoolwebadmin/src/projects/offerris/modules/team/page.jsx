...
const EMPTY_SOCIAL = { twitter: "", linkedin: "", github: "", dribbble: "", instagram: "" };
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Offerris Team</h1>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-5">
            ...
            <h2 className="mt-1 text-base font-bold text-gray-900">Team page hero</h2>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            <Field label="Badge"><input value={settingsDraft.badge || ""} onChange={(event) => updateSettingsDraft("badge", event.target.value)} className={inputClass} /></Field>
...
function MemberModal({ mode, member, members, onClose }) {
  ...
  const [form, setForm] = useState(() => ({
    ...
    social: { ...EMPTY_SOCIAL, ...(member?.social || {}) },
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[220px_1fr]">
          ...
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Department"><input value={form.department} onChange={(event) => setField("department", event.target.value)} className={inputClass} placeholder="Design" /></Field>
              <Field label="Experience"><input value={form.experience} onChange={(event) => setField("experience", event.target.value)} className={inputClass} placeholder="9 years in UI/UX design" /></Field>
            ...
            <div>
              <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-gray-500">Social Links</span>
              <div className="grid gap-3 sm:grid-cols-2">
                {SOCIAL_KEYS.map((key) => (
                  <Field key={key} label={key.charAt(0).toUpperCase() + key.slice(1)}>
                    <input value={form.social[key] || ""} onChange={(event) => setSocial(key, event.target.value)} className={inputClass} placeholder={SOCIAL_PLACEHOLDERS[key]} />
                  </Field>
                ))}
              </div>
            </div>
