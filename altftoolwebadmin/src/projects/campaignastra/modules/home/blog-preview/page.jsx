...
const SECTION_KEY = "blog-preview";
const ROUTE = "/campaignastra/home/blog-preview";

export default function CampaignastraHomeBlogPreviewPage() {
  ...
  useEffect(() => {
    return subscribeHomeSection(
      ...
      () => {
        emitAlert({ type: "error", message: "Failed to load blog preview section." });
        setLoading(false);
      },
    );
  }, []);
  ...
  async function save() {
    ...
    try {
      ...
      emitAlert({ type: "success", message: "Blog preview section saved." });
      logAuditEvent({
        module: "home",
        action: "HOME_BLOG_PREVIEW_UPDATE",
        entityType: "homeBlogPreview",
        entityId: "blog-preview",
        summary: "Updated Campaignastra home blog preview section",
        changes: draft,
        route: ROUTE,
      });
    } catch (error) {
      emitAlert({ type: "error", message: error?.message || "Failed to save blog preview section." });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function resetSection() {
    ...
    try {
      ...
      emitAlert({ type: "success", message: "Blog preview section reset." });
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-6 pb-6">
      <ActionHeader
        title="Home — Blog Preview"
        description="Edit the intro copy shown above the latest blog posts."
      ...
      <div className="mx-auto mt-6 grid max-w-7xl gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-widest text-gray-400">Blog Preview Management</p>
          {loading ? (
            ...
            <div className="mt-5 space-y-4">
              ...
              <p className="rounded-xl border border-dashed border-gray-200 bg-gray-50 px-3 py-3 text-xs font-medium text-gray-400">
                Note: the actual blog posts shown on the homepage are managed separately in the Blog module.
              </p>
            </div>
          )}
        </section>
        ...
        <section className="space-y-4">
          ...
          <SimplePreview icon={Newspaper} eyebrow={draft.eyebrow} title={draft.title} subtitle={draft.subtitle} />
        </section>
      </div>

      {confirmReset ? (
        <DeleteConfirmModal
          title="Reset blog preview section"
