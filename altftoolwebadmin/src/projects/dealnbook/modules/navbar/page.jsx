...
export default function DealnbookNavbarPage() {
  ...
  async function handleRemoveLogo() {
    const path = settings.imagePath;
    setSettings((prev) => ({ ...prev, logoType: "text", logoImageUrl: "", imagePath: "" }));
    try {
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Dealnbook Navbar</h1>
              <p className="text-sm text-gray-500">Manage the logo, CTA button, and primary navigation links.</p>
            </div>
          </div>
          ...
          <section className="space-y-5">
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-gray-400">Brand & CTA</p>
                  <h2 className="mt-1 text-base font-bold text-gray-900">Logo & CTA button</h2>
                </div>
              ...
              {loading ? (
                ...
                <div className="mt-5 space-y-4">
                  ...
                  <Field label="Logo Text" error={errors.logoText}>
                    <input value={settings.logoText || ""} onChange={(event) => setSetting("logoText", event.target.value)} className={inputClass} placeholder="Dealnbook" />
                  </Field>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="CTA Label"><input value={settings.ctaLabel || ""} onChange={(event) => setSetting("ctaLabel", event.target.value)} className={inputClass} placeholder="Get Started" /></Field>
                    <Field label="CTA Href"><input value={settings.ctaHref || ""} onChange={(event) => setSetting("ctaHref", event.target.value)} className={inputClass} placeholder="/contact" /></Field>
                  </div>
                </div>
              )}
            </div>
    ...
    <div className="overflow-hidden rounded-xl border border-orange-100 bg-orange-50/60">
      <div className="flex items-center justify-between gap-4 border-b border-orange-100 px-5 py-4">
        {settings.logoType === "image" && settings.logoImageUrl ? (
          <img src={settings.logoImageUrl} alt={settings.logoText || "Dealnbook logo"} className="h-8 max-w-40 object-contain" />
        ) : (
          <span className="text-lg font-bold tracking-tight text-gray-900">{settings.logoText || "Dealnbook"}</span>
        )}
