...
import {
  ...
  createSlug,
  deleteService,
  deleteServicesHeroImage,
  saveServicesSettings,
  subscribeServices,
  subscribeServicesSettings,
  toggleServiceStatus,
  updateService,
  uploadServicesHeroImage,
...
export default function ServicesPage() {
  ...
  async function handleHeroUpload(file) {
    ...
    setUploadProgress(0);
    try {
      const uploaded = await uploadServicesHeroImage({ file, onProgress: setUploadProgress });
      setSettingsDraft((prev) => ({ ...prev, heroImage: uploaded.url }));
  ...
  async function handleRemoveHero() {
    setSettingsDraft((prev) => ({ ...prev, heroImage: "" }));
    try {
      await deleteServicesHeroImage(settings.heroImage);
    } catch {
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Dealnbook Services</h1>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-5">
            ...
            <h2 className="mt-1 text-base font-bold text-gray-900">Services list page content</h2>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Hero Subcopy"><textarea value={settingsDraft.heroSubcopy || ""} onChange={(event) => updateSettingsDraft("heroSubcopy", event.target.value)} rows={2} className={textareaClass} /></Field>
          </div>
          <div className="mt-4">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <div className="flex h-16 w-28 items-center justify-center rounded-lg border border-gray-200 bg-white">
                    {settingsDraft.heroImage ? (
                      <img src={settingsDraft.heroImage} alt="Hero preview" className="h-full w-full object-cover" />
                    ) : (
                      <ImageIcon className="h-6 w-6 text-gray-300" />
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-gray-800">Upload the services hero image</p>
                    {uploading ? (
                  ...
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-700">
                    ...
                    Upload
                    <input type="file" accept="image/*" className="hidden" disabled={uploading} onChange={(event) => handleHeroUpload(event.target.files?.[0])} />
                  </label>
                  {settingsDraft.heroImage ? (
                    <button type="button" onClick={handleRemoveHero} className="rounded-xl border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-50">Remove</button>
                  ) : null}
                </div>
              </div>
            </Field>
      ...
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Slug" error={errors.slug}><input value={form.slug} onChange={(event) => setSlug(event.target.value)} className={inputClass} placeholder="hotel-booking-deals" /></Field>
            <Field label="Icon">
          ...
          <div className="rounded-xl border border-gray-200 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Heading"><input value={form.pricingCtaHeading} onChange={(event) => setField("pricingCtaHeading", event.target.value)} className={inputClass} /></Field>
              <Field label="Button Label"><input value={form.pricingCtaLabel} onChange={(event) => setField("pricingCtaLabel", event.target.value)} className={inputClass} placeholder="Get a Quote" /></Field>
              <Field label="Body"><input value={form.pricingCtaBody} onChange={(event) => setField("pricingCtaBody", event.target.value)} className={inputClass} /></Field>
              <Field label="Button Href"><input value={form.pricingCtaHref} onChange={(event) => setField("pricingCtaHref", event.target.value)} className={inputClass} placeholder="/contact" /></Field>
            </div>
          </div>
