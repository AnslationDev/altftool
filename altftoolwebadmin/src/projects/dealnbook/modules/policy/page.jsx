      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Dealnbook Privacy Policy</h1>
              <p className="text-sm text-gray-500">Manage the privacy policy title, effective date, and sections.</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
...
function PolicyPreview({ data }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border" style={{ borderColor: DN.border, background: DN.bg }}>
      <div className="border-b p-6" style={{ borderColor: DN.border, background: DN.raised }}>
        <h3 className="text-2xl font-bold leading-tight" style={{ color: DN.text }}>
          {data?.title || "Privacy Policy"}
        </h3>
        {data?.effectiveDate ? (
          <p className="mt-3 text-sm leading-relaxed" style={{ color: DN.dim }}>
            {data.effectiveDate}
          </p>
        ) : null}
      </div>
      ...
      <div className="flex flex-col gap-6 p-6">
        {sections.length ? (
          sections.map((section, index) => (
            <div key={index}>
              <h4 className="text-base font-bold leading-snug" style={{ color: DN.text }}>
                {section.title || `Section ${index + 1}`}
              </h4>
              <p className="mt-2 text-sm leading-relaxed" style={{ color: DN.dim }}>
                {section.body}
              </p>
            </div>
          ))
        ) : (
          <p className="text-sm" style={{ color: DN.dim }}>
            No sections yet.
          </p>
        )}
      </div>
