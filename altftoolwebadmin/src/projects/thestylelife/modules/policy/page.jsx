...
import { ArrowDown, ArrowUp, Eye, Loader2, Plus, Save, ScrollText, Trash2 } from "lucide-react";
...
export default function ThestylelifePolicyPage() {
  return (
    <LegalDocPage
      icon={ScrollText}
      title="TheStyleLife Privacy Policy"
      subtitle="Manage the privacy policy title, subcopy, and sections."
...
/**
 * Shared editor for the two legal docs (policy + terms) — title, subcopy,
 * and an orderable sections repeater with a themed live preview.
 *
 * NOTE: TheStyleLife's shape carries `subcopy` (the PageHero line rendered
 * directly under the page title) instead of Shophobia's `lastUpdated` field —
 * it's edited as a textarea here since it's a full sentence, not a short date.
 */
              ...
              <>
                <Card eyebrow="Header" title="Page header">
                  <div className="flex flex-col gap-4">
                    ...
                    <Field label="Subcopy" hint="Appears directly under the page title on the live site, e.g. &quot;Last updated July 1, 2026. ...&quot;">
                      <textarea value={data.subcopy || ""} onChange={(event) => setField("subcopy", event.target.value)} rows={3} className={textareaClass} />
                    </Field>
                  </div>
                </Card>
