...
export default function ThestylelifeFooterPage() {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">TheStyleLife Footer</h1>
            <p className="text-sm text-gray-500">
              Manage column headings, links, and the copyright line. The services column comes from the Services module.
            </p>
          </div>
        </div>
        ...
        >
          <SettingsCard
            ...
            fields={[
              ...
              { key: "resourcesHeading", label: "Resources Heading", type: "text", half: true },
        ...
        <SectionFrame index={2} icon={Link2} title="Link Columns" subtitle="The quick-links column plus the resource/legal links.">
          <CollectionManager
            eyebrow="Footer"
            title="Quick links"
            itemNoun="link"
    ...
    <PreviewShell>
      <div className="p-6">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <div>
            <p className="text-[9px] font-semibold uppercase tracking-[0.2em]" style={{ color: HEX.dim }}>
              {settings.resourcesHeading || "Resources"}
            </p>
            <ul className="mt-2 space-y-1">
              {activeResources.length ? activeResources.map((link) => (
                <li key={link.id} className="truncate text-[11px]" style={{ color: HEX.fg }}>{link.label}</li>
              )) : (
                <li className="text-[11px]" style={{ color: HEX.dim }}>No links yet</li>
              )}
          ...
          <div>
            <p className="text-[9px] font-semibold uppercase tracking-[0.2em]" style={{ color: HEX.dim }}>
              {settings.newsletterHeading || "The Edit"}
            </p>
            <p className="mt-2 text-[10px] leading-relaxed" style={{ color: HEX.dim }}>
              {settings.newsletterBlurb || "Monthly notes on brand and growth."}
            </p>
        ...
        <div className="mt-5 border-t pt-3 text-center text-[10px]" style={{ borderColor: HEX.border, color: HEX.dim }}>
          © {year} TheStyleLife Studio. {settings.copyrightSuffix || "All rights reserved."}
        </div>
      </div>
    </PreviewShell>
  );
