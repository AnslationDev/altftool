...
import { fetchBlogById, updateBlog, isSlugTaken } from "../../services/blogsService";
...
export default function EditBlogPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      emitAlert({ type: "error", title: "Load failed", message: err?.message });
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (taken) {
        emitAlert({ type: "error", title: "Slug taken", message: "This slug is already in use." });
        return;
      }
    } catch {
      // best-effort
    ...
    try {
      ...
      emitAlert({ type: "success", title: "Saved", message: "Blog post updated successfully." });
      router.push("/smartlucky/blog");
    } catch (err) {
      emitAlert({ type: "error", title: "Save failed", message: err?.message || "Could not update blog." });
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center p-16">
        <div className="flex items-center gap-3 text-sm" style={{ color: "var(--muted)" }}>
          <Loader2 size={18} className="animate-spin" /> Loading blog…
        </div>
      </div>
    );
  }
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center p-16 text-sm" style={{ color: "var(--muted)" }}>
        <p className="font-semibold text-base" style={{ color: "var(--foreground)" }}>Blog not found</p>
        <p className="mt-1">This blog post may have been deleted.</p>
        <button onClick={() => router.push("/smartlucky/blog")}
          className="mt-4 inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-xs font-bold transition hover:opacity-90"
      ...
      <div className="sticky top-0 z-10 -mx-4 mb-5 border-b px-4 py-4 backdrop-blur sm:-mx-6 sm:px-6"
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="truncate text-lg font-extrabold sm:text-xl" style={{ color: "var(--foreground)" }}>Edit Blog Post</h1>
              <p className="mt-0.5 text-xs" style={{ color: "var(--muted)" }}>Editing: {form.heading || "Untitled"}</p>
            </div>
          </div>
