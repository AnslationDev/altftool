...
export default function OfferhopprFooterPage() {
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Offerhoppr Footer</h1>
              <p className="text-sm text-gray-500">Manage the footer column headings, newsletter copy, resource links, and copyright.</p>
            </div>
          </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-base font-bold text-gray-900">Column headings & newsletter</h2>
                </div>
              ...
              {loading ? (
                ...
                <div className="mt-5 space-y-4">
                  ...
                  <Field label="Copyright Suffix" error={errors.copyrightSuffix}>
                    <input value={settings.copyrightSuffix || ""} onChange={(event) => setSetting("copyrightSuffix", event.target.value)} className={inputClass} placeholder="All rights reserved." />
                    <p className="mt-1.5 text-xs text-gray-400">Appended after the auto-generated “© {new Date().getFullYear()} {"{Site Name}"}” prefix on the frontend.</p>
                  </Field>
                </div>
              )}
            </div>
...
function FooterPreview({ settings, resources }) {
  ...
  const Column = ({ title, links }) => (
    <div>
      <h3 className="text-[10px] font-semibold uppercase tracking-[0.15em] text-teal-700">{title}</h3>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-teal-100 bg-teal-50/60 p-6">
      <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
        <div>
          <span className="text-lg font-bold tracking-tight text-gray-900">Offerhoppr</span>
      ...
      <div className="mt-6 border-t border-teal-100 pt-4">
        <p className="text-[10px] text-gray-500">© {new Date().getFullYear()} Offerhoppr. {settings.copyrightSuffix || "All rights reserved."}</p>
      </div>
    </div>
  );
}
