    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Offerhoppr Testimonials</h1>
              <p className="text-sm text-gray-500">Manage the customer testimonials shown on the Offerhoppr site.</p>
            </div>
          </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_170px]">
              <label className="relative">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} className={`${inputClass} pl-10`} placeholder="Search by author or role" />
              </label>
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-gray-200">
              <div className="min-w-[620px]">
                <div className="grid grid-cols-[1fr_1fr_100px_80px_120px] bg-gray-50 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  <span>Author</span><span>Role</span><span>Rating</span><span>Status</span><span>Actions</span>
                </div>
                {loading ? (
      ...
      {deleteTarget ? (
        <DeleteConfirmModal
          ...
          message={`Delete testimonial from "${deleteTarget.author}"?`}
            ...
            <figure key={item.id} className="flex w-full flex-col gap-4 rounded-lg border border-gray-200 bg-white p-5">
              <div className="flex gap-1" aria-label={`${item.rating} out of 5 stars`}>
                {Array.from({ length: 5 }).map((_, index) => (
                  <Star
                    key={index}
                    className={`h-4 w-4 ${index < (Number(item.rating) || 0) ? "fill-amber-400 text-amber-400" : "text-gray-300"}`}
                  />
                ))}
              </div>
