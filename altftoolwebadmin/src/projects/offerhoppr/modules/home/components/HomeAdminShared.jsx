...
/**
 * OfferHoppr — Home admin shared primitives.
 *
 * The OfferHoppr home page is a single admin page composed of several
 * sections. Each section is either a singleton settings document
 * (`home/<key>`) or a settings document paired with an ordered collection.
 * To keep the page DRY we drive every editor from a small config array,
 * mirroring the config-driven `SettingsCard` / `CollectionManager` pattern
 * used across the other admin modules (e.g. Samvatsara's home module).
 *
 * Field types supported by SettingsCard:
 *   text | textarea | list | number
 *
 * `list` renders a lightweight repeatable-text-input editor (add/remove
 * rows) for simple `string[]` fields such as hero stickers or about
 * highlights — not a Firestore sub-collection.
 */
...
/* -------------------------------------------------------------------------- */
/* OfferHoppr preview theme (light — preview only)                            */
/* -------------------------------------------------------------------------- */
export const HEX = {
  bg: "#faf8f4",
  raised: "#ffffff",
...
export function Eyebrow({ children }) {
  return (
    <p className="text-[10px] font-semibold uppercase tracking-[0.25em]" style={{ color: HEX.dim }}>
      {children || "EYEBROW"}
    </p>
  );
}

/** Renders a heading the way the OfferHoppr frontend does. */
...
export function CollectionManager({
  ...
  const hasImageColumn = columns.some((col) => col.image);
