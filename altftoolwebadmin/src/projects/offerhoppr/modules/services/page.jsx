...
const EMPTY_SERVICE = {
  ...
  workflow: [],
  technologies: "",
  pricingCta: { headline: "", subcopy: "", buttonLabel: "" },
  faq: [],
  relatedSlugs: [],
  order: 0,
  active: true,
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Offerhoppr Services</h1>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="CTA Headline"><input value={settingsDraft.ctaHeadline || ""} onChange={(event) => updateSettingsDraft("ctaHeadline", event.target.value)} className={inputClass} /></Field>
          </div>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <Field label="CTA Subcopy"><textarea value={settingsDraft.ctaSubcopy || ""} onChange={(event) => updateSettingsDraft("ctaSubcopy", event.target.value)} rows={2} className={textareaClass} /></Field>
          </div>
          <div className="mt-4">
            <Field label="CTA Button Label"><input value={settingsDraft.ctaButtonLabel || ""} onChange={(event) => updateSettingsDraft("ctaButtonLabel", event.target.value)} className={inputClass} placeholder="Get Started" /></Field>
          </div>
...
function ServiceModal({ mode, service, services, onClose }) {
  ...
  const [form, setForm] = useState(() => ({
    ...EMPTY_SERVICE,
    ...service,
    icon: SERVICE_ICONS.includes(service?.icon) ? service.icon : EMPTY_SERVICE.icon,
      ...
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Slug" error={errors.slug}><input value={form.slug} onChange={(event) => setSlug(event.target.value)} className={inputClass} placeholder="google-ads-management" /></Field>
            <Field label="Icon">
          ...
          <Repeater
            ...
            rows={form.workflow}
            onAdd={() => addRow("workflow", { step: "", description: "" })}
            onRemove={(index) => removeRow("workflow", index)}
            render={(row, index) => (
              <div className="grid gap-2 sm:grid-cols-[minmax(0,0.9fr)_minmax(0,1.4fr)]">
                <input value={row.step || ""} onChange={(event) => updateRow("workflow", index, { step: event.target.value })} className={inputClass} placeholder="Step" />
          ...
          <div className="rounded-xl border border-gray-200 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <Field label="Subcopy"><input value={form.pricingCta.subcopy} onChange={(event) => setPricing("subcopy", event.target.value)} className={inputClass} /></Field>
              <Field label="Button Label"><input value={form.pricingCta.buttonLabel} onChange={(event) => setPricing("buttonLabel", event.target.value)} className={inputClass} placeholder="Get a Quote" /></Field>
            </div>
          </div>
