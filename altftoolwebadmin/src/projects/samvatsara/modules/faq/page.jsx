      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Samvatsara FAQ</h1>
              <p className="text-sm text-gray-500">Manage the frequently asked questions shown on the Samvatsara site.</p>
            </div>
          </div>
              ...
              <div className="min-w-[620px]">
                ...
                {loading ? (
                  ...
                  <div key={item.id} className="grid grid-cols-[1.3fr_1.5fr_80px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                    ...
                    <p className="truncate text-xs font-semibold text-gray-500">{item.answer}</p>
                ...
                return (
                  <div key={item.id}>
                    <div className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left">
                      <span className="text-base font-medium text-[#1c1917]">{item.question}</span>
                      <span aria-hidden="true" className="shrink-0 text-lg text-[#78716c]">
                        {isOpen ? "−" : "+"}
                      </span>
                    </div>
                    {isOpen ? (
                      <p className="px-5 pb-5 text-sm leading-relaxed text-[#78716c]">{item.answer}</p>
                    ) : null}
                  </div>
                );
              })}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/55 p-4 backdrop-blur-sm">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          <Field label="Question" error={errors.question}><input value={form.question} onChange={(event) => setField("question", event.target.value)} className={inputClass} /></Field>
          <Field label="Answer" error={errors.answer}><textarea value={form.answer} onChange={(event) => setField("answer", event.target.value)} rows={5} className={textareaClass} /></Field>
