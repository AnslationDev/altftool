...
export default function PolicyPage() {
  const [data, setData] = useState(DEFAULT_POLICY);
  const [savedData, setSavedData] = useState(DEFAULT_POLICY);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [errors, setErrors] = useState({});

  useEffect(() => {
    const unsub = subscribePolicy(
      (next) => {
        setData(next);
        setSavedData(next);
        setLoading(false);
      ...
      () => {
        emitAlert({ type: "error", message: "Failed to load privacy policy." });
        setLoading(false);
      },
    );
    return () => unsub();
  ...
  async function save() {
    const nextErrors = {};
    if (!data.hero?.title?.trim()) nextErrors.title = "Hero title is required.";
    ...
    try {
      ...
      emitAlert({ type: "success", message: "Privacy policy saved." });
    } catch (error) {
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Samvatsara Privacy Policy</h1>
              <p className="text-sm text-gray-500">Manage the privacy policy hero, sections, and contact block.</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          ...
          <>
            <Card eyebrow="Hero" title="Page hero">
              <div className="space-y-4">
                <Field label="Eyebrow"><input value={data.hero?.eyebrow || ""} onChange={(event) => setHero("eyebrow", event.target.value)} className={inputClass} /></Field>
                <Field label="Title" error={errors.title}><input value={data.hero?.title || ""} onChange={(event) => setHero("title", event.target.value)} className={inputClass} /></Field>
                <Field label="Body"><textarea value={data.hero?.body || ""} onChange={(event) => setHero("body", event.target.value)} rows={3} className={textareaClass} /></Field>
                <Field label="Last Updated"><input value={data.lastUpdated || ""} onChange={(event) => setField("lastUpdated", event.target.value)} className={inputClass} /></Field>
              </div>
            </Card>
            ...
            <Card eyebrow="Contact Block" title="Contact block">
              <div className="space-y-4">
                <Field label="Contact Heading"><input value={data.contactHeading || ""} onChange={(event) => setField("contactHeading", event.target.value)} className={inputClass} /></Field>
                <Field label="Contact Intro"><textarea value={data.contactIntro || ""} onChange={(event) => setField("contactIntro", event.target.value)} rows={3} className={textareaClass} /></Field>
              </div>
            </Card>
          </>
        )}
...
function PolicyPreview({ data }) {
  const hero = data?.hero || {};
  const sections = data?.sections || [];

  return (
    <div className="overflow-hidden rounded-xl border" style={{ borderColor: SV.border, background: SV.bg }}>
      {/* Hero */}
      <div className="border-b p-6" style={{ borderColor: SV.border, background: SV.raised }}>
        <p className="text-[10px] font-bold uppercase tracking-[0.22em]" style={{ color: SV.accent }}>
          {hero.eyebrow || "Legal"}
        </p>
        <h3 className="mt-3 text-2xl font-bold leading-tight" style={{ color: SV.text }}>
          {hero.title || "Privacy Policy"}
        </h3>
        {hero.body ? (
          <p className="mt-3 text-sm leading-relaxed" style={{ color: SV.dim }}>
            {hero.body}
          </p>
        ) : null}
      </div>
      ...
      <div className="flex flex-col gap-6 p-6">
        {sections.length ? (
          sections.map((section, index) => (
            <div key={index}>
              <h4 className="text-base font-bold leading-snug" style={{ color: SV.text }}>
                {section.title || `Section ${index + 1}`}
              </h4>
              <p className="mt-2 text-sm leading-relaxed" style={{ color: SV.dim }}>
                {section.body}
              </p>
            </div>
          ))
        ) : (
          <p className="text-sm" style={{ color: SV.dim }}>
            No sections yet.
          </p>
        )}

        {data?.contactHeading || data?.contactIntro ? (
          <div>
            <h4 className="text-base font-bold leading-snug" style={{ color: SV.text }}>
              {sections.length + 1}. {data.contactHeading || "Contact Us"}
            </h4>
            {data.contactIntro ? (
              <p className="mt-2 text-sm leading-relaxed" style={{ color: SV.dim }}>
                {data.contactIntro}
