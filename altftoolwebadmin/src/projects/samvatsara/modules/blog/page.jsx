      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Samvatsara Blog</h1>
              <p className="text-sm text-gray-500">Manage the blog list page and studio journal articles.</p>
            </div>
          </div>
              ...
              {loading ? (
                ...
                <div key={item.id} className="grid grid-cols-[60px_1fr_1fr_110px_90px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                  <div className="h-11 w-11 overflow-hidden rounded-xl border border-gray-200 bg-gray-50">
                    {item.image || item.imageUrl ? <img src={item.image || item.imageUrl} alt="" className="h-full w-full object-cover" /> : <ImageIcon className="m-3 h-5 w-5 text-gray-300" />}
                  </div>
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-base font-bold text-gray-900">Blog page header &amp; SEO</h2>
        </div>
        <div className="flex items-center gap-3">
      ...
      {loading ? (
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="SEO Title"><input value={settings.seoTitle || ""} onChange={(event) => setField("seoTitle", event.target.value)} className={inputClass} /></Field>
            <Field label="SEO Description"><textarea value={settings.seoDescription || ""} onChange={(event) => setField("seoDescription", event.target.value)} rows={2} className={textareaClass} /></Field>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[260px_1fr]">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Category"><input value={form.category} onChange={(event) => setField("category", event.target.value)} className={inputClass} placeholder="Studio Life" /></Field>
              <Field label="Author (team slug)"><input value={form.author} onChange={(event) => setField("author", event.target.value)} className={inputClass} placeholder="marisol-abbott" /></Field>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Cover Gradient"><input value={form.coverGradient} onChange={(event) => setField("coverGradient", event.target.value)} className={inputClass} placeholder="from-terracotta to-blush" /></Field>
        </div>
