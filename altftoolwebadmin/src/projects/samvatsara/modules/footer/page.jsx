...
export default function SamvatsaraFooterPage() {
  ...
  async function saveSettings() {
    const nextErrors = {};
    if (!settings.copyrightText?.trim()) nextErrors.copyrightText = "Copyright text is required.";
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Samvatsara Footer</h1>
              <p className="text-sm text-gray-500">Manage the footer column headings, resource links, and copyright.</p>
            </div>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <StatCard label="Inactive Resources" value={loading ? "-" : resources.length - activeResources} tone="amber" />
        </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-base font-bold text-gray-900">Column headings</h2>
                </div>
              ...
              {loading ? (
                ...
                <div className="mt-5 space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <Field label="Resources Heading"><input value={settings.resourcesHeading || ""} onChange={(event) => setSetting("resourcesHeading", event.target.value)} className={inputClass} placeholder="Resources" /></Field>
                  ...
                  <Field label="Copyright Text" error={errors.copyrightText}>
                    <input value={settings.copyrightText || ""} onChange={(event) => setSetting("copyrightText", event.target.value)} className={inputClass} placeholder="© Samvatsara. All rights reserved." />
                  </Field>
                </div>
              )}
            </div>
          ...
          <section className="space-y-5">
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <FooterPreview settings={settings} resources={resources} />
            </div>
          </section>
        </div>
      </div>
      ...
      {modal ? (
        <LinkModal mode={modal.mode} item={modal.item} items={resources} onClose={() => setModal(null)} />
      ) : null}
      {deleteTarget ? (
        <DeleteConfirmModal
          title="Delete link"
...
function FooterPreview({ settings, resources }) {
  const res = [...resources]
  ...
  const Column = ({ title, links }) => (
    <div>
      <h3 className="text-[10px] font-semibold uppercase tracking-[0.15em] text-[#C96F4A]">{title}</h3>
      <ul className="mt-3 space-y-2">
        {links.length ? (
          links.map((link) => (
            <li key={link.id} className="truncate text-xs text-[#2B211C]/80">{link.label}</li>
          ))
        ) : (
          <li className="text-xs text-[#2B211C]/40">No links</li>
        )}
      </ul>
    </div>
  );
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#E8B4A6]/40 bg-[#FBF3EC] p-6">
      <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
        <div>
          <span className="font-serif text-lg font-semibold tracking-tight text-[#2B211C]">Samvatsara</span>
          <p className="mt-4 max-w-sm text-xs leading-relaxed text-[#2B211C]/60">
            {settings.newsletterHeading || "Newsletter"}
          </p>
        </div>
        <div className="grid grid-cols-3 gap-6">
          <Column title={settings.quickLinksHeading || "Quick Links"} links={[]} />
          <Column title={settings.servicesHeading || "Services"} links={[]} />
          <Column title={settings.resourcesHeading || "Resources"} links={res} />
        </div>
      </div>
      <div className="mt-6 border-t border-[#E8B4A6]/40 pt-4">
        <p className="text-[10px] text-[#2B211C]/50">{settings.copyrightText || "© Samvatsara. All rights reserved."}</p>
      </div>
    </div>
  );
}
...
function LinkModal({ mode, item, items, onClose }) {
  ...
  async function save() {
    ...
    try {
      if (mode === "edit") {
        await updateFooterResource(item.id, form);
        emitAlert({ type: "success", message: "Resource updated." });
      } else {
        await createFooterResource(form);
        emitAlert({ type: "success", message: "Resource added." });
      }
      onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/55 p-4 backdrop-blur-sm">
      <div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-gray-400">{mode === "edit" ? "Edit Resource" : "Add Resource"}</p>
            <h3 className="mt-1 text-lg font-bold text-gray-900">Footer resource link</h3>
          </div>
        ...
        <div className="mt-5 flex justify-end gap-3">
          ...
          <button onClick={save} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-gray-700 disabled:opacity-60">
            ...
            {mode === "edit" ? "Update Resource" : "Add Resource"}
          </button>
        </div>
      </div>
    </div>
