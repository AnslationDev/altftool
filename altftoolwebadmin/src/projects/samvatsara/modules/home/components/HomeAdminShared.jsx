...
/**
 * Samvatsara — Home admin shared primitives.
 *
 * The Samvatsara home page is a single admin page composed of many sections.
 * Each section is either a singleton settings document (`home/<key>`) or a
 * settings document paired with an ordered collection. To keep the page DRY we
 * drive every editor from a small config array, mirroring the config-driven
 * `SettingsCard` / `CollectionManager` pattern DailyHnt uses.
 *
 * Field types supported:
 *   text | textarea | lines | number | select | cta | image
 */
...
/* -------------------------------------------------------------------------- */
/* Samvatsara preview theme (light / cream — preview only)                    */
/* -------------------------------------------------------------------------- */
export const HEX = {
  bg: "#faf8f4",
  raised: "#ffffff",
...
/** Renders a lead + italic heading pair the way the Samvatsara frontend does. */
export function PreviewHeading({ lead, italic, className = "" }) {
  return (
    <h3 className={`font-semibold leading-tight ${className}`} style={{ color: HEX.fg }}>
      {lead || "Heading goes here"}{" "}
      {italic ? <em className="italic" style={{ color: HEX.dim }}>{italic}</em> : null}
    </h3>
  );
}
...
export function ImagePicker({ label, url, error, aspect = "aspect-video", uploading, progress, onUpload, onRemove }) {
  return (
    <div>
      <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-gray-500">{label}</span>
            ...
            if (field.type === "select") {
              return (
                <Field key={field.key} label={field.label} error={errors[field.key]} hint={field.hint}>
                  <select value={form[field.key] || ""} onChange={(event) => setValue(field.key, event.target.value)} className={inputClass}>
...
function CollectionModal({ mode, item, items, fields, imageField, title, itemNoun, create, update, onClose }) {
  ...
  const [form, setForm] = useState(() => {
    ...
    fields.forEach((field) => {
      base[field.key] = item?.[field.key] ?? (field.type === "select" ? field.options?.[0] ?? "" : "");
    });
    if (imageField) {
              ...
              <div key={field.key} className={field.half ? "" : "sm:col-span-2"}>
                <Field label={field.label} error={errors[field.key]} hint={field.hint}>
                  {field.type === "textarea" ? (
                    ...
                    <select value={form[field.key] || ""} onChange={(event) => setField(field.key, event.target.value)} className={inputClass}>
                      {(field.options || []).map((option) => (
                        <option key={option} value={option}>{option}</option>
                      ))}
                    </select>
