...
export default function ViewCards() {
  ...
  const categories = useMemo(() => ["all", ...Array.from(new Set(cards.map((b) => b.category?.trim()).filter(Boolean))).sort()], [cards]);
  

  const filtered = useMemo(() => cards.filter((b) => {
    if (statusFilter   !== "all" && b.status   !== statusFilter)   return false;
    if (categoryFilter !== "all" && b.category !== categoryFilter) return false;
;
    if (search) {
      const q = search.toLowerCase();
      return b.heading?.toLowerCase().includes(q) || b.category?.toLowerCase().includes(q);
    }
    return true;
  }), [cards, search, categoryFilter, statusFilter]);
  ...
  const published        = cards.filter((b) => b.status === "published").length;
  const drafts           = cards.filter((b) => b.status !== "published").length;
  const uniqueCategories = new Set(cards.map((b) => b.category).filter(Boolean)).size;
        ...
        {/* KPIs */}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                  <CategoryDistributionCard
                    title="Cards Category Distribution"
                    items={categoryData}
                    total={cards.length}
                ...
                <div key={cards.id} onClick={() => router.push(`/leadtree/credit-cards/view-cards/${cards.id}`)}
                  ...
                  {cards.image ? (
                    <div className="h-44 overflow-hidden bg-gray-100 shrink-0">
                      <img src={cards.image} alt={cards.heading} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    </div>
                  ) : (
                  ...
                  <div className="p-5 space-y-3 flex-1 flex flex-col">
                    <div className="flex items-start justify-between gap-2">
                      <h2 className="text-sm font-bold text-gray-900 line-clamp-2 leading-snug flex-1">{cards.heading}</h2>
                      <StatusBadge status={cards.status} />
                    </div>
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-gray-500">
                      <span className="font-medium"></span>
                      {cards.date && <><span className="text-gray-300">·</span><span>{cards.date}</span></>}
                      {cards.category && (
                        <span className="bg-indigo-50 text-indigo-600 font-semibold px-2 py-0.5 rounded-full">{cards.category}</span>
                      )}
                    </div>
