...
export default function SnapageeBlogPage() {
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Snapagee Blog</h1>
              <p className="text-sm text-gray-500">Manage the articles shown on /blog, article pages, and the home preview.</p>
            </div>
          </div>
        ...
        <SettingsCard
          eyebrow="/blog Page Header"
          title="Page header & related strip"
          defaults={DEFAULT_BLOG_SETTINGS}
          subscribe={subscribeBlogSettings}
          save={saveBlogSettings}
          errorLabel="blog page header"
          fields={[
            { key: "pageEyebrow", label: "Page Eyebrow", type: "text", placeholder: "The Blog" },
            { key: "pageHeading", label: "Page Heading", type: "text", required: true },
            { key: "pageDescription", label: "Page Description", type: "textarea", rows: 3 },
            { key: "relatedEyebrow", label: "Related Eyebrow", type: "text", half: true, placeholder: "Keep Reading" },
            { key: "relatedHeading", label: "Related Heading", type: "text", half: true, placeholder: "More from the blog" },
          ]}
        />
...
function ArticleModal({ mode, article, articles, onClose }) {
  ...
  const uploadImage = useMemo(() => createArticleImageUploader(form.slug), [form.slug]);
      ...
      <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Category"><input value={form.category} onChange={(event) => setField("category", event.target.value)} className={inputClass} placeholder="Growth" /></Field>
            <Field label="Author"><input value={form.author} onChange={(event) => setField("author", event.target.value)} className={inputClass} placeholder="Jordan Blake" /></Field>
