      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">ExclusInsider Team</h1>
...
function MemberModal({ mode, member, members, onClose }) {
  ...
  const [form, setForm] = useState(() => ({
    ...EMPTY_MEMBER,
    ...member,
    focus: Array.isArray(member?.focus) ? member.focus.join("\n") : member?.focus || "",
      ...
      <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[220px_1fr]">
          <div>
            ...
            <div className="mt-3">
              <Field label="Initials"><input value={form.initials} onChange={(event) => setField("initials", event.target.value)} className={inputClass} placeholder="JD" maxLength={3} /></Field>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <Field label="Focus Areas (one per line)"><textarea value={form.focus} onChange={(event) => setField("focus", event.target.value)} rows={4} className={textareaClass} placeholder={"Asset Verification\nDue Diligence\nRisk Analysis"} /></Field>
            <Field label="Quote"><textarea value={form.quote} onChange={(event) => setField("quote", event.target.value)} rows={2} className={textareaClass} /></Field>
            <Field label="Status">
