      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">ExclusInsider Testimonials</h1>
              <p className="text-sm text-gray-500">Manage the client testimonials shown on the ExclusInsider site.</p>
            </div>
          </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_170px]">
              <label className="relative">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} className={`${inputClass} pl-10`} placeholder="Search by name, title, or result" />
              </label>
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-gray-200">
              <div className="min-w-[640px]">
                <div className="grid grid-cols-[1fr_1.2fr_1fr_80px_120px] bg-gray-50 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  <span>Name</span><span>Title</span><span>Result</span><span>Status</span><span>Actions</span>
                </div>
                {loading ? (
                  ...
                  <div key={item.id} className="grid grid-cols-[1fr_1.2fr_1fr_80px_120px] items-center gap-3 border-t border-gray-100 px-3 py-3 text-sm">
                    <p className="truncate font-bold text-gray-900">{item.name}</p>
                    <p className="truncate text-xs font-semibold text-gray-500">{item.title}</p>
                    <p className="truncate text-xs font-semibold text-gray-500">{item.result}</p>
    ...
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-gray-50">
      <div className="p-6">
        <div className="text-center">
          <p className="text-[11px] uppercase tracking-[0.2em] text-gray-500">Kind words</p>
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
          {activeItems.length ? activeItems.map((item) => (
            <figure key={item.id} className="flex w-full flex-col gap-4 rounded-lg border border-gray-200 bg-white p-5">
              <blockquote className="flex-1 text-sm leading-relaxed text-gray-900">
                &ldquo;{item.quote}&rdquo;
              </blockquote>
              <figcaption className="border-t border-gray-200 pt-4 text-[11px] uppercase tracking-[0.08em] text-gray-500">
                {item.name}
                {item.title ? ` — ${item.title}` : ""}
              </figcaption>
              {item.result ? (
                <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-emerald-600">{item.result}</p>
              ) : null}
            </figure>
          )) : (
            <div className="col-span-full rounded-lg border border-gray-200 bg-white p-8 text-center text-sm text-gray-500">
      ...
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Title"><input value={form.title} onChange={(event) => setField("title", event.target.value)} className={inputClass} placeholder="CEO, Direct-to-Consumer Apparel Brand" /></Field>
            <Field label="Result"><input value={form.result} onChange={(event) => setField("result", event.target.value)} className={inputClass} placeholder="Revenue +182% in 14 months" /></Field>
