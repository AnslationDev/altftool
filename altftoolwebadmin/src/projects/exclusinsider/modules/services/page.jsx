...
const EMPTY_SERVICE = {
  ...
  workflow: [],
  technologies: "",
  pricingCta: { eyebrow: "", headline: "", body: "", ctaLabel: "" },
  faq: [],
  relatedSlugs: [],
  order: 0,
  active: true,
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">ExclusInsider Services</h1>
              <p className="text-sm text-gray-500">Manage the service dossiers shown on the site.</p>
            </div>
          </div>
      ...
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="mt-5 space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Slug" error={errors.slug}><input value={form.slug} onChange={(event) => setSlug(event.target.value)} className={inputClass} placeholder="asset-verification" /></Field>
            <Field label="Icon">
            ...
            <Field label="Dossier Number"><input value={form.dossierNumber} onChange={(event) => setField("dossierNumber", event.target.value)} className={inputClass} placeholder="EI-004" /></Field>
          ...
          <Repeater
            label="Features"
            rows={form.features}
            onAdd={() => addSimpleRow("features")}
            onRemove={(index) => removeSimpleRow("features", index)}
            render={(row, index) => (
              <input value={row || ""} onChange={(event) => updateSimpleRow("features", index, event.target.value)} className={inputClass} placeholder="Feature" />
            )}
          />
          ...
          <div className="rounded-xl border border-gray-200 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Eyebrow"><input value={form.pricingCta.eyebrow} onChange={(event) => setPricingCta("eyebrow", event.target.value)} className={inputClass} /></Field>
              <Field label="Headline"><input value={form.pricingCta.headline} onChange={(event) => setPricingCta("headline", event.target.value)} className={inputClass} /></Field>
              <Field label="Body"><input value={form.pricingCta.body} onChange={(event) => setPricingCta("body", event.target.value)} className={inputClass} /></Field>
              <Field label="Button Label"><input value={form.pricingCta.ctaLabel} onChange={(event) => setPricingCta("ctaLabel", event.target.value)} className={inputClass} placeholder="Request Retainer" /></Field>
            </div>
          </div>
