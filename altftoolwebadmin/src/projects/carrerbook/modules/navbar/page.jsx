...
function NavbarPreview({ settings, items }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-[#14062d] shadow-sm">
      <div className="flex min-h-20 items-center gap-5 bg-[linear-gradient(110deg,#241153,#050505_45%,#1c0a3d)] px-5 py-4 text-white">
        <div className="flex min-w-0 flex-1 items-center gap-3">
          {settings.logoType === "image" && settings.logoImageUrl ? (
            <img
              src={settings.logoImageUrl}
              alt={settings.logoText || "CareerBook logo"}
              className="h-10 max-w-48 rounded-md object-contain"
            />
          ) : (
            <span className="truncate text-2xl font-bold tracking-normal">
              {settings.logoText || "CarrerBook"}
            </span>
          )}
        </div>
...
export default function CareerBookNavbarPage() {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">CareerBook Navbar</h1>
                      ...
                      <tr>
                        <td colSpan={5} className="px-4 py-14 text-center">
                          ...
                          <p className="mt-1 text-xs text-gray-400">
                            Add the first CareerBook navigation item.
                          </p>
                        </td>
                      </tr>
                    )}
      ...
      {deleteTarget ? (
        <DeleteConfirmModal
          ...
          message={`Delete "${deleteTarget.title}" from the CareerBook navbar?`}
          confirmText="Delete"
