...
import { ArrowRight, BadgeCheck, Image as ImageIcon, Loader2, PanelsTopLeft } from "lucide-react";
import { ABOUT_SECTIONS, DEFAULT_ABOUT_SECTIONS, subscribeAboutSection } from "./service/about.service";

const ICONS = {
  "hero-section": ImageIcon,
  "lead-section": BadgeCheck,
};
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">CareerBook About Us</h1>
              <p className="text-sm text-gray-500">Manage About Us hero content and quality lead cards.</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {ABOUT_SECTIONS.map((section) => {
            const Icon = ICONS[section.key] || PanelsTopLeft;
            ...
            return (
              <div key={section.key} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <Link href={`/carrerbook/about-us/${section.key}`} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-gray-700">
                  Edit {section.label}
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 text-sm font-bold text-gray-900">projects / carrerbook / about-us</p>
          <p className="mt-2 text-xs font-semibold text-gray-500">hero-section, lead-section, lead-section/cards</p>
        </div>
      </div>
    </div>
  );
