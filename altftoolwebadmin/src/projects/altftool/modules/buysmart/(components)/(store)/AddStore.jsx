...
function AddStore({ setActive, editStore, setEditStore }) {
  ...
  useEffect(() => {
    if (editStore) {
      ...
      if (editStore.imageWidth && editStore.imageHeight) {
        setImageSize({
          width: editStore.imageWidth,
          height: editStore.imageHeight,
        });
      }
    } else {
      setForm(defaultForm); 
  ...
  const handleSubmit = async (e) => {
    ...
    if (editStore) {
      await firebaseBuySmartStoreSource.update(editStore.id, payload);
    } else {
      await firebaseBuySmartStoreSource.add({
        ...payload,
        createdAt: Date.now(),
      });
    }
  ...
  return (
    ...
    <div className="mb-8 flex justify-between ">
      ...
      <h2 className="text-3xl font-bold text-gray-800">
        {editStore ? "Edit Store Banner" : "Create Store Banner"}
      </h2>
    ...
    <form onSubmit={handleSubmit} className="space-y-10">
      ...
      <div className="flex justify-end gap-4 cursor-pointer">
        ...
        <button
          ...
          className={`px-6 py-3 rounded-sm text-white cursor-pointer flex items-center gap-2 transition ${
            loading || imageError
