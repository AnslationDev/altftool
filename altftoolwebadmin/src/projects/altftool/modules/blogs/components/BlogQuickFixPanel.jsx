    ...
    <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-xs font-black uppercase tracking-widest text-muted">Quick Fix</h2>
            <p className="mt-1 text-xs leading-5 text-muted">Contextual fixes from the current SEO and schema audit.</p>
          </div>
        </div>
      ...
      <div className="mt-4 space-y-2">
        <p className="text-[10px] font-black uppercase tracking-wider text-muted">Open gaps</p>
        {audit.gaps.length ? (
          audit.gaps.slice(0, 5).map((gap) => (
            <div key={gap.key} className="flex items-start gap-2 rounded-xl bg-surface-soft px-3 py-2">
              <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-warning" />
              <div className="min-w-0">
                <p className="text-xs font-bold text-foreground">{gap.label}</p>
                <p className="mt-0.5 line-clamp-2 text-[11px] leading-4 text-muted">{gap.detail}</p>
              </div>
            </div>
          ))
        ) : (
          <div className="flex items-center gap-2 rounded-xl bg-success-soft px-3 py-3 text-sm font-semibold text-success">
      ...
      <div className="mt-4 flex items-start gap-2 rounded-xl border border-primary bg-primary-soft px-3 py-2.5">
        <RefreshCw className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" />
        <p className="text-xs leading-5 text-primary">Blocks are duplicate-safe, so repeated fixes skip already-added FAQ, source, and refresh sections.</p>
      </div>
    </div>
  );
}
