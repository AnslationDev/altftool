...
function statusTone(preview) {
  if (preview.health.richResultReady) return "border-green-100 bg-green-50 text-green-700";
  if (preview.health.articleSchemaReady) return "border-amber-100 bg-amber-50 text-amber-700";
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-gray-100 px-5 py-4">
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-wider text-blue-600">Schema preview</p>
            <h2 id="schema-preview-title" className="mt-1 line-clamp-2 text-xl font-black text-gray-900">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-500 transition hover:bg-gray-50"
        ...
        <div className="max-h-[56vh] overflow-y-auto px-5 py-4">
          <div className="grid grid-cols-3 gap-2 text-center">
            {preview.schemas.map((schema) => (
              <div key={schema.key} className={`rounded-xl px-3 py-3 ${schema.ready ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
          ...
          {preview.missingRequired.length || preview.missingRecommended.length ? (
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <article className="rounded-xl border border-red-100 bg-red-50/60 p-3">
                <p className="text-[10px] font-black uppercase tracking-wider text-red-700">Required gaps</p>
                <div className="mt-2 space-y-1.5">
                  {preview.missingRequired.length ? (
                    preview.missingRequired.map((issue) => (
                      <p key={issue.key} className="text-xs font-semibold leading-5 text-red-700">
                        {issue.label}: {issue.detail}
                      </p>
                    ))
                  ) : (
                    <p className="text-xs font-semibold text-green-700">No required schema gaps.</p>
                  )}
                </div>
              </article>
              <article className="rounded-xl border border-amber-100 bg-amber-50/60 p-3">
                <p className="text-[10px] font-black uppercase tracking-wider text-amber-700">Rich result gaps</p>
                <div className="mt-2 space-y-1.5">
                  {preview.missingRecommended.length ? (
                    preview.missingRecommended.slice(0, 5).map((issue) => (
                      <p key={issue.key} className="text-xs font-semibold leading-5 text-amber-700">
                        {issue.label}: {issue.detail}
                      </p>
                    ))
                  ) : (
                    <p className="text-xs font-semibold text-green-700">Rich-result inputs look ready.</p>
                  )}
                </div>
              </article>
            </div>
          ...
          <div className="mt-4 rounded-xl border border-gray-100 bg-gray-950 p-3">
            <pre className="max-h-80 overflow-auto whitespace-pre-wrap break-words text-xs leading-5 text-gray-100">{jsonText}</pre>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 px-5 py-4">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex h-9 items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 text-xs font-bold text-gray-700 transition hover:bg-gray-50"
          >
          ...
          <button
            type="button"
            onClick={copyJson}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-gray-900 px-4 text-sm font-semibold text-white transition hover:bg-gray-700"
          >
      ...
      {missing.length ? (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {missing.slice(0, 3).map((issue) => (
            <span key={`${preview.blog.id}-${issue.key}`} className="rounded-lg bg-white/80 px-2 py-1 text-[10px] font-bold uppercase tracking-wide">
              {issue.label}
            </span>
          ))}
          {missing.length > 3 ? (
            <span className="rounded-lg bg-white/70 px-2 py-1 text-[10px] font-black">+{missing.length - 3}</span>
          ) : null}
        </div>
      ) : null}
  ...
  return (
    <>
      <section className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-wider text-gray-500">Schema preview</p>
            <h2 className="mt-1 text-xl font-black text-gray-900">{summary.richReady}/{summary.total} rich ready</h2>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <SchemaStat label="Article" value={summary.articleReady || 0} tone="bg-green-50 text-green-700" />
          <SchemaStat label="Rich" value={summary.richReady || 0} tone={summary.richReady === summary.total ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"} />
          <SchemaStat label="FAQ" value={summary.faqReady || 0} tone="bg-blue-50 text-blue-700" />
          <SchemaStat label="Sources" value={summary.sourceReady || 0} tone="bg-slate-100 text-slate-700" />
        </div>

        <div className="mt-4 space-y-2">
          <div className="flex items-center justify-between gap-3">
            <p className="text-[10px] font-black uppercase tracking-wider text-gray-400">Schema queue</p>
