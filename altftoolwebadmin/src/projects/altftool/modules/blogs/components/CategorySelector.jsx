...
import { fetchCategoryNames } from "../services/blogsService";
...
export default function CategorySelector({ value, onChange }) {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      {open && (
        <div className="absolute left-0 right-0 mt-1 bg-surface border rounded shadow z-20 max-h-56 overflow-y-auto">
          {filtered.length > 0 ? (
            filtered.map((name) => (
              <div
                key={name}
                ...
                className={`px-3 py-2 cursor-pointer hover:bg-surface-soft ${
                  value === name ? "bg-surface-soft" : ""
                }`}
              >
                {name} {/* display EXACT */}
              </div>
