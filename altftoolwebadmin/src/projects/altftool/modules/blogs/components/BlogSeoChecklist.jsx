  ...
  return (
    <div className="bg-surface rounded-2xl border border-border shadow-sm p-5 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-xs font-black text-muted uppercase tracking-widest">Quality & SEO</h2>
          <p className="mt-1 text-xs text-muted">Publish-ready score for search, social, and reading flow.</p>
        </div>
      ...
      <div className="h-2 w-full overflow-hidden rounded-full bg-surface-soft">
        <div
          className={`h-full rounded-full transition-all ${score >= 80 ? "bg-success" : score >= 55 ? "bg-warning" : "bg-danger"}`}
          style={{ width: `${score}%` }}
        />
      </div>
      ...
      <div className="space-y-2.5">
        {checks.map((check) => (
          <div key={check.label} className="flex items-start justify-between gap-3 text-xs">
            ...
            <span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${check.done ? "bg-success-soft" : "bg-warning-soft"}`}>
              {check.done ? (
      ...
      {quality.suggestions.length > 0 && (
        <div className="rounded-2xl border border-primary bg-primary-soft px-3 py-3">
          <div className="mb-2 flex items-center gap-1.5 text-[11px] font-black uppercase tracking-widest text-primary">
            <Sparkles className="h-3.5 w-3.5" />
            Next improvements
          </div>
          <div className="space-y-1.5">
            {quality.suggestions.slice(0, 3).map((suggestion) => (
              <p key={suggestion} className="text-xs leading-5 text-primary">
                {suggestion}
              </p>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 gap-2 border-t border-border pt-3 text-[11px]">
        <div className="flex items-center gap-1.5 rounded-xl bg-surface-soft px-2.5 py-2 text-muted">
