...
export default function BlogLivePreview({
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-black uppercase tracking-widest text-muted">Live Preview</h2>
          </div>
      ...
      <div className="mb-4 rounded-2xl border border-border bg-surface-soft p-3">
        <div className="mb-2 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted">
          <Search className="h-3.5 w-3.5" />
          Search Result
        </div>
        <p className="truncate text-[11px] text-success">altftool.com/blogs/{preview.slug || "new-blog"}</p>
        <p className="mt-1 line-clamp-1 text-sm font-semibold text-primary">{preview.seoTitle}</p>
      ...
      <div className={`mx-auto transition-all ${mode === "mobile" ? "max-w-[280px]" : "max-w-full"}`}>
        <div className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
          <div className="relative flex h-36 items-end bg-surface-soft">
            {imagePreview ? (
              <img
                src={imagePreview}
            ...
            <div className="relative p-3 text-white">
              <span className="inline-flex h-6 items-center rounded-lg bg-primary px-2 text-[10px] font-bold uppercase tracking-wide">
                {preview.category}
              </span>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px] text-white/80">
                ...
                <span className="h-1 w-1 rounded-full bg-surface/35" />
          ...
          <div className="max-h-56 overflow-hidden p-4">
            <div
              className="space-y-3 text-sm leading-6 text-foreground [&_a]:font-semibold [&_a]:text-primary [&_h2]:text-base [&_h2]:font-bold [&_h3]:font-semibold [&_img]:rounded-xl [&_li]:ml-4 [&_ul]:list-disc"
      ...
      {preview.warnings.length ? (
        <div className="mt-4 space-y-2">
          {preview.warnings.slice(0, 4).map((warning) => (
            <div key={warning} className="flex items-center gap-2 rounded-xl border border-warning bg-warning-soft px-3 py-2 text-xs font-medium text-warning">
        ...
        <div className="mt-4 rounded-xl border border-success bg-success-soft px-3 py-2 text-xs font-semibold text-success">
