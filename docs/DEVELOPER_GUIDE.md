...
`seo:route-quality:report` reconciles every prerendered HTML page with its title, description, canonical, robots directive, H1, structured-data presence, and membership across every sitemap advertised by the rendered `robots.txt` (including recursively expanded sitemap indexes). It also reads compiled `X-Robots-Tag` response rules so header-level noindex policies are audited alongside HTML metadata. The command writes the bounded dashboard artifact to `altftoolwebadmin/src/data/routeQualityReport.json`. `seo:index-control` rejects sitemap/noindex conflicts and missing indexable canonical targets, except for the narrow Lexicon families whose alternate or archival pages are deliberately discovered through internal links instead of submitted through the bounded corpus sitemap. `seo:route-quality:strict` blocks malformed metadata, missing H1s, and index-control failures; ideal title and description lengths remain visible as non-blocking advisories in the report. The root `npm run build` refreshes this artifact between the web and admin builds so the admin dashboard always bundles evidence from the same render.
