...
export const spec = {
  ...
  compute: (values) => { const num=(v)=>typeof v==="number"?v:Number(v); const money=(n)=>Number.isFinite(Number(n))?Number(n).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"—"; const P = num(values.amount), rate = num(values.rate), r = rate / 100 / 12, n = num(values.years) * 12; if (P <= 0 && n <= 0) return { result: "—", caption: "Loan amount and term must be greater than zero." }; if (P <= 0) return { result: "—", caption: "Loan amount must be greater than zero." }; if (n <= 0) return { result: "—", caption: "Term must be greater than zero." }; if (rate < 0) return { error: "Interest rate cannot be negative." }; const emi = r === 0 ? P / n : (P * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1); return { result: money(emi) + " / month", rows: [["Total interest", money(emi * n - P)], ["Total payable", money(emi * n)]] }; },
};
