...
import { ShoppingBag, Check, Copy, RotateCcw } from "lucide-react";
...
export default function ToolHome() {
  const [appName, setAppName] = useState("My shopping app");
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Shopping App Permission Audit</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          A shopping app needs a delivery address, a payment method and a camera for the odd barcode. Everything else — contacts, SMS, call logs, the calendar — is collection for advertising. Tick what yours holds to see the gap.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="shop-app-name">
        ...
        {mode === "paste" ? (
          <div className="mt-4">
            <label className={LABEL_CLASS} htmlFor="shop-paste">
              ...
              <ul className="mt-3 space-y-2">
                {PERMISSIONS.map((permission) => (
                  <li key={permission.id}>
                    <label
                      htmlFor={`shop-perm-${permission.id}`}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational only. Revoking a permission never deletes what an app already sent; use the account's data-download and data-deletion options as well. Under India's DPDP Act 2023 you can ask a company to erase personal data it no longer needs.
      </p>
    </main>
  );
}
