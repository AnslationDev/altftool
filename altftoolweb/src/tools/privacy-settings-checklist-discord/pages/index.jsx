...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, MessageSquare } from "lucide-react";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (!hasScore) return "";
    const lines = [
      "Discord Privacy Settings Checklist",
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          ...
          Discord privacy audit
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Discord Privacy Settings Checklist</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Twenty-six Discord controls, ranked by how much exposure each one actually closes. Discord's defaults are built for making friends, not for staying private — pick the profile that matches why you are here, tick what you have already changed, and the score shows what is still open.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="dc-profile">
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]" aria-labelledby="dc-score">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p id="dc-score" className="text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
              Protection score
            </p>
        ...
        {hasScore && score.capped ? (
          <p className="mt-4 flex items-start gap-2 rounded-md bg-[var(--warning-soft)] px-3 py-2 text-sm text-[var(--warning)]">
            ...
            <span>
              ...
              ones that stay exploitable no matter what else you tidy up.
            </span>
          </p>
        ) : null}
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Turning off &ldquo;Allow direct messages from server members&rdquo; globally and assuming it applied everywhere. It only governs servers you join afterwards &mdash; every server you were already in keeps its own override.</li>
          <li>Unticking &ldquo;Everyone&rdquo; on friend requests but leaving &ldquo;Server members&rdquo; ticked, which lets anyone in a shared server request you anyway.</li>
          <li>Leaving activity status on, which broadcasts every application you run to everyone who can see your profile.</li>
          <li>Sharing a whole screen instead of one window, so window titles, notifications and file paths carrying your real name go out live.</li>
          <li>Pasting a script into the browser console because a &ldquo;giveaway bot&rdquo; asked. That steals your session token and walks straight past two-factor authentication.</li>
          <li>Deleting your account to remove your messages. Messages belong to the channel &mdash; they stay, attributed to a Deleted User.</li>
        </ul>
      </section>

      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Everything you tick stays in this browser tab &mdash; nothing is uploaded and the page never asks
        for an account name, password or code. {PLATFORM.note} Menu wording moves between releases, so
