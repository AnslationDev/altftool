...
export default function BmrCalculator() {
  ...
  // before it fires, so it never calls setState on an unmounted component.
  useEffect(() => {
    return () => {
  ...
  const handleReset = () => {
    if (
      !window.confirm(
        "Reset every field? This will clear your entered profile and any calculated results and cannot be undone.",
      )
    ) {
      return;
    }
  ...
  const calorieGoals = {
    ...
    maintain: {
      ...
      value: finalTdee,
      colorClass: "text-blue-600 dark:text-blue-400 bg-blue-50/40 border-blue-250/20",
...
- Ideal Daily Water Intake: ${finalMinWater} - ${finalMaxWater} ${waterUnit}/day`;
  ...
  return (
    <div className="bg-[var(--page)] text-[var(--foreground)] min-h-screen py-10 px-4 md:px-8 flex flex-col items-center justify-center font-sans">
      <div className="w-full max-w-5xl space-y-6">
        ...
        <div className="bg-gradient-to-r from-emerald-500/10 via-teal-500/5 to-transparent rounded-3xl p-6 md:p-8 border border-emerald-500/10 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
          <div className="flex-1 space-y-4 z-10">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 font-bold text-xs uppercase tracking-wider">
          ...
          <div className="relative flex items-center justify-center w-48 h-48 md:w-56 md:h-56 z-10 shrink-0">
            ...
            {/* Floating Pill Tags */}
            <div className="absolute -top-1 right-2 bg-[var(--card)] px-3 py-1.5 rounded-full border border-[var(--border)] shadow-sm text-[10px] font-bold text-[var(--muted-foreground)] flex items-center gap-1">
              <Compass className="w-3 h-3 text-emerald-500" />
              Know your baseline
            </div>
            <div className="absolute top-20 -left-6 bg-[var(--card)] px-3 py-1.5 rounded-full border border-[var(--border)] shadow-sm text-[10px] font-bold text-[var(--muted-foreground)] flex items-center gap-1">
              <Apple className="w-3 h-3 text-emerald-500" />
              Plan your nutrition
            </div>
            <div className="absolute -bottom-2 right-4 bg-[var(--card)] px-3 py-1.5 rounded-full border border-[var(--border)] shadow-sm text-[10px] font-bold text-[var(--muted-foreground)] flex items-center gap-1">
        ...
        {/* Loading State */}
        {isLoading && (
          <div className="bg-[var(--card)] border border-[var(--border)]/60 rounded-3xl p-12 shadow-sm text-center space-y-6 max-w-md mx-auto animate-pulse flex flex-col items-center justify-center min-h-[400px]">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-[var(--foreground)]">Analyzing Profile</h3>
        ...
        {!isLoading && !showResult && (
          <div className="bg-[var(--card)] border border-[var(--border)]/60 rounded-3xl p-6 md:p-8 shadow-sm space-y-6 w-full">
            ...
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-3">
              ...
              <button
                onClick={handleReset}
                className="flex items-center gap-1 text-[11px] font-bold text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-[var(--muted-foreground)] uppercase tracking-wider block">Unit System</label>
                <div className="grid grid-cols-2 p-1 bg-[var(--muted)] rounded-xl border border-[var(--border)]/40">
                  <button
                    onClick={() => {
                      setUnitSystem("metric");
                      setWeight(68);
                    ...
                    className={`py-2 rounded-lg text-xs font-bold transition-all ${
                      unitSystem === "metric"
                        ? "bg-[var(--card)] text-emerald-600 dark:text-emerald-400 shadow-sm border border-emerald-500/10"
              ...
              {/* Activity Level */}
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-[var(--muted-foreground)] uppercase tracking-wider block">Activity Level</label>
                <div className="relative flex items-center">
                  ...
                  <ChevronDown className="w-4 h-4 text-[var(--muted-foreground)] absolute right-3 pointer-events-none" />
                </div>
                <p className="text-[10px] text-[var(--muted-foreground)] font-medium pl-1">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    max="120"
                    value={age}
                    onChange={(e) => setAge(e.target.value === "" ? "" : parseFloat(e.target.value) || 0)}
                    className="w-full bg-[var(--muted)]/50 border border-[var(--border)]/80 rounded-xl pl-10 pr-12 py-2.5 text-xs font-bold focus:outline-none focus:border-emerald-500 focus:bg-[var(--card)] transition-colors"
                  />
                  <span className="absolute right-3.5 text-[9px] font-bold text-[var(--muted-foreground)] uppercase">years</span>
                </div>
              </div>
                  ...
                  {unitSystem === "metric" ? (
                    <div className="w-full pl-10 relative flex items-center">
                      <input
                        ...
                        className="w-full bg-[var(--muted)]/50 border border-[var(--border)]/80 rounded-xl px-3.5 pr-12 py-2.5 text-xs font-bold focus:outline-none focus:border-emerald-500 focus:bg-[var(--card)] transition-colors"
                      />
                      <span className="absolute right-3.5 text-[9px] font-bold text-[var(--muted-foreground)]">cm</span>
                    </div>
                  ) : (
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    className="w-full bg-[var(--muted)]/50 border border-[var(--border)]/80 rounded-xl pl-10 pr-12 py-2.5 text-xs font-bold focus:outline-none focus:border-emerald-500 focus:bg-[var(--card)] transition-colors"
                  />
                  <span className="absolute right-3.5 text-[9px] font-bold text-[var(--muted-foreground)] uppercase font-bold">
            ...
            <button
              ...
              className="w-full bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-[var(--primary-foreground)] font-bold py-3.5 px-4 rounded-xl text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-emerald-700/10 transition-colors mt-6"
            >
        ...
        {!isLoading && showResult && (
          <div className="bg-[var(--card)] border border-[var(--border)]/60 rounded-3xl p-6 md:p-8 shadow-sm space-y-6 w-full animate-fadeIn">

            {/* Header Action Row */}
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-[var(--border)] pb-4 gap-4">
              <div>
                <h2 className="text-lg font-bold text-[var(--foreground)]">Your Results</h2>
                <p className="text-[10px] text-[var(--muted-foreground)] font-medium">Metabolic profiles calculated successfully.</p>
              </div>
              <div className="flex gap-2 self-end">
                <button
                  onClick={handleReset}
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-[var(--border)] text-xs font-bold text-[var(--foreground)] hover:bg-[var(--muted)] transition-colors"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
              ...
              {/* BMR Card */}
              <div className="bg-[var(--muted)]/50 p-6 rounded-2xl border border-[var(--border)] flex flex-col justify-between text-center relative overflow-hidden">
                <div>
                  <h3 className="text-xs font-bold text-[var(--muted-foreground)] uppercase tracking-widest block mb-4">Basal Metabolic Rate (BMR)</h3>
                  <div className="text-4xl font-black text-emerald-600 dark:text-emerald-400 tracking-tight">
                    {finalBmr.toLocaleString()}
                  </div>
                  <div className="text-xs font-bold text-emerald-700 dark:text-emerald-300/80 dark:text-emerald-300/80 uppercase tracking-widest mt-1">
                    calories/day
                  </div>
                </div>
                <div className="text-[10px] text-[var(--muted-foreground)] leading-relaxed mt-6 border-t border-[var(--border)]/70 pt-3">
                  This is the number of calories your body burns at rest.
                </div>
              </div>

              {/* TDEE Card */}
              <div className="bg-[var(--muted)]/50 p-6 rounded-2xl border border-[var(--border)] flex flex-col justify-between text-center">
                <div>
                  <h3 className="text-xs font-bold text-[var(--muted-foreground)] uppercase tracking-widest block mb-4">Total Daily Energy Expenditure (TDEE)</h3>
                  <div className="text-4xl font-black text-[var(--foreground)] tracking-tight">
                    {finalTdee.toLocaleString()}
                  </div>
                  <div className="text-xs font-bold text-[var(--muted-foreground)] uppercase tracking-widest mt-1">
                    calories/day
                  </div>
                </div>
                <div className="text-[10px] text-[var(--muted-foreground)] leading-relaxed mt-6 border-t border-[var(--border)]/70 pt-3">
              ...
              <div className="bg-[var(--muted)]/50 p-6 rounded-2xl border border-[var(--border)] flex flex-col items-center justify-between text-center">
                <h3 className="text-xs font-bold text-[var(--muted-foreground)] uppercase tracking-widest block mb-2">Calorie Level</h3>
                <div className="relative w-48 h-24 overflow-hidden flex items-center justify-center">
                  ...
                  <div className="absolute bottom-0 text-center">
                    <span className="text-lg font-black text-[var(--foreground)] tracking-tight">{calorieCategory}</span>
                  </div>
                </div>
                <div className="text-[10px] text-[var(--muted-foreground)] leading-relaxed border-t border-[var(--border)]/70 pt-3 w-full">
                  {calorieDesc}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 pt-4">
              ...
              {/* BMI Badge */}
              <div className="bg-[var(--muted)]/50 p-4 rounded-xl border border-[var(--border)] flex items-center gap-3">
                <div className="p-2.5 rounded-lg bg-[var(--card)] border border-[var(--border)] text-[var(--muted-foreground)]">
                  <Activity className="w-4 h-4" />
                </div>
                <div>
                  <span className="text-[9px] text-[var(--muted-foreground)] uppercase tracking-wider block font-bold">BMI</span>
                  <span className="text-xs font-black text-[var(--foreground)] block">{finalBmi}</span>
              ...
              <div className="bg-[var(--muted)]/50 p-4 rounded-xl border border-[var(--border)] flex items-center gap-3">
                ...
                <div>
                  <span className="text-[9px] text-[var(--muted-foreground)] uppercase tracking-wider block font-bold">Healthy Weight</span>
                  <span className="text-xs font-black text-[var(--foreground)] block mt-0.5">
                    {finalMinWeight} – {finalMaxWeight}
                  </span>
                  <span className="text-[9px] text-[var(--muted-foreground)] font-bold uppercase">{unitSystem === "metric" ? "kg" : "lbs"}</span>
                </div>
              </div>
              ...
              <div className="bg-[var(--muted)]/50 p-4 rounded-xl border border-[var(--border)] flex items-center gap-3">
                ...
                <div>
                  <span className="text-[9px] text-[var(--muted-foreground)] uppercase tracking-wider block font-bold">Body Fat (Est.)</span>
                  <span className="text-xs font-black text-[var(--foreground)] block">{finalBodyFat}%</span>
              ...
              <div className="bg-[var(--muted)]/50 p-4 rounded-xl border border-[var(--border)] flex items-center gap-3">
                ...
                <div>
                  <span className="text-[9px] text-[var(--muted-foreground)] uppercase tracking-wider block font-bold">Protein Intake</span>
                  <span className="text-xs font-black text-[var(--foreground)] block mt-0.5">
                    {minProtein} – {maxProtein}
                  </span>
                  <span className="text-[9px] text-[var(--muted-foreground)] font-bold uppercase">g / day</span>
                </div>
              </div>
              ...
              <div className="bg-[var(--muted)]/50 p-4 rounded-xl border border-[var(--border)] flex items-center gap-3">
                ...
                <div>
                  <span className="text-[9px] text-[var(--muted-foreground)] uppercase tracking-wider block font-bold">Water Intake</span>
                  <span className="text-xs font-black text-[var(--foreground)] block mt-0.5">
                    {finalMinWater} – {finalMaxWater}
                  </span>
                  <span className="text-[9px] text-[var(--muted-foreground)] font-bold uppercase">{waterUnit} / day</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-[var(--border)]">
              ...
              {/* Daily Calorie Goal */}
              <div className="bg-[var(--muted)]/30 p-5 rounded-2xl border border-[var(--border)] flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <Target className="w-5 h-5 text-emerald-500" />
                    <div>
                      <h4 className="text-xs font-black text-[var(--foreground)] uppercase tracking-wider">Daily Calorie Goal</h4>
                      <p className="text-[10px] text-[var(--muted-foreground)] font-medium">Choose your goal to see daily calorie target</p>
                    </div>
                  </div>
                      ...
                      return (
                        <label
                          ...
                          className={`flex items-center justify-between p-3 rounded-xl border text-xs font-bold cursor-pointer transition-all ${
                            isSelected
                              ? "bg-[var(--card)] border-emerald-500/25 shadow-sm text-[var(--foreground)]"
                              : "bg-[var(--card)]/40 border-[var(--border)] text-[var(--muted-foreground)] hover:bg-[var(--card)]/70"
                          }`}
                        >
                          <div className="flex items-center gap-2.5">
                            <input
                              ...
                              checked={isSelected}
                              onChange={() => {}}
                              className={`w-3.5 h-3.5 border-[var(--border-strong)] ${goal.radioColor}`}
                            />
                            <span>{goal.label}</span>
                          </div>
                          <span className={isSelected ? "text-emerald-600 dark:text-emerald-400 font-extrabold" : "text-[var(--muted-foreground)]"}>
                ...
                <div className="mt-5 p-3 rounded-xl bg-amber-50/30 dark:bg-amber-500/10 border border-amber-100/50 dark:border-amber-400/20 text-[10px] text-amber-700 dark:text-amber-300 font-bold flex items-center gap-1.5">
                  <span>💡</span>
              ...
              <div className="bg-[var(--muted)]/30 p-5 rounded-2xl border border-[var(--border)] flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <h4 className="text-xs font-black text-[var(--foreground)] uppercase tracking-wider">Macronutrient Breakdown</h4>
                      <p className="text-[10px] text-[var(--muted-foreground)] font-medium">Balanced split for clean performance</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-around gap-4 mt-6">
                    ...
                    <div className="relative w-28 h-28 flex items-center justify-center shrink-0">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                        <span className="text-sm font-black text-[var(--foreground)] tracking-tight">{targetKcal.toLocaleString()}</span>
                        <span className="text-[8px] font-bold text-[var(--muted-foreground)] uppercase tracking-widest">kcal</span>
                      </div>
                    </div>

                    {/* Macro details list */}
                    <div className="space-y-3 w-full sm:w-auto flex-1">
                      <div className="flex justify-between items-center text-xs font-bold border-b border-[var(--border)] pb-1.5">
                        <span className="flex items-center gap-1.5 text-[var(--muted-foreground)]">
                          <span className="w-2.5 h-2.5 rounded-full bg-emerald-500" />
                          Protein (25%)
                        </span>
                        <span className="text-[var(--foreground)] font-black">{proteinG}g</span>
                      </div>
                      <div className="flex justify-between items-center text-xs font-bold border-b border-[var(--border)] pb-1.5">
                        <span className="flex items-center gap-1.5 text-[var(--muted-foreground)]">
                        ...
                        <span className="text-[var(--foreground)] font-black">{carbsG}g</span>
                      </div>
                      <div className="flex justify-between items-center text-xs font-bold pb-0.5">
                        ...
                        <span className="text-[var(--foreground)] font-black">{fatG}g</span>
                      </div>
                    </div>
                  </div>
                </div>

                <div className="mt-5 p-3 rounded-xl bg-emerald-50/30 dark:bg-emerald-500/10 border border-emerald-100/50 dark:border-emerald-400/20 text-[10px] text-emerald-700 dark:text-emerald-300 font-bold flex items-center gap-1.5">
                  <span>💡</span>
              ...
              <div className="bg-[var(--muted)]/30 p-5 rounded-2xl border border-[var(--border)] flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <h4 className="text-xs font-black text-[var(--foreground)] uppercase tracking-wider">Calorie Breakdown (Example)</h4>
                      <p className="text-[10px] text-[var(--muted-foreground)] font-medium">Suggested calorie allocation per meal</p>
                    </div>
                  </div>
                  <div className="space-y-2 mt-4">
                    <div className="flex items-center justify-between p-3 rounded-xl bg-[var(--card)] border border-[var(--border)] text-xs font-bold">
                      <span className="text-[var(--muted-foreground)]">Breakfast (25%)</span>
                      <span className="text-[var(--foreground)] font-black">{breakfastKcal} kcal</span>
                    </div>
                    <div className="flex items-center justify-between p-3 rounded-xl bg-[var(--card)] border border-[var(--border)] text-xs font-bold">
                      <span className="text-[var(--muted-foreground)]">Lunch (35%)</span>
                      <span className="text-[var(--foreground)] font-black">{lunchKcal} kcal</span>
                    </div>
                    <div className="flex items-center justify-between p-3 rounded-xl bg-[var(--card)] border border-[var(--border)] text-xs font-bold">
                      <span className="text-[var(--muted-foreground)]">Dinner (30%)</span>
                      <span className="text-[var(--foreground)] font-black">{dinnerKcal} kcal</span>
                    </div>
                    <div className="flex items-center justify-between p-3 rounded-xl bg-[var(--card)] border border-[var(--border)] text-xs font-bold">
                      <span className="text-[var(--muted-foreground)]">Snacks (10%)</span>
                      <span className="text-[var(--foreground)] font-black">{snacksKcal} kcal</span>
                    </div>
                  </div>
                </div>
                <div className="mt-5 p-3 rounded-xl bg-sky-50/30 dark:bg-sky-500/10 border border-sky-100/50 dark:border-sky-400/20 text-[10px] text-sky-700 dark:text-sky-300 font-bold flex items-center gap-1.5">
                  <span>💡</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-[var(--border)]">
              ...
              <div className="bg-[var(--muted)]/30 p-5 rounded-2xl border border-[var(--border)] md:col-span-2 flex flex-col justify-between">
                <div>
                  <div className="flex items-center mb-4">
                    <div className="flex items-center gap-2">
                      <Flame className="w-5 h-5 text-emerald-500" />
                      <h4 className="text-xs font-black text-[var(--foreground)] uppercase tracking-wider">Calories Burned by Activities (30 min)</h4>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-4">
                    {/* Walking */}
                    <div className="bg-[var(--card)] p-3 rounded-xl border border-[var(--border)] flex flex-col items-center justify-center text-center">
                      <span className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 mb-2 font-bold text-xs">🚶</span>
                      <span className="text-[9px] text-[var(--muted-foreground)] font-bold block">Walking</span>
                      <span className="text-xs font-black text-[var(--foreground)] mt-1">{Math.round(weightKg * 1.9)} kcal</span>
                    </div>
                    {/* Running */}
                    <div className="bg-[var(--card)] p-3 rounded-xl border border-[var(--border)] flex flex-col items-center justify-center text-center">
                      <span className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 mb-2 font-bold text-xs">🏃</span>
                      <span className="text-[9px] text-[var(--muted-foreground)] font-bold block">Running</span>
                      <span className="text-xs font-black text-[var(--foreground)] mt-1">{Math.round(weightKg * 4.7)} kcal</span>
                    </div>
                    {/* Cycling */}
                    <div className="bg-[var(--card)] p-3 rounded-xl border border-[var(--border)] flex flex-col items-center justify-center text-center">
                      <span className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 mb-2 font-bold text-xs">🚴</span>
                      <span className="text-[9px] text-[var(--muted-foreground)] font-bold block">Cycling</span>
                      <span className="text-xs font-black text-[var(--foreground)] mt-1">{Math.round(weightKg * 3.8)} kcal</span>
                    </div>
                    {/* Swimming */}
                    <div className="bg-[var(--card)] p-3 rounded-xl border border-[var(--border)] flex flex-col items-center justify-center text-center">
                      <span className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 mb-2 font-bold text-xs">🏊</span>
                      <span className="text-[9px] text-[var(--muted-foreground)] font-bold block">Swimming</span>
                      <span className="text-xs font-black text-[var(--foreground)] mt-1">{Math.round(weightKg * 4.1)} kcal</span>
                    </div>
                    {/* Strength Training */}
                    <div className="bg-[var(--card)] p-3 rounded-xl border border-[var(--border)] flex flex-col items-center justify-center text-center">
                      <span className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 mb-2 font-bold text-xs">🏋️</span>
                      <span className="text-[9px] text-[var(--muted-foreground)] font-bold block">Strength</span>
                      <span className="text-xs font-black text-[var(--foreground)] mt-1">{Math.round(weightKg * 2.9)} kcal</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[var(--muted)]/30 p-5 rounded-2xl border border-[var(--border)] flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-xs font-black text-[var(--foreground)] uppercase tracking-wider">Health Insights</h4>
                  </div>
                  <div className="space-y-3">
                    <div className="flex items-start gap-2 text-xs font-bold text-[var(--muted-foreground)]">
                      <span className="text-emerald-500 font-bold text-sm shrink-0">✓</span>
                      <span>Your BMR of {finalBmr.toLocaleString()} kcal/day is calculated from your age, weight, height and sex.</span>
                    </div>
                    <div className="flex items-start gap-2 text-xs font-bold text-[var(--muted-foreground)]">
                      <span className="text-emerald-500 font-bold text-sm shrink-0">✓</span>
                      <span>You have a <span className="text-[var(--foreground)] font-black">{bmiCategory.toLowerCase()} BMI</span>. Great job!</span>
                    </div>
                    <div className="flex items-start gap-2 text-xs font-bold text-[var(--muted-foreground)]">
                      <span className="text-emerald-500 font-bold text-sm shrink-0">✓</span>
