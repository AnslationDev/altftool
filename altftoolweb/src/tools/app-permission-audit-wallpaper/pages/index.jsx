...
import { Wallpaper, Check, Copy, RotateCcw } from "lucide-react";
...
export default function ToolHome() {
  const [appName, setAppName] = useState("My wallpaper app");
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          <Wallpaper className="h-4 w-4" aria-hidden="true" />
          Permission audit
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Wallpaper App Permission Audit</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          A wallpaper app has one job: hand a bitmap to the system. Anything beyond an internet connection and the wallpaper API is almost always the advertising SDK bundled inside it. This audit scores exactly how far past that job yours reaches.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="wall-app-name">
        ...
        {mode === "paste" ? (
          <div className="mt-4">
            <label className={LABEL_CLASS} htmlFor="wall-paste">
              ...
              <ul className="mt-3 space-y-2">
                {PERMISSIONS.map((permission) => (
                  <li key={permission.id}>
                    <label
                      htmlFor={`wall-perm-${permission.id}`}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational only. Wallpaper apps are among the most common carriers of aggressive ad SDKs; if one holds overlay, install or accessibility access, uninstalling is usually a better answer than tuning its permissions.
      </p>
    </main>
  );
}
