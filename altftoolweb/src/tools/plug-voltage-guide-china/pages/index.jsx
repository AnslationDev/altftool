...
const PRESETS = [
  ...
  { label: "UK 230 V kettle", minV: "230", maxV: "240", freq: "50", watts: "2000" },
];
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (hasError) return "";
    return [
      "Plug and Voltage Guide for China",
      `Mainland China mains: ${MAINS_VOLTAGE_V} V, ${MAINS_FREQUENCY_HZ} Hz, type A, C and I sockets`,
  ...
  const rows = [
    ...
    [
      "Polarity matches the Chinese standard",
      hasError ? DASH : result.polarityUnverified ? "Unverified — same shape, other wiring" : "Yes",
    ],
    [
    ...
    [
      `Power a resistive appliance delivers at ${MAINS_VOLTAGE_V} V`,
      hasError || result.resistivePowerHereW === null
        ? DASH
        : `${NUM0.format(result.resistivePowerHereW)} W of its rated ${NUM0.format(toNum(deviceWatts))} W`,
    ],
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          ...
          Mainland China · 220 V · 50 Hz
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Plug and Voltage Guide for China</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Mainland China runs {MAINS_VOLTAGE_V} V at {MAINS_FREQUENCY_HZ} Hz on combination type A and type I wall
          sockets, with round-pin type C outlets also in use. Hong Kong and Macau are different again — enter your
          device to see what you actually need.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="cn-plug">
              Plug on your device
            </label>
            <select
              id="cn-plug"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="cn-minv">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="cn-maxv">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="cn-freq">
              Label frequency
            </label>
            <select
              id="cn-freq"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="cn-watts">
              Rated power (W)
            </label>
            <input
              id="cn-watts"
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={copyResult}
              disabled={hasError}
              aria-label="Copy the China plug and voltage verdict"
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <h2 className="text-lg font-semibold">Sockets you will meet in mainland China</h2>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <h2 className="text-lg font-semibold">Mainland China, Hong Kong and Macau are not the same</h2>
        <div className="mt-3 overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-sm">
            ...
            <tbody>
              {REGIONS.map((region) => (
                <tr key={region.code} className="border-b border-[var(--border)] align-top last:border-0">
                  <td className="py-2.5 pr-4 font-semibold">{region.label}</td>
                  <td className="py-2.5 pr-4">
                    {region.voltageV} V · {region.frequencyHz} Hz
                  </td>
                  <td className="py-2.5 pr-4">{region.plugs}</td>
                  <td className="py-2.5 text-[var(--muted-foreground)]">{region.note}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        draw a little more. The delivered-power figure applies to purely resistive appliances, where power falls with
        the square of the voltage — a switch-mode charger holds its output instead. The tool plans for the supply
        sitting anywhere between about {NUM0.format(SUPPLY_MIN_V)} V and {NUM0.format(SUPPLY_MAX_V)} V. This is
        general travel guidance, not an electrical inspection — ask a qualified electrician about anything unusual.
      </p>
    </main>
  );
}
