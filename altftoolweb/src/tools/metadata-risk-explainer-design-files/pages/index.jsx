...
import { HANDOFFS, SEVERITY_LABEL, assessDesignRisk, groupedCatalogue } from "../lib";
...
export default function ToolHome() {
  ...
  const [handoffId, setHandoffId] = useState(DEFAULT_HANDOFF);
  const [copied, setCopied] = useState(false);

  const result = useMemo(
    () => assessDesignRisk({ selectedIds: selected, handoffId }),
    [selected, handoffId],
  );
  ...
  const summary = useMemo(() => {
    if (hasError) return "";
    return [
      "Design hand-off metadata check",
      `Hand-off: ${result.handoff.label}`,
      `Exposure to the recipient: ${result.score}/100 (${result.band.label})`,
      `${result.surviving.length} of ${result.selectedCount} ticked signals travel with it`,
      "",
      "Clean up before hand-off:",
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Design File Metadata Explainer
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Layer names become SVG ids, hidden layers become PDF layers, and an XMP packet can list
          every file an asset was pasted from. Tick what is true of your document and pick how you
          are handing it over.
        </p>
      </header>

      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <label className="block text-sm font-semibold" htmlFor="design-handoff">
          How are you handing it over?
        </label>
        <select
          id="design-handoff"
          ...
          value={handoffId}
          onChange={(event) => {
            setHandoffId(event.target.value);
            setCopied(false);
          }}
        >
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <h2 className="text-base font-semibold">What is in the document?</h2>
            ...
            <fieldset key={group.name} className="border-0 p-0">
              ...
              <div className="mt-2 grid gap-2">
                {group.items.map((item) => {
                  const inputId = `design-item-${item.id}`;
                  return (
                    <div
                      key={item.id}
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy design hand-off metadata summary"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
      ...
      {!hasError && (
        <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
          <h2 className="text-base font-semibold">Clean these before you send the files</h2>
          {result.actions.length === 0 ? (
            <p className="mt-3 text-sm text-[var(--success)]">
              Nothing you ticked survives this hand-off method.
            </p>
          ) : (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Educational guidance only — no file is read or uploaded. Where a client NDA or a font licence
        governs what you may hand over, check the agreement rather than relying on a checklist.
      </p>
    </main>
  );
}
