...
export default function ProgressTracker({ readiness }) {
  return (
    <div className="mb-4 min-w-0 rounded-2xl border border-(--border) bg-(--muted)/30 p-3">
      ...
      <div className="grid min-w-0 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {readiness.checks.map((item) => (
          <div key={item.label} className="flex min-w-0 items-center gap-2 rounded-xl bg-(--background)/40 px-2.5 py-1.5 text-xs">
