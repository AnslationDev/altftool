...
import { CalendarSearch, Check, Copy, RotateCcw } from "lucide-react";
...
const DEFAULTS = {
  patternId: PATTERNS[0].id, // ISO 8601 date
  sample: "2026-07-26",
};
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Regex Library for Dates</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Seven copy-ready patterns covering ISO 8601 dates and timestamps, DD/MM/YYYY and US slash
          dates, clock times, and syslog / Apache log timestamps — each with live testing.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="rld-pattern">
              Date / time format
            </label>
            <select
              id="rld-pattern"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="rld-sample">
              Test a date, time or log line
            </label>
            <input
              id="rld-sample"
              className={`mt-2 ${INPUT_CLASS}`}
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyPattern}
              aria-label="Copy the selected date regex pattern"
              className={GHOST_BTN}
            >
              {copied ? (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Regex validates the shape of a date, not the calendar: month lengths and leap years need a
        real date parser. Use these patterns for format checks, log grepping and extraction, then
        parse the match to confirm the date actually exists.
      </p>
    </main>
  );
}
