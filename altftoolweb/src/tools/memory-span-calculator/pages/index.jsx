...
const MODES = [
  { id: "digits", label: "Digits Forward", icon: Hash, desc: "Remember a sequence of numbers in order" },
  { id: "digits-reverse", label: "Digits Reverse", icon: Hash, desc: "Remember numbers and recall them in reverse" },
  { id: "letters", label: "Letters", icon: LetterText, desc: "Remember a sequence of random letters" },
  { id: "visual", label: "Visual Colours", icon: Eye, desc: "Remember a sequence of colours shown on screen" },
];
...
function generateSequence(mode, length) {
  if (mode === "digits" || mode === "digits-reverse") {
    const seq = [];
  ...
  if (mode === "letters") {
    ...
    const seq = [];
    for (let i = 0; i < length; i++) seq.push(pool[Math.floor(Math.random() * pool.length)]);
    return seq;
  }
  if (mode === "visual") {
    const seq = [];
    for (let i = 0; i < length; i++) seq.push(COLOURS[Math.floor(Math.random() * COLOURS.length)]);
    return seq;
  }
  return [];
}

function getSequenceDisplay(mode, seq) {
  if (mode === "visual") return seq.map((c) => c.name).join(", ");
  return seq.join(", ");
}
...
function buildSummary(span, bestSpan, attempts, modeResults) {
  return [
    "Memory Span Calculator Summary",
    `Test mode: ${MODES.find((m) => m.id === attempts[0]?.mode)?.label || "Multiple"}`,
    `Current span: ${span} items`,
    `Best span: ${bestSpan} items`,
    `Total attempts: ${attempts.length}`,
    `Overall accuracy: ${attempts.length > 0 ? Math.round((attempts.filter((a) => a.correct).length / attempts.length) * 100) : 0}%`,
    `Grade: ${getSpanGrade(span).label}`,
    ...attempts.map((a, i) => `${i + 1}. ${a.mode} — Length ${a.length}: ${a.correct ? "Correct" : "Wrong"} ${a.correct ? `(${a.timeMs}ms)` : ""}`),
  ].join("\n");
}
...
function exportCsv(attempts, span, bestSpan) {
  ...
  a.href = url;
  a.download = "memory-span-test.csv";
...
export default function MemorySpanCalculator() {
  ...
  const startTest = useCallback(() => {
    cleanup();
    const seq = generateSequence(mode, span);
    setSequence(seq);
  ...
  const handleSubmit = useCallback(() => {
    ...
    const answer = isReverse ? correctSeq.join(", ") : getSequenceDisplay(mode, sequence);
  ...
  }, [userInput, mode, sequence, span, startTime, consecutiveCorrect, consecutiveWrong, maxConsecutiveCorrect, bestSpan, attempts]);
    ...
    <div className="bg-[var(--background)] text-[var(--foreground)] transition-colors py-6 px-4">
        <header className="mb-8 text-center">
          ...
          <p className="mx-auto mt-2 max-w-2xl text-[var(--muted-foreground)]">
            Measure your short-term memory capacity. See a sequence, recall it, and discover your digit span — a core metric in cognitive neuroscience.
          </p>
        </header>
                ...
                {sequence.map((item, i) => {
                  ...
                  if (mode === "visual") {
                    return (
                      <div
                        key={i}
                        className={`h-20 w-20 rounded-2xl transition-all duration-300 ${
                          isCurrent ? "scale-125 shadow-2xl ring-4 ring-amber-400" : isPast ? "scale-90 opacity-30" : "scale-90 opacity-10"
                        }`}
                  ...
                  return (
                    <span
                      ...
                      className={`text-5xl font-extrabold transition-all duration-300 ${
                        isCurrent ? "scale-125 text-[var(--foreground)] drop-shadow-lg" : isPast ? "scale-90 text-[var(--muted-foreground)] opacity-30" : "scale-90 opacity-10"
                      }`}
                    >
                      {item}
                    </span>
          ...
          {phase === PHASES.INPUT && (
            <div className="flex h-80 w-full flex-col items-center justify-center rounded-3xl border-4 border-[var(--primary)] bg-[var(--primary)]/5 transition-all sm:h-96">
              ...
              <p className="mb-4 text-xs text-[var(--muted-foreground)]">
                {mode === "visual" ? "Type colour names separated by commas (e.g. Red, Blue, Green)" : "Separate items with commas or spaces"}
              </p>
              <input
                type="text"
                value={userInput}
                ...
                autoFocus
                placeholder={mode === "visual" ? "Red, Blue, Green..." : "1, 2, 3..."}
