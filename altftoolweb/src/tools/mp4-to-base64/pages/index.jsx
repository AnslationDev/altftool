...
export default function ToolHome() {
  ...
  const download = () => {
    ...
    anchor.href = url;
    anchor.download = `${report.name || "video"}.base64.txt`;
    anchor.click();
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl leading-tight font-semibold sm:text-4xl">MP4 to Base64</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Turn a local video file into a Base64 data URL you can paste straight into HTML, CSS or
          JSON. The file never leaves your browser — encoding happens on this page.
        </p>
      </header>

      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <label className={LABEL_CLASS} htmlFor="video-file">
          Choose a video file (up to {formatBytes(MAX_FILE_BYTES)})
        </label>
        <input
          id="video-file"
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button
            ...
            aria-label="Convert the selected video to Base64"
          >
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              {hasError || !report
                ? "Pick a video file to see the encoded size."
                : `${formatBytes(report.size)} of video becomes ${NUM.format(report.dataUrlLength)} characters`}
            </p>
          </div>
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Detected type", hasError || !report ? DASH : report.mime],
            ["Container", hasError || !report ? DASH : report.containerLabel],
            ["Original size", hasError || !report ? DASH : formatBytes(report.size)],
            [
            ...
            [
              "Size increase",
              hasError || !report ? DASH : `+${report.growthPercent.toFixed(1)}%`,
            ],
          ].map(([label, value]) => (
      ...
      {dataUrl && !hasError && (
        <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
          ...
          <h3 className="mt-5 text-sm font-semibold">Playback check</h3>
          <video
            className="mt-2 w-full rounded-md border border-[var(--border)]"
            src={dataUrl}
            controls
            preload="metadata"
          >
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Base64 stores three bytes as four characters, so a data URL is always about 33% larger than
        the file it carries. Inlining video this way suits tiny clips and test fixtures; for real
        playback, serve the file over HTTP so the browser can stream and cache it.
      </p>
    </main>
  );
}
