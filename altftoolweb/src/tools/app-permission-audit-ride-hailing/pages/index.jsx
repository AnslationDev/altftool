...
export default function ToolHome() {
  const [appName, setAppName] = useState("My cab app");
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Ride Hailing App Permission Audit</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Ride hailing is the hardest permission call on your phone: the same grants that power SOS, trip sharing and audio recording also produce the most detailed movement record any app holds. This audit separates the safety features you opted into from the collection you did not.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="ride-app-name">
        ...
        {mode === "paste" ? (
          <div className="mt-4">
            <label className={LABEL_CLASS} htmlFor="ride-paste">
              ...
              <ul className="mt-3 space-y-2">
                {PERMISSIONS.map((permission) => (
                  <li key={permission.id}>
                    <label
                      htmlFor={`ride-perm-${permission.id}`}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational only. Safety features are worth having — the point is to switch them on deliberately rather than leave a standing grant. Trip history stays in your account after you revoke a permission, so use the app's data-deletion request if you want it gone.
      </p>
    </main>
  );
}
