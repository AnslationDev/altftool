...
const STORAGE_KEY = "altft-kannada-wotd-studied";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (error) return "";
    return [
      `Kannada Word of the Day — ${prettyDate(card.date)}`,
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Kannada Word of the Day</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          One Kannada word per calendar day, in ಕನ್ನಡ script with transliteration, meaning and a
          full example sentence. The same date always shows the same word, so a class or a family
          can revise together.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="kn-date">
              Date
            </label>
            <input
              id="kn-date"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="kn-category">
              Browse by topic
            </label>
            <select
              id="kn-category"
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy today's Kannada word card"
              className={GHOST_BTN}
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Example (Kannada)", error ? DASH : card.exampleNative],
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <dl className="mt-4 grid gap-3 sm:grid-cols-3">
          {[
            ["Current streak", streak.error ? DASH : `${streak.current} day${streak.current === 1 ? "" : "s"}`],
            ["Longest streak", streak.error ? DASH : `${streak.longest} day${streak.longest === 1 ? "" : "s"}`],
        ...
        <p className="mt-3 text-xs text-[var(--muted-foreground)]">
          Marks are stored only in this browser. A streak stays alive until you miss a full day.
        </p>
      </section>
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <label className={`mt-3 ${LABEL_CLASS}`} htmlFor="kn-search">
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Transliterations follow the ISO 15919 style used in Kannada textbooks. Spoken Kannada varies
        by region, so a local speaker may shorten or soften some of these forms.
      </p>
    </main>
  );
}
