...
import { Check, Copy, Joystick, RotateCcw } from "lucide-react";

import { MODELS, ONLINE_MODES, buildPlan } from "../lib";

const DEFAULTS = { childAge: "9", model: "switch", onlineMode: "friends", dailyMinutes: "60" };
...
export default function ToolHome() {
  ...
  const plan = useMemo(
    () => buildPlan({ childAge, model, onlineMode, dailyMinutes, completed }),
    [childAge, model, onlineMode, dailyMinutes, completed],
  );
  ...
  const summary = useMemo(() => {
    ...
    const lines = [
      "Nintendo Switch parental controls plan",
      `Child age ${childAge} · ${modelLabel} · ${modeLabel}`,
      `Coverage: ${plan.score}% (${plan.band}) — ${plan.doneSteps} of ${plan.totalSteps} steps done`,
      `Suggested preset: ${plan.preset.label}`,
      plan.playLimit.limitSet
        ? `Play-time limit to enter: ${plan.playLimit.appliedMinutes} minutes a day (${plan.playLimit.weeklyHours} hours a week)`
        : "Play-time limit: none set",
      "",
      "Still to do:",
    ];
    ...
    return lines.join("\n");
  }, [plan, childAge, model, onlineMode]);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          <Joystick className="h-4 w-4" aria-hidden="true" />
          Kids device safety
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Nintendo Switch Parental Setup
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Get the restriction preset, the exact play-time figure the app will accept, and the eShop,
          chat and posting settings that match how your child plays.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="ns-age">
              Child&apos;s age (years)
            </label>
            <input
              id="ns-age"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ns-model">
              Console
            </label>
            <select
              id="ns-model"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ns-minutes">
              Daily play allowance you want (minutes)
            </label>
            <input
              id="ns-minutes"
              className={`mt-2 ${INPUT_CLASS}`}
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the Nintendo Switch parental controls plan"
              className={GHOST_BTN}
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Restriction preset for this age", plan.error ? DASH : plan.preset.label],
            [
              "Play-time limit to enter",
              plan.error
                ? DASH
                : plan.playLimit.limitSet
                  ? `${plan.playLimit.appliedMinutes} minutes a day`
                  : "No daily limit",
            ],
            [
              "That works out at",
              plan.error ? DASH : `${plan.playLimit.weeklyHours} hours a week`,
            ],
            ["VR mode", plan.error ? DASH : plan.vrAllowed ? "Old enough (7+)" : "Keep restricted (under 7)"],
          ].map(([label, value]) => (
        ...
        {!plan.error && (
          <p className="mt-4 rounded-md bg-[var(--muted)] px-3 py-2 text-xs leading-5 text-[var(--muted-foreground)]">
            {plan.playLimit.note} {plan.preset.blurb}
          </p>
        )}
      </section>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational guidance only. Preset names and the exact rating each one allows vary by region,
        and menu names change between system updates.
      </p>
    </main>
  );
}
