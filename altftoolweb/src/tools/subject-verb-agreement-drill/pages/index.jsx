...
import { Check, CheckCheck, Copy, RefreshCw, RotateCcw } from "lucide-react";
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl leading-tight font-semibold sm:text-4xl">Subject Verb Agreement Drill</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          The easy cases never trip anyone up. This drill targets the hard ones — words wedged
          between subject and verb, subjects joined by or and nor, each and none, fractions, and
          nouns like news and scissors that lie about their number.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="sva-count">
            ...
            return (
              ...
              >
                ...
                {answered ? (
                  ...
                  >
                    <p>{right ? "Correct." : `Not quite — the answer is “${question.answer}”.`}</p>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <h2 className="text-base font-semibold">The rules behind these questions</h2>
        <dl className="mt-3 divide-y divide-[var(--border)] text-sm">
          {CORE_RULES.map((rule) => (
            <div key={rule.id} className="py-3">
              <dt className="font-semibold text-[var(--primary)]">{rule.rule}</dt>
              <dd className="mt-1 text-[var(--muted-foreground)]">{rule.detail}</dd>
            </div>
          ))}
        </dl>
      </section>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Collective nouns are the one area where the two main varieties genuinely differ: British
        English allows &ldquo;the committee have decided&rdquo; when the members are acting
        individually, while American English keeps the singular throughout.
      </p>
    </main>
  );
}
