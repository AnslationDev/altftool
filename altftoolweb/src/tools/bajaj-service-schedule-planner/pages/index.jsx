...
export default function ToolHome() {
  ...
  const [coolingType, setCoolingType] = useState(DEFAULTS.coolingType);
  ...
  const [prepaid, setPrepaid] = useState(DEFAULTS.prepaid);
  ...
  const result = useMemo(
    () =>
      planServiceSchedule({
        ...
        prepaidPackagePrice: toNumber(prepaid),
      }),
    [
      purchaseDate,
      odometer,
  ...
  const summary = useMemo(() => {
    if (hasError) return "";
    return [
      "Bajaj Service Schedule",
      `Model type: ${result.coolingType === "liquid" ? "Liquid cooled" : "Air / oil cooled"}`,
      next
      ...
      result.prepaidPackagePrice > 0
        ? `Prepaid package ${money(result.prepaidPackagePrice)} — ${result.packageWorthIt ? `saves ${money(result.packageSaving)}` : `costs ${money(-result.packageSaving)} more`}`
        : "",
    ]
      .filter(Boolean)
      .join("\n");
  ...
  const reset = () => {
    ...
    setCoolingType(DEFAULTS.coolingType);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Lay out every free and paid service on a Bajaj motorcycle by kilometres and months, price
          the consumables that fall due at each visit, and check whether a prepaid service package
          actually beats paying as you go.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-cooling">
              Engine cooling
            </label>
            <select
              id="bajaj-cooling"
              ...
              value={coolingType}
              onChange={(event) => setCoolingType(event.target.value)}
            >
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-date">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-odo">
              Current odometer (km)
            </label>
            <input
              id="bajaj-odo"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-monthly">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-count">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-free">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-first">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-second">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-interval">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-labour">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-index">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bajaj-prepaid">
              Prepaid service package price (INR, 0 to skip)
            </label>
            <input
              id="bajaj-prepaid"
              className={`mt-2 ${INPUT_CLASS}`}
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy Bajaj service schedule"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Total pay-as-you-go cost", hasError ? DASH : money(result.totalCost)],
          ].map(([label, value]) => (
        ...
        {!hasError && result.prepaidPackagePrice > 0 ? (
          <p
            className={
              result.packageWorthIt
                ? "mt-4 rounded-md border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm font-medium text-[var(--success)]"
                : "mt-4 rounded-md border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm font-medium text-[var(--muted-foreground)]"
            }
          >
            {result.packageWorthIt
              ? `The prepaid package at ${money(result.prepaidPackagePrice)} saves ${money(result.packageSaving)} against paying per visit.`
              : `The prepaid package at ${money(result.prepaidPackagePrice)} costs ${money(-result.packageSaving)} more than paying per visit over this plan.`}
          </p>
        ) : null}
      </section>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Intervals and part prices are typical published values and stay editable, because Bajaj
        varies the maintenance chart by model and package. Confirm against your own owner&apos;s
        manual, and note that twin-spark models need two plugs each time the plug interval comes up.
      </p>
    </main>
  );
}
