...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, Twitter } from "lucide-react";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (!hasScore) return "";
    const lines = [
      "X (Twitter) Privacy Settings Checklist",
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          ...
          X and Twitter privacy audit
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">X (Twitter) Privacy Settings Checklist</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          X leaks in two directions: your phone number and email make you findable, and your posts, likes and location tags make you readable. These 30 controls close both, weighted for why you are here.
        </p>
      </header>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Keeping a phone number on the account for two-factor while leaving phone discoverability on &mdash; that pairing is what unmasks most pseudonymous accounts.</li>
          <li>Turning off contact syncing without using the separate remove-all-contacts action.</li>
          <li>Turning off new location tagging and never running the bulk removal for posts already published.</li>
          <li>Setting reply controls after a post has taken off, when the replies causing the problem are already there.</li>
          <li>Protecting your posts and assuming old public ones vanished &mdash; screenshots, archives and quote posts survive it.</li>
        </ul>
      </section>
