...
import { Check, Clock, Copy, MailWarning, RotateCcw, ShieldCheck, TriangleAlert } from "lucide-react";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (!hasScore) return "";
    const lines = [
      "Email Data Breach Response Checklist",
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          A leaked email address is a username, and often a recovery contact, for every account tied
          to it. Tick each step as you finish it &mdash; the score weights the ones that actually close
          off account access over the ones that only reduce nuisance.
        </p>
      </header>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the email breach response plan"
              className={GHOST_BTN}
            >
              {copied ? (
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>
            Resetting other accounts before the email account itself, leaving the one account that
            can approve every other password reset still on its old password.
          </li>
          <li>
            Clicking the reset link inside an email that mentions the breach by name instead of
            typing the site&rsquo;s address in directly.
          </li>
          <li>
            Treating a caller who cites real breach details as legitimate. That information is now
            public, not proof of who they are.
          </li>
          <li>
            Checking the breach-lookup service once and never again, when new breaches involving the
            same address keep surfacing for years.
          </li>
        </ul>
      </section>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        page never asks for your email address or any password. If an account shows signs of active
        takeover, contact that provider&rsquo;s support directly rather than handling it alone.
      </p>
    </main>
  );
}
