...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, Landmark } from "lucide-react";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    ...
    const lines = [
      "Net Banking 2FA Hardening Guide",
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Net Banking 2FA Hardening Guide</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Bank-neutral checklist: every item exists at practically every Indian bank, though the menu names differ. Tick each one as you set it; the score weights them by how much money each control keeps out of reach.
        </p>
      </header>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Asking the branch to waive the new-payee cooling period for convenience, which removes your only chance to spot an added payee.</li>
          <li>Setting the SMS alert threshold high, so the small test debit that precedes a large fraud never reaches you.</li>
          <li>Reading an OTP to a caller who already knew your name, card type and last transaction &mdash; that information is bought, not proof.</li>
          <li>Waiting to see if the money comes back on its own, and missing the three-working-day reporting window.</li>
        </ul>
      </section>
