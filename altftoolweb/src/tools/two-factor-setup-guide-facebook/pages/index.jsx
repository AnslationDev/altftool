...
export default function ToolHome() {
  ...
  const [completed, setCompleted] = useState(() => ["open-accounts-center", "open-2fa"]);
  const [copied, setCopied] = useState(false);

  const steps = useMemo(() => buildChecklist(methodId), [methodId]);
  const result = useMemo(
    () => assessSetup({ methodId, completedIds: completed }),
    [methodId, completed]
  );
  ...
  const reset = () => {
    ...
    setCompleted(["open-accounts-center", "open-2fa"]);
    setCopied(false);
  };
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl leading-tight font-semibold sm:text-4xl">
          Facebook 2FA Setup Guide
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Walk through Meta&rsquo;s Accounts Center flow, tick off each step and see how much of a
          real lockout or takeover risk is left. Nothing you enter leaves this page.
        </p>
      </header>

      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <fieldset>
          <legend className={LABEL_CLASS}>Which second factor are you setting up?</legend>
          <div className="mt-3 grid gap-3">
            {TWO_FACTOR_METHODS.map((method) => {
              ...
              return (
                <label
                  key={method.id}
                  htmlFor={`fb2fa-method-${method.id}`}
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the Facebook 2FA setup summary"
              className={GHOST_BTN}
            >
              {copied ? (
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ["Second factor chosen", hasError ? dash : result.method.label],
            ...
            [
              "Phishing-resistant",
              hasError ? dash : result.method.phishingResistant ? "Yes" : "No",
            ],
          ].map(([label, value]) => (
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ol className="mt-3 grid gap-2">
          {steps.map((step, index) => {
            ...
            const inputId = `fb2fa-step-${step.id}`;
            return (
              <li key={step.id}>
                <label
                  htmlFor={inputId}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational guide only. Meta changes the Accounts Center layout from time to time, so a
        menu name may differ from what you see; check Facebook&rsquo;s own Help Centre if a screen
        does not match, and never share a 2FA code or recovery code with anyone who contacts you.
      </p>
    </main>
  );
}
