...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (!ok) return "";
    const lines = [
      "Income tax refund phishing check",
    ...
    lines.push("", "Check the real refund status by typing incometax.gov.in yourself; report fraud on 1930 and cybercrime.gov.in.");
    return lines.join("\n");
  }, [ok, result]);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Income Tax Refund Phishing Anatomy
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          The &ldquo;your refund is approved, update your bank account&rdquo; notice taken apart
          line by line, including how these messages imitate official domains and the one check
          &mdash; the DIN &mdash; that settles it. The scanner weighs the same tells against
          anything you paste, entirely in this tab.
        </p>
      </header>
      ...
      <section className={CARD} aria-labelledby="pa-scan">
        ...
        <div className="mt-3">
          ...
          <p className="mt-2 text-xs leading-5 text-[var(--muted-foreground)]">
            ...
            Never paste your own PAN, banking login or OTP anywhere, including here.
          </p>
        </div>
      ...
      <section className={`mt-6 ${CARD}`}>
        ...
        <p className="mt-3 text-xs leading-5 text-[var(--muted-foreground)]">
          Addresses above are written with [.] so nothing here is clickable, and the amounts are
          illustrative. This is a composite of reported messages, not a copy of one real notice.
        </p>
      </section>

      <section className={`mt-6 ${CARD}`}>
        <h2 className="text-base font-semibold">What the department actually does</h2>
      ...
      <section className={`mt-6 ${CARD}`}>
        <h2 className="flex items-center gap-2 text-base font-semibold">
          ...
          If you already logged in or replied
        </h2>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        The scanner looks for documented markers of the refund-notice family and weighs them. A
        high score means the message is built the way these scams are built; a zero is not a
        certificate that a message is genuine, because a careful forgery can avoid every marker.
        Verify any notice through the DIN authentication on the e-filing portal you typed yourself.
        This page is general security education, not tax or legal advice &mdash; speak to a
        chartered accountant or the department for anything about your actual assessment.
      </p>
    </main>
  );
}
