...
import {
  ...
  PROHIBITED_ITEMS,
  TYPICAL_REPORTING_LEAD_MINUTES,
  buildTimeline,
  checklistReadiness,
  itemKey,
  shiftById,
...
export default function ToolHome() {
  const [shiftId, setShiftId] = useState(EXAM_SHIFTS[0].id);
  const [examStart, setExamStart] = useState(EXAM_SHIFTS[0].start);
  ...
  const pickShift = (id) => {
    ...
    if (shift) setExamStart(shift.start);
  };
  ...
  const summary = useMemo(() => {
    const lines = ["SSC exam-day checklist"];
    const shift = shiftById(shiftId);
    if (shift) lines.push(shift.label);
    if (hasTimeline) {
      lines.push(`Test starts: ${timeline.examStart.label}`);
    ...
    return lines.join("\n");
  }, [shiftId, timeline, hasTimeline, readiness]);
  ...
  const reset = () => {
    setShiftId(EXAM_SHIFTS[0].id);
    setExamStart(EXAM_SHIFTS[0].start);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold tracking-wide uppercase text-[var(--primary)]">
          ...
          SSC computer-based test
        </div>
        <h1 className="text-3xl leading-tight font-semibold sm:text-4xl">SSC Exam Day Checklist</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          The documents, biometric preparation and reporting plan for an SSC computer-based test —
          built around the {GATE_CLOSE_LEAD_MINUTES}-minute gate closing rule and the thumb-impression
          capture that catches candidates out at the door.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="ssc-shift">
              Shift
            </label>
            <select
              id="ssc-shift"
            ...
            <p className="mt-2 text-xs leading-5 text-[var(--muted-foreground)]">
              Shift start times differ from exam to exam. Overwrite the time below with the one
              printed on your own admit card — the reporting time is usually about{" "}
              {TYPICAL_REPORTING_LEAD_MINUTES} minutes before the test.
            </p>
          </div>

          <div>
            <label className={LABEL_CLASS} htmlFor="ssc-start">
              Test start time (24-hour)
            </label>
            <input
              id="ssc-start"
              className={`mt-2 ${INPUT_CLASS}`}
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ssc-travel">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ssc-buffer">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ssc-ready">
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              {hasTimeline
                ? `At the centre ${NUM.format(timeline.minutesBeforeStartAtCentre)} minutes before the test begins`
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the SSC exam-day plan"
              className={GHOST_BTN}
            >
              {copied ? (
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Test starts", hasTimeline ? timeline.examStart.label : DASH],
          ].map(([label, value]) => (
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <h2 className="text-base font-semibold">Photo IDs accepted in original</h2>
        <ul className="mt-3 grid gap-2 text-sm text-[var(--muted-foreground)] sm:grid-cols-2">
          {ACCEPTED_IDS.map((id) => (
            <li key={id} className="rounded-md border border-[var(--border)] px-3 py-2">
              {id}
            </li>
          ))}
        </ul>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational only. The instructions on your own admit card and the Commission&apos;s latest
        notice for that examination always take precedence over this list.
      </p>
    </main>
  );
}
