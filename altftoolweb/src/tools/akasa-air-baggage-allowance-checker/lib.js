/**
 * Akasa Air (QP) baggage allowance checker.
 *
 * Akasa Air applies a *weight* concept on the sectors it operates: the free
 * check-in allowance is a single figure in kilograms that the sum of every
 * checked piece must stay under, not a number of free bags. Two 9 kg bags
 * therefore already breach a 15 kg domestic allowance even though each one is
 * light. On top of that comes one cabin bag and one small personal article,
 * each with its own weight and size ceiling.
 *
 * The constants below are Akasa's published standard allowances. Fare bundles,
 * codeshare sectors, infants and defence/medical concessions can change them, so
 * treat the verdict as a pre-flight check and confirm the allowance printed on
 * your own booking.
 *
 * Nothing here calls the airline. Every figure comes from the constants in this
 * file and the numbers the traveller types in.
 */

/** Airline identity, so the UI never hard-codes a name. */
export const AIRLINE = { name: "Akasa Air", code: "QP", country: "India" };

/** Cabin bag: one piece, 7 kg, 55 × 35 × 25 cm. */
export const CABIN_BAG_KG = 7;
export const CABIN_DIM_RULE = { type: "each", maxEachCm: [55, 35, 25] };

/** Personal article carried in addition to the cabin bag: 3 kg, 40 × 30 × 15 cm. */
export const PERSONAL_BAG_KG = 3;
export const PERSONAL_DIM_RULE = { type: "each", maxEachCm: [40, 30, 15] };

/**
 * Checked bag size is policed as a linear dimension — length + width + height —
 * with 158 cm the standard ceiling for a single piece.
 */
export const CHECKED_DIM_RULE = { type: "linear", maxSumCm: 158 };

/** Akasa publishes no combined-size cap across all checked pieces. */
export const COMBINED_CHECKED_DIM_LIMIT_CM = null;

/**
 * Manual-handling ceiling for one piece. A bag heavier than this is refused at
 * the counter whatever excess you have paid — it has to be split in two.
 */
export const MAX_SINGLE_PIECE_KG = 32;

/** Reference domestic airport excess rate, in rupees per extra kilogram. Editable. */
export const DEFAULT_EXCESS_RATE_PER_KG = 550;
...
export const CURRENCIES = [
  ...
  { code: "SAR", locale: "en-SA", label: "SAR ﷼" },
  { code: "QAR", locale: "en-QA", label: "QAR ﷼" },
...
/**
 * Fare and route options.
 *   cabin.kg / cabin.dimRule — the main cabin piece
 *   personal                 — the extra under-seat article, null when a fare has none
 *   checked.type             — "weight" (one total for all pieces) or "piece" (N bags of X kg)
 */
export const FARE_OPTIONS = [
  {
    key: "dom-15",
    label: "Domestic — standard fare (15 kg check-in)",
    cabin: { pieces: 1, kg: CABIN_BAG_KG, combinedKg: null, dimRule: CABIN_DIM_RULE },
    personal: { kg: PERSONAL_BAG_KG, dimRule: PERSONAL_DIM_RULE },
    checked: { type: "weight", allowanceKg: 15 },
  },
  {
    key: "dom-cabin-only",
    label: "Domestic — cabin baggage only fare (no free check-in)",
    cabin: { pieces: 1, kg: CABIN_BAG_KG, combinedKg: null, dimRule: CABIN_DIM_RULE },
    personal: { kg: PERSONAL_BAG_KG, dimRule: PERSONAL_DIM_RULE },
  ...
  {
    key: "infant",
    label: "Infant without a seat (no free check-in)",
    cabin: { pieces: 1, kg: CABIN_BAG_KG, combinedKg: null, dimRule: CABIN_DIM_RULE },
    personal: { kg: PERSONAL_BAG_KG, dimRule: PERSONAL_DIM_RULE },
    checked: { type: "weight", allowanceKg: 0 },
  },
...
export const DEFAULTS = {
  ...
  bags: [{ weightKg: "15", dims: ["70", "45", "25"] }],
