 ...
 *                      an eighth, a fortieth).
 *   3. Fraction rule — the closest simple fraction, reduced, with powers of ten pulled back out.
 *
 * The second half is Singapore-specific, and it is the reason a menu price and a bill total differ
 * by a fifth: restaurants and hotels quote "plus plus", meaning before the service charge and GST,
 * and the two compound.
 */

/**
 * The Singapore dollar. The Monetary Authority of Singapore manages the dollar against an
 * undisclosed basket rather than pegging it, so the rate floats and `defaultInrPerUnit` is only a
 * starting point in the band the pair has traded in recently. Portrait series notes circulate as
 * S$2, S$5, S$10, S$50, S$100 and S$1,000, and the S$1,000 note has been withdrawn from issue.
 */
export const CURRENCY = {
  ...
  symbol: "S$",
  country: "Singapore",
  defaultInrPerUnit: 66,
  notes: [100, 50, 10, 5, 2],
  pricePoints: [
    { amount: 2, note: "Kopi at a hawker centre" },
    { amount: 6, note: "Hawker centre meal" },
    { amount: 12, note: "Food court lunch in the CBD" },
    { amount: 25, note: "Cocktail" },
    { amount: 50, note: "Restaurant dinner for one, before ++" },
    { amount: 100, note: "A day of attractions and transport" },
    { amount: 250, note: "Mid-range hotel night" },
    { amount: 600, note: "A weekend of spending" },
    { amount: 1500, note: "Phone" },
  ],
};
...
/**
 * Singapore till rules. GST rose to 9% on 1 January 2024. Retail prices must be displayed
 * GST-inclusive, so a shop tag is the price; restaurants and hotels are permitted to advertise
 * before charges and mark prices "++", meaning a 10% service charge is added first and GST is then
 * charged on the total including it. Departing visitors can claim GST back on eligible goods
 * through the electronic tourist refund scheme, subject to a minimum purchase.
 */
export const SINGAPORE_GST_PERCENT = 9;
export const TYPICAL_SERVICE_CHARGE_PERCENT = 10;
export const GST_REFUND_MIN_PURCHASE_SGD = 100;
...
/**
 * Menu price to bill total in Singapore. The service charge goes on first and GST is charged on
 * the total including it, so the two compound:
 *
 *     factor = (1 + service) x (1 + GST)
 *
 * At 10% and 9% that is 1.10 x 1.09 = 1.199 — a fifth on top of the menu price, not 19%.
 *
 * @returns {{factor:number, serviceAmount:number, gstAmount:number, total:number, steps:string[]}}
 */
export function tillPrice({ sticker, servicePercent, gstPercent }) {
  const base = Number.isFinite(sticker) && sticker > 0 ? sticker : 0;
  const s = Number.isFinite(servicePercent) ? servicePercent : 0;
  const g = Number.isFinite(gstPercent) ? gstPercent : 0;

  const afterService = base * (1 + s / 100);
  const total = afterService * (1 + g / 100);

  const steps = [];
  if (s > 0) steps.push(`Service charge ${num(s)}% on the menu price`);
  if (g > 0) steps.push(`GST ${num(g)}% on the total including service`);

  return {
    factor: base > 0 ? total / base : 1,
    serviceAmount: round(afterService - base, 2),
    gstAmount: round(total - afterService, 2),
    total: round(total, 2),
    steps,
  };
}
 ...
 *
 * @param {object} input
 * @param {number|string} input.inrPerUnit rupees per 1 Singapore dollar
 * @param {number|string} [input.amount] a price to work through as an example
 * @param {number|string} [input.servicePercent]
 * @param {number|string} [input.gstPercent]
...
export function buildCheatSheet({
  ...
  gstPercent = SINGAPORE_GST_PERCENT,
} = {}) {
  ...
  const g = toNumber(gstPercent);
  if (![s, g].every((value) => Number.isFinite(value) && value >= 0 && value <= 50)) {
    return { error: "Service charge and GST must each be between 0% and 50%." };
  }
  ...
  if (Number.isFinite(parsedAmount) && parsedAmount >= 0) {
    const till = tillPrice({ sticker: parsedAmount, servicePercent: s, gstPercent: g });
  ...
  return {
    ...
    servicePercent: s,
    gstPercent: g,
    grossUpPercent: round(
      (tillPrice({ sticker: 100, servicePercent: s, gstPercent: g }).factor - 1) * 100,
      3,
    ),
    worked,
  };
