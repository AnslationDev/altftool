...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    ...
    lines.push("", "If money has already moved, call 1930 and file at cybercrime.gov.in.");
    return lines.join("\n");
  }, [ok, result]);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Bank OTP Phishing Anatomy Explainer
        </h1>
      ...
      <section className={CARD} aria-labelledby="pa-scan">
        ...
        <div className="mt-3">
          ...
          <p className="mt-2 text-xs leading-5 text-[var(--muted-foreground)]">
            ...
            paste your own OTP, card number or password anywhere, including here.
          </p>
        </div>
      ...
      <section className={`mt-6 ${CARD}`}>
        ...
        <p className="mt-3 text-xs leading-5 text-[var(--muted-foreground)]">
          Addresses above are written with [.] so nothing here is clickable, and the numbers are
          redacted. This is a composite of reported messages, not a copy of one real institution.
        </p>
      </section>

      <section className={`mt-6 ${CARD}`}>
        <h2 className="text-base font-semibold">What a real bank actually does</h2>
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          {REAL_BANK_BEHAVIOUR.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
      </section>
      ...
      <section className={`mt-6 ${CARD}`}>
        <h2 className="flex items-center gap-2 text-base font-semibold">
          ...
          If you already replied or tapped
        </h2>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        The scanner looks for documented markers of this scam family and weighs them. A high score
        means the message is built the way these scams are built; a zero is not a certificate that a
        message is genuine, because a careful forgery can avoid every marker. When money is
        involved, verify through the number printed on your card and treat this page as general
        security education rather than legal or financial advice.
      </p>
    </main>
  );
}
