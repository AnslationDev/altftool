    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--muted-foreground)] sm:text-base">
          Score Linksys routers for admin password, Smart Wi-Fi account security,
          remote management, WPS, UPnP, guest access, firmware and port-forward hygiene.
        </p>
      </header>

      <section className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <div className={CARD}>
          <p className="rounded-lg bg-[var(--background)] p-4 text-sm leading-6 text-[var(--muted-foreground)] ring-1 ring-[var(--border)]">
            <strong className="text-[var(--foreground)]">Admin page:</strong>{" "}
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button type="button" className={BTN} onClick={() => { setCompleted(DEFAULT_DONE); setNotApplicable([]); }}>
                ...
                <div className="mt-3 grid gap-3">
                  {HARDENING_STEPS.filter((step) => step.group === group).map((step) => (
                    <article key={step.id} className="rounded-lg border border-[var(--border)] bg-[var(--background)] p-3">
                      ...
                      <p className="mt-1 text-xs leading-5 text-[var(--muted-foreground)]">{step.how}</p>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <label className="inline-flex min-h-10 items-center gap-2 rounded-md border border-[var(--border)] px-3 text-xs font-semibold">
                          <input type="checkbox" className="h-4 w-4 accent-[var(--primary)]" checked={completed.includes(step.id)} onChange={() => toggleDone(step.id)} />
                          Done
                        </label>
                        <label className="inline-flex min-h-10 items-center gap-2 rounded-md border border-[var(--border)] px-3 text-xs font-semibold">
                          <input type="checkbox" className="h-4 w-4 accent-[var(--primary)]" checked={notApplicable.includes(step.id)} onChange={() => toggleSkip(step.id)} />
                          Not applicable
                        </label>
                      </div>
                    </article>
            ...
            <>
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-2 text-sm font-semibold text-[var(--foreground)]">{result.bandLabel}</p>
                </div>
              ...
              <p className="mt-5 text-sm leading-6 text-[var(--muted-foreground)]">
                {result.doneCount} done · {result.openCount} open · {result.openCritical} critical open ·{" "}
                {result.openMinutes} minutes estimated.
              </p>
              <div className="mt-4 grid gap-3">
                {result.openSteps.slice(0, 4).map((step) => (
                  <div key={step.id} className="rounded-lg bg-[var(--background)] p-3 text-sm text-[var(--muted-foreground)] ring-1 ring-[var(--border)]">
