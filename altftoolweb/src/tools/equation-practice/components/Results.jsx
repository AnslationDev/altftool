...
function MetricCard({ icon: Icon, label, value, detail, tone = "default" }) {
  const toneClass =
    ...
    : tone === "warn" ? "bg-rose-500/10 text-rose-600"
    : tone === "watch" ? "bg-amber-500/10 text-amber-600"
...
export default function Results({ answers, onRestart, onReset }) {
  ...
  const handleExportCSV = () => {
    ...
    const rows = answers
      .map(
        (a) =>
          `"${a.question}","${a.userAnswer}","${a.type === "quadratic_two" ? a.exact : a.answer}",${a.correct ? "Yes" : "No"},${(a.timeTaken || 0).toFixed(1)},"${a.type}"`
      )
      .join("\n");
    ...
    el.href = url;
    el.download = `equation-practice-report-${Date.now()}.csv`;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <MetricCard icon={Trophy} label="Accuracy" value={`${accuracy}%`} detail={`${correct} of ${total} correct`} tone={accuracy >= 70 ? "good" : accuracy >= 50 ? "watch" : "warn"} />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
          <div className="flex items-center gap-2 border-b border-[var(--border)] px-6 py-4">
            ...
            <h3 className="text-base font-bold text-[var(--foreground)]">Accuracy</h3>
          </div>
          <div className="flex items-center gap-6 p-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2"><span className="h-3 w-3 rounded-full bg-emerald-500" /><span className="text-sm text-[var(--foreground)]">Correct: {correct}</span></div>
              <div className="flex items-center gap-2"><span className="h-3 w-3 rounded-full bg-rose-500" /><span className="text-sm text-[var(--foreground)]">Incorrect: {total - correct}</span></div>
            </div>
          </div>
        </div>
        ...
        {topicData.length > 1 && (
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
            <div className="flex items-center gap-2 border-b border-[var(--border)] px-6 py-4">
              ...
              <h3 className="text-base font-bold text-[var(--foreground)]">By Topic</h3>
            </div>
            <div className="h-64 p-6">
              <SafeResponsiveContainer width="100%" height="100%">
                <BarChart data={topicData} margin={{ left: 0, right: 0, top: 5, bottom: 5 }}>
                  ...
                  <Tooltip contentStyle={{ borderRadius: 8, border: "1px solid var(--border)", background: "var(--card)", fontSize: 12 }} formatter={(v) => [`${v}%`, "Accuracy"]} />
                  <Bar dataKey="accuracy" radius={[4, 4, 0, 0]}>
                    {topicData.map((entry, i) => <Cell key={i} fill={entry.accuracy >= 70 ? "#14B8A6" : entry.accuracy >= 50 ? "#F59E0B" : "#EF4444"} />)}
                  </Bar>
                </BarChart>
              </SafeResponsiveContainer>
            </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
          <div className="flex items-center gap-2 border-b border-[var(--border)] px-6 py-4">
            ...
            <h3 className="text-base font-bold text-[var(--foreground)]">Weak Areas</h3>
          </div>
          <div className="p-6">
            {weakTopics.length > 0 ? (
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
          <div className="flex items-center gap-2 border-b border-[var(--border)] px-6 py-4">
            ...
            <h3 className="text-base font-bold text-[var(--foreground)]">Strong Areas</h3>
          </div>
          <div className="p-6">
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
        <div className="flex items-center gap-2 border-b border-[var(--border)] px-6 py-4">
          ...
          <h3 className="text-base font-bold text-[var(--foreground)]">Answer Review</h3>
        </div>
        <div className="max-h-96 space-y-2 overflow-y-auto p-6">
          {answers.map((a, i) => (
            <div key={i} className={`flex items-center gap-3 rounded-lg border p-3 ${a.correct ? "border-emerald-500/20 bg-emerald-500/5" : "border-rose-500/20 bg-rose-500/5"}`}>
              {a.correct ? <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600" /> : <XCircle className="h-4 w-4 shrink-0 text-rose-600" />}
              <div className="min-w-0 flex-1">
                <p className="font-mono text-sm font-semibold text-[var(--foreground)]">{a.question}</p>
                <p className="text-xs text-[var(--muted-foreground)]">
                  Your answer: {a.userAnswer} | Correct: {a.type === "quadratic_two" ? a.exact : a.answer}
                </p>
              </div>
      ...
      <div className="flex flex-wrap gap-3">
        <button onClick={onRestart} className="flex items-center gap-2 rounded-xl bg-[var(--primary)] px-6 py-3 text-sm font-bold text-white shadow-md transition-all hover:shadow-lg hover:brightness-110">
          <RotateCcw className="h-4 w-4" /> Practice Again
        </button>
        <button onClick={onReset} className="flex items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--card)] px-6 py-3 text-sm font-bold text-[var(--foreground)] shadow-sm transition-all hover:bg-[var(--section-highlight)]">
          New Setup
        </button>
        <button onClick={handleExportCSV} className="flex items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--card)] px-6 py-3 text-sm font-bold text-[var(--foreground)] shadow-sm transition-all hover:bg-[var(--section-highlight)]">
