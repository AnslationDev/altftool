...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, TabletSmartphone } from "lucide-react";
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">iPhone Privacy Settings Checklist</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Twenty-seven iPhone settings, ranked by how much exposure each one actually closes. Apple's defaults are better than most, which is exactly why the leaks that remain are the ones nobody checks — pick your profile, tick what you have done, and see what is left.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="ios-profile">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ios-target">
              Target score (%)
            </label>
            <input
              id="ios-target"
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Assuming App Tracking Transparency covers everything. Apple&rsquo;s own personalised ads are a completely separate setting in the same menu.</li>
          <li>Never opening Significant Locations. It is four levels deep under Location Services and holds a dated log of everywhere you regularly go.</li>
          <li>Leaving Control Centre reachable from the lock screen, so a thief can enable Aeroplane Mode and drop the phone off Find My before you react.</li>
          <li>Turning on Advanced Data Protection without first setting a recovery contact or storing the recovery key. Lose both and the data is gone permanently &mdash; Apple cannot restore it.</li>
          <li>Forgetting a Find My location share set up for one trip years ago. It never expires and never reminds you.</li>
          <li>Using an end-to-end encrypted messenger while its full history sits in a standard iCloud backup outside that encryption.</li>
          <li>Sharing a photo straight from the library without opening the share sheet Options, so the GPS coordinates go with it.</li>
        </ul>
      </section>
