...
export const spec = {
  ...
  compute: (values) => {
      const data = String(values.data || "").split(/[\s,;]+/).filter(Boolean).map(Number).filter(Number.isFinite);
      if (!data.length) return { result: "—", caption: "Enter numeric sample values" };
      const iterationsRaw = Number(values.iterations);
      const iterations = Math.max(100, Math.min(20000, Math.round(Number.isFinite(iterationsRaw) ? iterationsRaw : 1000)));
      ...
      return { result: quantile(alpha).toFixed(6) + " to " + quantile(1 - alpha).toFixed(6), caption: values.confidence + "% seeded percentile interval for mean", rows: [["Sample size", data.length], ["Sample mean", sampleMean.toFixed(6)], ["Iterations", iterations], ["Seed", normalizedSeed], ["Bootstrap median", quantile(0.5).toFixed(6)]] };
    },
};
