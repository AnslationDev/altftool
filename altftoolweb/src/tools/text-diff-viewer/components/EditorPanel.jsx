    ...
    <div className="flex flex-col h-full">
      <div className={`flex items-center justify-between px-3 py-2 border-b border-(--border) bg-(--muted) rounded-t-xl`}>
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full bg-${color === "primary" ? "primary" : "blue-500"}`} />
      ...
      <textarea
        ...
        className="flex-1 w-full min-h-[180px] px-3 py-3 text-sm font-mono bg-(--card) text-(--foreground) resize-none focus:outline-none rounded-b-xl leading-relaxed"
        spellCheck={false}
      />
    </div>
  );
