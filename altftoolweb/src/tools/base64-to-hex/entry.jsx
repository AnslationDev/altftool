...
/* ---------------------------------------------------------------------------
   Conversion engine (pure functions)
--------------------------------------------------------------------------- */
...
const DEFAULT_OPTS = {
  ...
  uppercase: true,
  prefix0x: false,
  groupSize: 0, // hex chars per group: 0 = no grouping, 2 = pairs, 4, 8
};

const GROUP_CHOICES = [
  { value: 0, label: "No grouping" },
  { value: 2, label: "Pairs (2 chars)" },
  { value: 4, label: "Words (4 chars)" },
  { value: 8, label: "Blocks (8 chars)" },
];

function base64ToBytes(raw, opts) {
  let s = (raw || "").trim();
  ...
  if (opts.removeWhitespace) {
    ...
    return { ok: false, message: "Whitespace found in input — enable “Remove whitespace”." };
  }

  // Accept URL-safe Base64 transparently.
  s = s.replace(/-/g, "+").replace(/_/g, "/");

  if (!s) return { ok: false, message: "Input is empty after cleanup." };
  if (s.length % 4 === 1) {
    return { ok: false, message: "Incorrect length — this is not valid Base64." };
  }
  const core = s.replace(/=+$/, "");
  if (!/^[A-Za-z0-9+/]*$/.test(core)) {
    return { ok: false, message: "Invalid characters found in the Base64 string." };
  }
  ...
  try {
    ...
    return { ok: false, message: "Could not decode — the data is not valid Base64." };
  }
}
...
function bytesToHex(bytes, opts) {
  ...
  if (opts.groupSize > 0) {
    const groups = [];
    for (let i = 0; i < hex.length; i += opts.groupSize) {
      const chunk = hex.slice(i, i + opts.groupSize);
      groups.push(opts.prefix0x ? `0x${chunk}` : chunk);
    }
    return groups.join(" ");
  }
  return opts.prefix0x && hex ? `0x${hex}` : hex;
}

function convertBase64ToHex(raw, opts) {
  ...
  const decoded = base64ToBytes(raw, opts);
  if (!decoded.ok) return done(decoded);
  return done({ ok: true, hex: bytesToHex(decoded.bytes, opts), byteLength: decoded.bytes.length });
}
...
const SAMPLE_TEXT = "Hello from AltFTool! Base64 → Hex made easy. ✓";
...
export default function ToolEntry() {
  ...
  const runConvert = useCallback((raw, o) => {
    if (!raw.trim()) {
      setOutput("");
      setStatus("empty");
      setErrorMsg("");
    ...
    setProcessing(true);
    window.setTimeout(() => {
      const res = convertBase64ToHex(raw, o);
      if (res.ok) {
        setOutput(res.hex);
        setStatus("valid");
        setErrorMsg("");
  ...
  // Auto convert while typing (debounced) — options changes re-convert too.
  useEffect(() => {
    if (!opts.autoConvert) return undefined;
    const t = window.setTimeout(() => runConvert(input, opts), 220);
    return () => window.clearTimeout(t);
  }, [input, opts, runConvert]);
  ...
  const setGroupSize = (value) => setOpts((prev) => ({ ...prev, groupSize: value }));
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "Hex output — AltFTool", text });
      } catch {
  ...
  const statusTile =
    status === "valid"
      ? { value: "Valid Base64", sub: "Converted successfully", tone: "text-emerald-600 dark:text-emerald-400" }
      : status === "invalid"
  ...
  const statusPill = processing
    ? { dot: "bg-blue-500 animate-pulse", label: "Converting…", tone: "text-blue-600 dark:text-blue-400" }
    : status === "invalid"
      ? { dot: "bg-red-500", label: "Invalid", tone: "text-red-500" }
      : status === "valid"
        ? { dot: "bg-emerald-500", label: "Done", tone: "text-emerald-600 dark:text-emerald-400" }
        : { dot: "bg-emerald-500", label: "Ready", tone: "text-slate-600 dark:text-slate-300" };
  ...
  const renderHowModal = () => {
    ...
    const steps = [
      ...
      ["Convert", "With Auto Convert on it converts as you type — or press the Convert button."],
      ["Shape the hex", "Use Advanced Options for uppercase, 0x prefixes and grouping, then copy, download or share."],
    ];
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" onClick={() => setHowOpen(false)}>
        ...
        >
          ...
          <p className="mt-5 rounded-xl bg-slate-50 p-3 text-xs leading-relaxed text-slate-500 dark:bg-slate-800/60 dark:text-slate-400">
            <Lock className="mr-1 inline h-3.5 w-3.5 text-violet-500" />
  ...
  const features = [
    { icon: <ShieldCheck className="h-5 w-5 text-emerald-500" />, tint: "bg-emerald-500/10", title: "Live Validation", body: "Instantly validates Base64 format as you type." },
    { icon: <Zap className="h-5 w-5 text-blue-500" />, tint: "bg-blue-500/10", title: "Lightning Fast", body: "Ultra-fast conversion right in your browser." },
    { icon: <Lock className="h-5 w-5 text-violet-500" />, tint: "bg-violet-500/10", title: "Privacy First", body: "All conversions happen locally. Your data never leaves your device." },
    { icon: <span className="text-[10px] font-black leading-none text-orange-500">UTF-8</span>, tint: "bg-orange-500/10", title: "UTF-8 Support", body: "Properly handles UTF-8 encoded Base64 strings." },
    { icon: <FileText className="h-5 w-5 text-sky-500" />, tint: "bg-sky-500/10", title: "Large File Support", body: "Convert large Base64 strings with ease." },
    { icon: <Clipboard className="h-5 w-5 text-emerald-500" />, tint: "bg-emerald-500/10", title: "Clipboard Ready", body: "Quick copy results with one click." },
  ];
      ...
      <div className="mx-auto w-full max-w-[1440px] rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6 lg:p-8 dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-700 leading-none shadow-lg shadow-blue-500/30">
              <span className="text-[11px] font-black text-white">B64</span>
              <span className="text-[9px] font-bold text-white/80">→</span>
              <span className="text-[11px] font-black text-white">HEX</span>
            </div>
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl dark:text-white">Base64 to Hex</h1>
              <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">Decode Base64 strings and convert them to hexadecimal values instantly.</p>
            </div>
          </div>
        ...
        <div className="mt-7 grid grid-cols-1 items-stretch gap-5 lg:grid-cols-[minmax(0,1fr)_180px_minmax(0,1fr)]">
          ...
          <section className="flex flex-col rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700/70 dark:bg-slate-950/40">
            <div className="flex flex-wrap items-center justify-between gap-2 px-4 pb-3 pt-4 sm:px-5">
              <h2 className="flex items-center gap-2 text-sm font-bold text-slate-900 sm:text-base dark:text-white">
                <FileInput className="h-[18px] w-[18px] text-blue-600" /> Base64 Input
              </h2>
          ...
          <div className="flex flex-row items-center justify-center gap-4 lg:flex-col">
            ...
            <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 shadow-sm dark:border-slate-700 dark:bg-slate-900">
              {processing ? (
          ...
          <section className="flex flex-col rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700/70 dark:bg-slate-950/40">
            <div className="flex flex-wrap items-center justify-between gap-2 px-4 pb-3 pt-4 sm:px-5">
              <h2 className="flex items-center gap-2 text-sm font-bold text-slate-900 sm:text-base dark:text-white">
                <span className="rounded bg-blue-100 px-1 py-0.5 text-[10px] font-black text-blue-700 dark:bg-blue-500/20 dark:text-blue-300">16</span> Hex Output
              </h2>
              <div className="flex items-center gap-2">
            ...
            <div className="mx-4 flex-1 rounded-xl border border-slate-200 bg-slate-50/50 sm:mx-5 dark:border-slate-700/70 dark:bg-slate-900/60">
              <textarea
                ...
                spellCheck={false}
                suppressHydrationWarning
                className="h-64 w-full resize-none rounded-xl bg-transparent p-4 font-mono text-sm leading-relaxed text-slate-800 outline-none placeholder:text-slate-400 dark:text-slate-200 dark:placeholder:text-slate-500"
              />
            </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
          <GhostButton icon={ClipboardPaste} label="Paste" onClick={handlePaste} className="!py-2.5" />
          <GhostButton icon={FileText} label="Sample Data" onClick={handleSample} className="!py-2.5" />
          <button
            type="button"
            onClick={() => toggleOpt("autoConvert")}
            suppressHydrationWarning
          ...
          >
            ...
            <span className="whitespace-nowrap">Auto Convert</span>
            <span
              className={`rounded-md px-1.5 py-0.5 text-[10px] font-bold tracking-wide text-white ${opts.autoConvert ? "bg-emerald-500" : "bg-slate-400 dark:bg-slate-600"}`}
            >
              {opts.autoConvert ? "ON" : "OFF"}
            </span>
          </button>
          <GhostButton icon={Copy} label="Copy Result" onClick={handleCopy} disabled={!output} className="!py-2.5" />
          <GhostButton icon={Download} label="Download" onClick={handleDownload} disabled={!output} className="!py-2.5" />
          <GhostButton icon={RotateCcw} label="Reset" onClick={handleReset} className="!py-2.5" />
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700/70 dark:bg-slate-950/40">
          ...
          {advOpen ? (
            <div className="flex flex-col gap-x-8 gap-y-3.5 border-t border-slate-100 px-5 pb-5 pt-4 lg:flex-row lg:flex-wrap lg:items-center dark:border-slate-800">
              <OptionCheck checked={opts.autoConvert} onToggle={() => toggleOpt("autoConvert")} label="Auto convert while typing" />
              <OptionCheck checked={opts.removeWhitespace} onToggle={() => toggleOpt("removeWhitespace")} label="Remove whitespace" />
              <OptionCheck checked={opts.uppercase} onToggle={() => toggleOpt("uppercase")} label="Uppercase Hex (A-F)" />
              <OptionCheck checked={opts.prefix0x} onToggle={() => toggleOpt("prefix0x")} label="Add 0x prefix" />
              <OptionCheck
                checked={opts.groupSize === 2}
                onToggle={() => setGroupSize(opts.groupSize === 2 ? 0 : 2)}
                label="Group hex pairs"
              />
              <div className="relative lg:ml-auto">
                <select
                  ...
                  suppressHydrationWarning
                  className="w-full appearance-none rounded-xl border border-slate-200 bg-white py-2.5 pl-4 pr-10 text-sm font-medium text-slate-700 shadow-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 lg:w-48 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                >
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
          {features.map((f) => (
            <div key={f.title} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-700/70 dark:bg-slate-950/40">
              <div className={`flex h-10 w-10 items-center justify-center rounded-full ${f.tint}`}>{f.icon}</div>
