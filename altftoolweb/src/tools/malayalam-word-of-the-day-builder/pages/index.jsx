...
const STORAGE_KEY = "altft-malayalam-wotd-studied";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (error) return "";
    return [
      `Malayalam Word of the Day — ${prettyDate(card.date)}`,
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          One Malayalam word per calendar day in മലയാളം script, with transliteration, meaning and a
          full example sentence. The card is derived from the date, so everyone studying on the same
          day gets the same word.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ml-category">
              Browse by topic
            </label>
            <select
              id="ml-category"
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy today's Malayalam word card"
              className={GHOST_BTN}
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Example (Malayalam)", error ? DASH : card.exampleNative],
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <p className="mt-3 text-xs text-[var(--muted-foreground)]">
          Marks stay in this browser only. A streak survives until you miss a full day.
        </p>
      </section>
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <label className={`mt-3 ${LABEL_CLASS}`} htmlFor="ml-search">
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Transliterations follow ISO 15919 conventions. Malayalam has regional and register
        differences, so a Kozhikode speaker and a Thiruvananthapuram speaker may pronounce some of
