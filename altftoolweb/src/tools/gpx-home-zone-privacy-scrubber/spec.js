...
export const spec = {
  ...
  "confirmReset": "Reset and discard the pasted GPX text? This cannot be undone."
},
  compute: (values) => {
      const text = String(values.gpx || ""), points = [...text.matchAll(/<trkpt\b[^>]*\/>|<trkpt\b[^>]*>(?:(?!<trkpt\b)[\s\S])*?<\/trkpt>/g)].map((match) => ({ raw: match[0], index: match.index }));
      ...
      let cursor = 0, output = "";
      points.forEach((point, index) => { output += text.slice(cursor, point.index); if (index >= keepStart && index < keepEnd) output += point.raw.replace(/(lat|lon)=(["'])(-?\d+(?:\.\d+)?)\2/g, (_, key, quote, number) => key + "=" + quote + Number(number).toFixed(round) + quote); cursor = point.index + point.raw.length; });
      output += text.slice(cursor);
      return { result: output || "—", caption: Math.max(0, keepEnd - keepStart) + "/" + points.length + " track points retained", rows: [["Start removed", keepStart], ["End removed", Math.max(0, points.length - keepEnd)], ["Coordinate decimals", round], ["Output size (bytes)", new TextEncoder().encode(output).length]] };
    },
};
