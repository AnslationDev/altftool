...
export const spec = {
  ...
  compute: (values) => {
      ...
      const windows = String(values.windows || "").split(/\r?\n/).map((line) => { const [range, price] = line.split("|").map((cell) => cell.trim()); const [start, end] = String(range || "").split("-"); const toHour = (value) => { const [h, m] = String(value || "").split(":").map(Number); return (h === 24 ? 24 : h || 0) + (m || 0) / 60; }; const rawSpan = toHour(end) - toHour(start); const span = rawSpan < 0 ? rawSpan + 24 : rawSpan; return { range, price: Number(price), duration: Math.max(0, span) }; }).filter((row) => Number.isFinite(row.price) && row.duration > 0).sort((a, b) => a.price - b.price);
