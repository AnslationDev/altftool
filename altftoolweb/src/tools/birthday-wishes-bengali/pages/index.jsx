    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Pick the relationship and the tone. Every wording is written out separately for তুমি and
          আপনি, so the possessive and the verb ending are right — with romanisation, an English
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="bn-name">
              Their name (optional)
            </label>
            <input
              id="bn-name"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bn-relationship">
              Who is it for?
            </label>
            <select
              id="bn-relationship"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bn-register">
              Politeness
            </label>
            <select
              id="bn-register"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bn-tone">
              Tone
            </label>
            <select
              id="bn-tone"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bn-count">
              How many messages
            </label>
            <select
              id="bn-count"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="bn-sender">
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button
            ...
            aria-label="Copy every generated Bengali birthday message"
        ...
        {!hasError && result.registerOverridden ? (
          <p className="mt-4 rounded-md bg-[var(--muted)] px-3 py-2 text-sm text-[var(--muted-foreground)]">
            For a {result.relationship.label.toLowerCase()}, Bengali speakers would usually use the{" "}
            {result.autoRegister === "respectful" ? "আপনি" : "তুমি"} form.
          </p>
        ) : null}
      </section>
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <button
                  ...
                  aria-label="Copy this Bengali birthday message"
                  className={GHOST_BTN}
                >
                  {copiedId === item.id ? (
                    <Check className="h-4 w-4" aria-hidden="true" />
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Bengali sentences close with the দাঁড়ি (।) rather than a full stop, and Bengali script is
        sent over SMS as UCS-2 — 70 characters in a single message, 67 per part after that. WhatsApp
        and other data messaging apps have no such limit.
      </p>
    </main>
  );
}
