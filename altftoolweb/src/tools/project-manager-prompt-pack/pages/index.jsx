...
const DEFAULT_VALUES = {
  ...
  goal: "move 40,000 accounts off the legacy biller before the March renewal",
  stakeholders: "Finance director, customer support lead, two vendor PMs",
  method: "Scrum",
  context:
    "Vendor sandbox slipped two weeks. UAT is unblocked. Three open defects, none severity 1.",
};
...
export default function ToolHome() {
  ...
  const [audienceId, setAudienceId] = useState("sponsor");
  const [category, setCategory] = useState("All");
  const [query, setQuery] = useState("");
  const [promptId, setPromptId] = useState(PM_PROMPTS[0].id);
  const [copied, setCopied] = useState("");

  const visible = useMemo(() => filterPrompts({ category, query }), [category, query]);
  const result = useMemo(
    () => buildPrompt({ promptId, values, lengthId, audienceId }),
    [promptId, values, lengthId, audienceId],
  );
  const pack = useMemo(
    () => buildPack({ values, category, lengthId, audienceId }),
    [values, category, lengthId, audienceId],
  );
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Twelve delivery prompts — status updates, RAID logs, escalations, scope changes and
          stakeholder emails — assembled with your project context, a word budget and a directive
          matched to whoever is reading.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {CONTEXT_FIELDS.map((field) => (
            <div key={field.key} className={field.type === "textarea" ? "sm:col-span-2" : undefined}>
              <label className={LABEL_CLASS} htmlFor={`pmp-${field.key}`}>
                {field.label}
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="pmp-audience">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="pmp-length">
              Output length
            </label>
            <select
              id="pmp-length"
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="pmp-category">
              Category
            </label>
            <select
              id="pmp-category"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="pmp-search">
              Search prompts
            </label>
            <input
              id="pmp-search"
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">{ok ? result.title : DASH}</p>
          </div>
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Written for", ok ? result.audienceLabel : DASH],
            ["Word budget given to the model", ok ? `${NUM.format(result.wordBudget)} words` : DASH],
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">
          {pack.error
            ? pack.error
            : `${NUM.format(pack.count)} prompts, ${NUM.format(pack.words)} words, filled with your project context.`}
        </p>
        <button
          type="button"
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Prompts are drafting aids. Check every date, cost and commitment against your own plan
        before an escalation or client message leaves your outbox.
      </p>
    </main>
  );
}
