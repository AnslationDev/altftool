...
export default function ToolHome() {
  ...
  const result = useMemo(() => findParsiNames(form), [form]);
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Parsi Baby Name Finder</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Names from the Amesha Spentas and yazatas, the {ZOROASTRIAN_DAYS_PER_MONTH} named days of
          the Zoroastrian month, the Shahnameh and Persian history — each with its Avestan or Persian
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="pbn-gender">
              Gender
            </label>
            <select id="pbn-gender" className={`mt-2 ${INPUT_CLASS}`} value={form.gender} onChange={setField("gender")}>
              {GENDERS.map((item) => (
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="pbn-category">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="pbn-origin">
              Language of origin
            </label>
            <select id="pbn-origin" className={`mt-2 ${INPUT_CLASS}`} value={form.origin} onChange={setField("origin")}>
              {ORIGINS.map((item) => (
                <option key={item.id} value={item.id}>
                  {item.label}
                </option>
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="pbn-syllables">
              Syllables
            </label>
            <select
              id="pbn-syllables"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="pbn-query">
            ...
            <input
              ...
              className={`mt-2 ${INPUT_CLASS}`}
              type="search"
              placeholder="fire, sun, victory, Shahnameh…"
              value={form.query}
              onChange={setField("query")}
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="pbn-limit">
              Names to show
            </label>
            <input
              id="pbn-limit"
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Parsi spellings vary widely between families — Meher and Mihir, Firoz and Feroze, Hormazd and
        Hormuzd are the same names. Check the form and the roj your family prefers with your own
        elders or mobed.
      </p>
    </main>
  );
