...
import { Sun, RotateCcw, Sliders, Info, Zap } from "lucide-react";
...
export default function LightRefractionSimulator() {
  ...
  useEffect(() => {
    ...
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.arc(midX, midY, 45, -Math.PI / 2, -Math.PI / 2 + rad1);
    ctx.stroke();
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 space-y-5">
            <div className="rounded-xl border border-border bg-card p-5 space-y-4 shadow-sm">
              ...
              {/* Medium 1 Select */}
              <div>
                <label className="text-xs font-semibold mb-1.5 block">Incident Medium 1 ($n_1$)</label>
              ...
              {/* Medium 2 Select */}
              <div>
                <label className="text-xs font-semibold mb-1.5 block">Refracting Medium 2 ($n_2$)</label>
                <div className="grid grid-cols-2 gap-2">
                  {MATERIALS.map((m) => (
                    <button
                      key={`m2-${m.name}`}
              ...
              <div>
                <div className="flex justify-between text-xs font-semibold mb-1">
                  <span>Incident Angle ($\theta_1$)</span>
            ...
            <div className="rounded-xl border border-border bg-card p-5 space-y-3 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="p-3 rounded-lg bg-surface-soft border border-border">
                  <div className="text-muted-foreground">Incident Angle ($\theta_1$)</div>
                  <div className="text-base font-bold font-mono text-foreground mt-0.5">
                    {theta1}°
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-surface-soft border border-border">
                  <div className="text-muted-foreground">Refraction Angle ($\theta_2$)</div>
                  <div className="text-base font-bold font-mono text-foreground mt-0.5">
                    {isTIR ? "TIR" : `${theta2.toFixed(2)}°`}
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-surface-soft border border-border">
                  <div className="text-muted-foreground">Critical Angle ($\theta_c$)</div>
                  <div className="text-base font-bold font-mono text-foreground mt-0.5">
                    {criticalAngleDeg ? `${criticalAngleDeg.toFixed(2)}°` : "N/A"}
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-surface-soft border border-border">
                  <div className="text-muted-foreground">Relative Ratio ($n_1/n_2$)</div>
                  <div className="text-base font-bold font-mono text-foreground mt-0.5">
                    {(n1 / n2).toFixed(3)}
                  </div>
                </div>
