...
import { BookOpen, Check, Copy, RefreshCw, RotateCcw } from "lucide-react";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    ...
    return [
      ...
      ...result.wrong.map((item) => `Missed: ${item.corrected} — ${item.rule}`),
    ].join("\n");
  }, [hasError, result]);
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl leading-tight font-semibold sm:text-4xl">English Article Usage Drill</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          a, an, the — or nothing at all. Every question offers all four choices and explains the
          rule behind the right one, including the sound rule that makes it &ldquo;an hour&rdquo; but
          &ldquo;a university&rdquo;.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="art-count">
            ...
            return (
              ...
              >
                ...
                <p className="mt-2 text-base leading-7">
                  ...
                  <span className="mx-1 inline-block min-w-14 border-b-2 border-[var(--primary)] text-center font-semibold text-[var(--primary)]">
                    {answered ? ARTICLE_LABELS[chosen] : " "}
                  </span>
                ...
                {answered ? (
                  ...
                  >
                    <p>
                      {right
                        ? "Correct."
                        : `Not quite — the answer is “${ARTICLE_LABELS[question.answer]}”.`}
                    </p>
                    <p className="mt-1 text-[var(--foreground)]">
                      {fillSentence(question.sentence, question.answer)}
                    </p>
                    <p className="mt-1 text-[var(--foreground)]">{question.rule}</p>
                  </div>
                ) : null}
              </div>
            );
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <h2 className="text-base font-semibold">The four decisions</h2>
        <div className="mt-3 overflow-x-auto">
          <table className="w-full min-w-[460px] text-left text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] text-xs tracking-wide uppercase text-[var(--muted-foreground)]">
                <th scope="col" className="py-2 pr-3 font-semibold">Article</th>
                <th scope="col" className="py-2 pr-3 font-semibold">Use it when</th>
                <th scope="col" className="py-2 font-semibold">Example</th>
              </tr>
            </thead>
            <tbody>
              {CORE_RULES.map((rule) => (
                <tr key={rule.rule} className="border-b border-[var(--border)] last:border-0">
                  <td className="py-2 pr-3 font-semibold text-[var(--primary)]">{rule.rule}</td>
                  <td className="py-2 pr-3">{rule.when}</td>
                  <td className="py-2 text-[var(--muted-foreground)]">{rule.example}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Answers follow standard British usage. American English differs on a few institution phrases
        — &ldquo;in hospital&rdquo; against &ldquo;in the hospital&rdquo; — so check which variety
        your exam expects.
      </p>
    </main>
  );
