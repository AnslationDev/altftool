...
import { checkKaalSarp } from "../utils/kaalSarpCalc";

const now = new Date();

export default function KaalSarpChecker() {
  ...
  const result = useMemo(() => {
    ...
    return checkKaalSarp({ year: data.year, month: data.month, day: data.day }, data.time);
  }, [data, calculated]);
    ...
    <div className="bg-[var(--background)] px-4 py-6 text-[var(--foreground)] transition-colors sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="space-y-6">
          ...
          {result && <DoshaResult result={result} />}
        </div>

        <p className="mt-6 text-center text-xs text-[var(--muted-foreground)]">
          All calculations run in your browser using VSOP87-based planetary positions. For informational purposes only.
        </p>
      </div>
    </div>
  );
