...
export default function Results({ answers, config, onRestart, onReset }) {
  ...
  const handleExportCSV = () => {
    const header = "Dividend,Divisor,Your Answer,Correct Answer,Correct,Time (s),Type,Hint Used\n";
    const rows = answers
      .map(
        (a) =>
          `${a.dividend},${a.divisor},"${a.userAnswer}","${a.answer}",${a.correct ? "Yes" : "No"},${(a.timeTaken || 0).toFixed(1)},"${a.type}",${a.hintUsed ? "Yes" : "No"}`
      )
      .join("\n");
    ...
    el.href = url;
    el.download = `division-practice-report-${Date.now()}.csv`;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          label="Correct"
          value={correct}
          detail={`${total - correct} incorrect`}
          tone="good"
        />
        <MetricCard
          icon={Clock}
          label="Avg Time"
          value={`${avgTime}s`}
          detail={`Total: ${(totalTime / 60).toFixed(1)} min`}
        />
        <MetricCard
          icon={Flame}
          label="Best Streak"
          value={bestStreak}
          detail={`${hintsUsed} hints used`}
        />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {/* Topic Performance */}
        {topicData.length > 0 && (
          <SectionCard title="By Division Type" icon={TrendingUp}>
            <div className="h-64">
              <SafeResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[var(--muted-foreground)]">
                  Your answer: {a.userAnswer} | Correct: {a.answer}
                </p>
              </div>
