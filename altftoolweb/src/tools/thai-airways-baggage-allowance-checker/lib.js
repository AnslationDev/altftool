/**
 * Thai Airways (TG) baggage allowance checker.
 *
 * Thai Airways uses a weight concept on almost its whole network: the free
 * checked allowance is one figure in kilograms covering every checked piece
 * together, rising with the fare family and cabin, from 20 kg on the cheapest
 * Economy fares to 50 kg in Royal First. The exception is journeys to and from
 * the Americas, where IATA piece rules apply instead and the allowance becomes a
 * counted number of bags — 2 × 23 kg in Economy, 2 × 32 kg in Royal Silk Business
 * and Royal First.
 *
 * Cabin baggage carries two size rules at once, and the second is the one that
 * usually bites: no side longer than 56 × 45 × 25 cm AND the three sides added
 * together no more than 115 cm. A case built exactly to 56 × 45 × 25 measures
 * 126 cm and therefore fails, which is why a "fits the carry-on box" suitcase can
 * still be turned away. The weight limit is 7 kg.
 *
 * These are the airline's published standard allowances. Royal Orchid Plus tiers,
 * Star Alliance status, codeshare sectors, infants and sports equipment follow
 * separate rules, so confirm what is printed on your own booking.
 *
...
export const AIRLINE = { name: "Thai Airways", code: "TG", country: "Thailand" };

/** Cabin bag: 7 kg, within 56 × 45 × 25 cm and within 115 cm of total dimensions. */
export const CABIN_BAG_KG = 7;
export const CABIN_DIM_RULE = { type: "both", maxEachCm: [56, 45, 25], maxSumCm: 115 };
...
/** Royal Silk and Royal First carry two cabin pieces of 7 kg each. */
...
/**
 * No default excess rate ships with this tool. Thai prices excess per kilogram by
 * route band and the figure differs sharply between domestic Thailand, regional
 * Asia and long-haul sectors, so enter the rate quoted for your own booking.
 */
...
export const FARE_OPTIONS = [
  {
    key: "eco-30",
    label: "Economy Standard / Flexible — 30 kg",
    cabin: ECONOMY_CABIN,
  ...
  {
    key: "eco-20",
    label: "Economy Saver / domestic Thailand — 20 kg",
    cabin: ECONOMY_CABIN,
    personal: PERSONAL_ITEM,
  ...
  {
    key: "biz-40",
    label: "Royal Silk Business — 40 kg",
    cabin: PREMIUM_CABIN,
  ...
  {
    key: "americas-biz",
    label: "Royal Silk Business to/from the Americas — 2 × 32 kg",
    cabin: PREMIUM_CABIN,
    personal: SECOND_CABIN_PIECE,
  ...
  {
    key: "americas-first",
    label: "Royal First to/from the Americas — 2 × 32 kg",
    cabin: PREMIUM_CABIN,
    personal: SECOND_CABIN_PIECE,
    checked: { type: "piece", pieces: 2, perPieceKg: 32 },
  },
