  ...
  return (
    <div className="p-4 shadow-sm rounded-lg border border-(--border) bg-(--card)">
      ...
      <p className="text-sm mt-1">
        {session.exercise} - {session.sets} sets x {session.reps}
      </p>
      ...
      <ul className="mt-3 space-y-1 text-sm hidden md:block">
        ...
        <div className="flex items-center gap-3">
          <label className="text-xs flex items-center gap-1">
            <input
              ...
              onChange={() => onToggleComplete(sessionKey)}
            />{" "}
            Completed
          </label>
          <label className="text-xs">
            ...
            <input
              ...
              max="10"
              value={log?.rpe ?? 7}
              onChange={(e) => onUpdateRpe(sessionKey, e.target.value)}
              className="ml-2 w-16 px-2 py-1 rounded border border-(--border) bg-(--card) text-(--foreground) outline-none focus:border-(--primary)"
            />
          </label>
        </div>
      </div>
      ...
      <div className="mt-3 p-3 rounded-lg border border-(--border) bg-(--background)">
        ...
        {session.weekPlan.map((w) => (
          <p key={w.week} className="text-xs text-(--muted-foreground)">
            Week {w.week}: {w.prescription}
          </p>
        ))}
      </div>
    </div>
