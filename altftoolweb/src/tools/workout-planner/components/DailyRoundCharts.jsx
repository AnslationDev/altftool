    ...
    <section className="mt-2">
      <div className="p-5 rounded-lg border border-(--border) bg-(--card)">
        ...
        <p className="text-sm text-(--muted-foreground) mb-5">
          Each circle shows warm-up, main block, and cooldown time split for the
          day.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {timeline.map((session) => {
            ...
            return (
              <div
                key={session.day}
                className="p-4 text-center rounded-lg border border-(--border) bg-(--background)"
              >
                <p className="text-sm font-semibold mb-3">{session.day}</p>
                <div
                  className="mx-auto h-28 w-28 rounded-full relative"
                  style={{ background: bg }}
                >
