...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (!ok) return "";
    const lines = [
      "Electricity disconnection scam check",
    ...
    lines.push("", "If a UPI request was approved or a screen-share app installed, call 1930 and file at cybercrime.gov.in.");
    return lines.join("\n");
  }, [ok, result]);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Electricity Bill Scam Anatomy
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          The &ldquo;your power will be disconnected tonight at 9:30&rdquo; SMS taken apart line by
          line &mdash; including the remote-access endgame it is really aiming at &mdash; with a
          scanner that weighs the same tells against anything you paste. Everything runs in this
      ...
      <section className={CARD} aria-labelledby="pa-scan">
        ...
        <div className="mt-3">
          ...
          <p className="mt-2 text-xs leading-5 text-[var(--muted-foreground)]">
            Defanged addresses work too &mdash; bad[.]top and hxxp:// are read as written. Never
            paste your own UPI PIN, OTP or card number anywhere, including here.
          </p>
        </div>
      ...
      <section className={`mt-6 ${CARD}`}>
        ...
        <p className="mt-3 text-xs leading-5 text-[var(--muted-foreground)]">
          Addresses above are written with [.] so nothing here is clickable, and the phone number
          is redacted. This is a composite of reported messages, not a copy of one real
          operator's SMS.
        </p>
      </section>

      <section className={`mt-6 ${CARD}`}>
        <h2 className="text-base font-semibold">What a real distribution company does</h2>
      ...
      <section className={`mt-6 ${CARD}`}>
        <h2 className="flex items-center gap-2 text-base font-semibold">
          ...
          If you already called back or paid
        </h2>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        The scanner looks for documented markers of the disconnection-tonight family and weighs
        them. A high score means the message is built the way these scams are built; a zero is not
        a certificate that a message is genuine, because a careful forgery can avoid every marker.
        Check what you actually owe on your own bill or in a BBPS-enabled app, and treat this page
        as general security education rather than legal or financial advice.
      </p>
    </main>
  );
}
