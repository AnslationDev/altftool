...
import { Check, Copy, Globe, Plus, RotateCcw, Trash2 } from "lucide-react";
...
const STORAGE_KEY = "altft-summits-reports-notes-tracker-v1";
...
const EMPTY_FORM = { name: "", organisation: "", host: "", year: "", outcome: "", notes: "" };
    ...
    <main className="mx-auto w-full max-w-4xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Summits And Reports Tracker
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          One table for &quot;summit — host — year&quot; and &quot;report — publisher — India&apos;s
          rank&quot; facts. Rows are saved in your browser and export as CSV.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <form onSubmit={addRow} className="mt-3">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className={LABEL_CLASS} htmlFor="sr-name">
                Summit / report / index
              </label>
              <input
                id="sr-name"
                ...
                placeholder="e.g. BRICS Summit, Global Hunger Index"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="sr-org">
              ...
              <input
                ...
                placeholder="e.g. UNFCCC, World Bank"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="sr-host">
                Host / venue (optional)
              </label>
              <input
                id="sr-host"
                ...
                placeholder="City, country — or 'Annual report'"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="sr-year">
                Year
              </label>
              <input
                id="sr-year"
            ...
            <div>
              <label className={LABEL_CLASS} htmlFor="sr-outcome">
                India&apos;s rank / outcome (optional)
              </label>
              <input
                id="sr-outcome"
                className={`mt-2 ${INPUT_CLASS}`}
                ...
                placeholder="e.g. India ranked 105 of 127"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="sr-notes">
                Notes (optional)
              </label>
              <input
                id="sr-notes"
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="w-full sm:max-w-xs">
            <label className={LABEL_CLASS} htmlFor="sr-search">
              Search the table
            </label>
            <input
              id="sr-search"
              ...
              placeholder="Summit, host, publisher, year…"
            />
          </div>
              ...
              {visible.length === 0 ? (
                ...
                visible.map((row) => (
                  <tr key={row.id} className="border-b border-[var(--border)] last:border-0">
                    ...
                    <td className="py-2.5 pr-3">{row.organisation}</td>
                    <td className="py-2.5 pr-3">{row.host || "—"}</td>
                    <td className="py-2.5 pr-3">{row.year}</td>
                    <td className="py-2.5 pr-3">{row.outcome || "—"}</td>
                    <td className="py-2.5 pr-3 text-[var(--muted-foreground)]">{row.notes || "—"}</td>
                    <td className="py-2.5 text-right">
                      <button
                        type="button"
                        onClick={() => deleteRow(row.id)}
                        aria-label={`Delete ${row.name} ${row.year}`}
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          ...
          {stats.byOrganisation.map(([org, count]) => (
            <div key={`o-${org}`} className="flex items-center justify-between gap-4 py-2.5">
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        Ranks change every edition, so verify against the official report before the exam; starter
        rows are examples, not a complete list.
      </p>
    </main>
  );
}
