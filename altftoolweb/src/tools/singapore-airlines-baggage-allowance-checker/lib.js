/**
 * Singapore Airlines (SQ) baggage allowance checker.
 *
 * Singapore Airlines uses two checked-baggage systems depending on the route:
 *
 *   • Weight concept — most of the network. One total figure in kilograms that
 *     the sum of all checked pieces must stay under, rising with the fare family
 *     and cabin (Economy Lite 25 kg through First and Suites 50 kg).
 *   • Piece concept — journeys to and from the Americas. A fixed number of bags
 *     each with its own weight ceiling: 2 × 23 kg in Economy and Premium Economy,
 *     2 × 32 kg in Business, First and Suites.
 *
 * Cabin baggage is policed by linear dimension, not by a length × width × height
 * triple: one piece whose three sides add up to no more than 115 cm, weighing up
 * to 7 kg in Economy and Premium Economy. Business, First and Suites carry two
 * such pieces under a single 14 kg combined weight limit.
 *
 * All figures below are the airline's published standard allowances. KrisFlyer
 * elite tiers, Star Alliance status, codeshare sectors, staff and group fares can
 * change them, so confirm the allowance on your own booking before you travel.
 *
 * Nothing here calls the airline: every result is computed from these constants
 * and the numbers the traveller types in.
 */

/** Airline identity, so the UI never hard-codes a name. */
export const AIRLINE = { name: "Singapore Airlines", code: "SQ", country: "Singapore" };

/** Economy and Premium Economy cabin bag: one piece, 7 kg, 115 cm total. */
export const CABIN_BAG_KG = 7;
export const CABIN_DIM_RULE = { type: "linear", maxSumCm: 115 };

/** The second cabin piece in premium cabins is measured on the same 115 cm rule. */
export const PERSONAL_DIM_RULE = { type: "linear", maxSumCm: 115 };

/** Business, First and Suites: two cabin pieces under one 14 kg combined ceiling. */
export const PREMIUM_CABIN_PIECES = 2;
export const PREMIUM_CABIN_COMBINED_KG = 14;

/** Checked pieces are limited by linear dimension: 158 cm of length + width + height. */
...
/**
 * No default excess rate ships with this tool. Singapore Airlines prices excess
 * per kilogram by route and sells prepaid weight at a different rate again, so a
 * stored average would be misleading. Enter the figure quoted for your booking.
 */
...
export const CURRENCIES = [
  ...
  { code: "MYR", locale: "en-MY", label: "MYR RM" },
];
...
/** Small personal item: no published weight, must fit under the seat in front. */
...
export const FARE_OPTIONS = [
  ...
  {
    key: "eco-30",
    label: "Economy Standard / Value — 30 kg",
    cabin: ECONOMY_CABIN,
    personal: PERSONAL_ITEM,
  ...
  {
    key: "americas-eco",
    label: "Economy to/from the Americas — 2 × 23 kg",
    cabin: ECONOMY_CABIN,
    personal: PERSONAL_ITEM,
  ...
  {
    key: "americas-pey",
    label: "Premium Economy to/from the Americas — 2 × 23 kg",
    cabin: ECONOMY_CABIN,
    personal: PERSONAL_ITEM,
    checked: { type: "piece", pieces: 2, perPieceKg: 23 },
  },
  {
    key: "americas-biz",
    label: "Business to/from the Americas — 2 × 32 kg",
    cabin: PREMIUM_CABIN,
    personal: PERSONAL_ITEM,
  ...
  {
    key: "americas-first",
    label: "First / Suites to/from the Americas — 2 × 32 kg",
    cabin: PREMIUM_CABIN,
    personal: PERSONAL_ITEM,
    checked: { type: "piece", pieces: 2, perPieceKg: 32 },
  },
...
export const DEFAULTS = {
  ...
  bags: [{ weightKg: "25", dims: ["70", "45", "25"] }],
