...
const DEFAULT_SELECTED = ["icloud-photos", "media-services", "account-device", "maps", "health"];
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (hasError) return "";
    return [
      "Apple data request plan",
      `Categories selected: ${result.count}`,
      `From privacy.apple.com: ${result.portalLabel} in about ${result.parts} part file(s) of ${result.partSizeGb} GB`,
      `Exported from the device instead: ${result.deviceLabel}`,
      `Combined: ${result.totalLabel}`,
      `Sensitivity: ${result.sensitivityScore}/100 (${result.band})`,
      `Apple may take up to ${VERIFICATION_DAYS} days to verify and prepare it`,
      `Download window once ready: ${DOWNLOAD_WINDOW_DAYS} days`,
    ].join("\n");
  }, [hasError, result]);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Apple Data Download Request Guide
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Plan a request at privacy.apple.com: choose categories, size the archive, pick a sensible
          part size, and see which data Apple cannot include because it is end-to-end encrypted and
          has to be exported from your device.
        </p>
      </header>

      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <h2 className="text-base font-semibold">1. Choose the data you want</h2>
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="apl-years">
              Apple Account age (years)
            </label>
            <input
              id="apl-years"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="apl-weight">
              How much media is in iCloud
            </label>
            <select
              id="apl-weight"
              className={`mt-2 ${INPUT_CLASS}`}
          ...
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="apl-split">
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
              Archive from privacy.apple.com
            </p>
            <p className="mt-1 text-4xl font-semibold text-[var(--primary)]">
              {hasError ? DASH : result.portalLabel}
            </p>
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              {hasError
                ? "Fix the input above to see an estimate."
                : result.parts === 0
                  ? "Nothing in this selection comes from the portal"
                  : `About ${NUM.format(result.parts)} part file${result.parts === 1 ? "" : "s"} at ${result.partSizeGb} GB each`}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the Apple data request plan"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
            ...
            <button
              type="button"
              onClick={reset}
              aria-label="Reset the Apple data request planner"
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            [
              "Exported from the device instead",
              hasError ? DASH : `${result.deviceLabel} across ${result.deviceCount} item(s)`,
            ],
            ["Combined size", hasError ? DASH : result.totalLabel],
            ...
            ["Apple verification and preparation", `up to ${VERIFICATION_DAYS} days`],
            ["Download window once ready", `${DOWNLOAD_WINDOW_DAYS} days`],
          ].map(([label, value]) => (
        ...
        {!hasError && result.deviceOnlyLabels.length > 0 ? (
          <p className="mt-4 rounded-md bg-[var(--muted)] px-3 py-2 text-xs leading-5 text-[var(--muted-foreground)]">
            Not included in the portal archive: {result.deviceOnlyLabels.join(", ")}. Apple cannot
            read this data, so you export it from the iPhone or Mac yourself.
          </p>
        ) : null}
      </section>
      ...
      {!hasError ? (
        <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
          <h2 className="text-base font-semibold">Size, source and risk by category</h2>
                ...
                {result.rows.map((row) => (
                  <tr key={row.id} className="border-b border-[var(--border)] last:border-0">
                    ...
                    <td className="py-2 pr-3 align-top text-xs text-[var(--muted-foreground)]">
                      {row.source === SOURCE_DEVICE ? "Device" : "Portal"}
                    </td>
