...
import { PERMISSIONS, auditPermissions, resultToText } from "../lib";

const DEFAULT_GRANTED = ["camera", "microphone", "photos-all", "location", "contacts", "notifications", "ad-id", "background-refresh"];
const INPUT_CLASS =
...
export default function ToolHome() {
  const [appName, setAppName] = useState("My social app");
  const [granted, setGranted] = useState(DEFAULT_GRANTED);
  const [copied, setCopied] = useState(false);
  const result = useMemo(() => auditPermissions(granted), [granted]);
    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          ...
          Social app permissions
        </div>
        <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">
          Social Media App Permission Audit
        </h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--muted-foreground)] sm:text-base">
          Tick the permissions a social app currently holds and get a privacy score with
          practical suggestions for camera, microphone, location, contacts, photos, tracking
        ...
        <div className={CARD}>
          ...
          <fieldset className="mt-5">
            <legend className="text-sm font-semibold text-[var(--foreground)]">
              Granted permissions and settings
            </legend>
          ...
          {result.error ? (
            ...
            <>
              ...
              <div className="mt-5 h-3 overflow-hidden rounded-full bg-[var(--muted)]">
                <div className="h-full rounded-full bg-[var(--primary)]" style={{ width: `${result.privacyScore}%` }} />
              </div>
