...
const DEFAULTS = {
  ...
  tripDays: 12,
  laundryEveryDays: 4,
  venueIds: ["bali-temple","beach","volcano","mall"],
  headCover: false,
};
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Indonesia Dress Code Planner for Travellers
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Pick the month and the places you plan to visit. Indonesia&rsquo;s rules are provincial, not
          national &mdash; Balinese sarong and sash, mosque cover-up, Aceh&rsquo;s Qanun Jinayat &mdash; so the
          planner applies only the ones your itinerary triggers, then sizes the list for the climate.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="id-month">
              Month of travel
            </label>
            <select
              id="id-month"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="id-days">
              Trip length (days)
            </label>
            <input
              id="id-days"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="id-laundry">
              Laundry every (days)
            </label>
            <input
              id="id-laundry"
            ...
            <p className="mt-1 text-xs text-[var(--muted-foreground)]">
              Laundry is priced by the kilo across Bali and Java and usually returns next day.
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the Indonesia packing plan"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Climate figures are Jakarta normals. Bali is drier, Sumatra and Papua wetter, and a volcano
        rim before dawn is single-figure Celsius whatever the coast is doing. Provincial rules change
        &mdash; this is travel guidance, not legal advice.
      </p>
    </main>
  );
}
