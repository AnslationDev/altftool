...
import { BEARD_STYLES, BEARD_COLORS, renderBeardStyle } from '../utils/beardStyles';
...
export default function BeardTool() {
  const [image, setImage] = useState(null);
  const [selectedStyle, setSelectedStyle] = useState('clean-shaven');
  const [selectedColor, setSelectedColor] = useState(BEARD_COLORS[1]);
  ...
  const [adjustments, setAdjustments] = useState({
    scale: 100, opacity: 100, length: 50, density: 50,
    darkness: 50, sharpness: 50, rotation: 180,
  });
  ...
  const applyPreview = useCallback(async () => {
    ...
    try {
      ...
      if (faceData) {
        renderBeardStyle(ctx, selectedStyle, faceData, {
          color: selectedColor.value || selectedColor.name === 'Custom' ? '#5c3a1e' : selectedColor.value,
          ...adjustments,
        });
      }
  ...
  const handleShare = async () => {
    ...
    await shareImage(blob, 'My Beard Style Preview!');
  };
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl font-bold text-foreground">Beard Style Preview</h1>
          <p className="text-sm text-muted-foreground">Upload a photo to try on different beard styles with AI-powered face detection</p>
        </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Beard Style Preview</h1>
          <p className="text-sm text-muted-foreground">Try different beard styles with AI-powered placement</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {processing && (
              <div className="absolute inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center">
                <div className="bg-card rounded-xl p-4 shadow-lg flex items-center gap-3">
                  ...
                  <span className="text-sm text-foreground">Applying beard style...</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-[var(--border)] p-4 space-y-2 max-h-80 overflow-y-auto">
            ...
            {BEARD_STYLES.filter(s => category === 'all' || s.category === category).map(style => (
          ...
          <div className="rounded-xl border border-[var(--border)] p-4 space-y-3">
            <h3 className="text-sm font-medium text-foreground">Beard Color</h3>
            <div className="flex flex-wrap gap-2">
              {BEARD_COLORS.filter(c => c.name !== 'Custom').map(c => (
          ...
          <div className="rounded-xl border border-[var(--border)] p-4 space-y-3">
            ...
            {[
              ...
              { key: 'length', label: 'Length', min: 10, max: 100 },
              { key: 'density', label: 'Density', min: 10, max: 100 },
              { key: 'darkness', label: 'Darkness', min: 10, max: 100 },
