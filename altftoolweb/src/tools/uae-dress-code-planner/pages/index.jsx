...
const DEFAULT_VENUES = ["malls", "beach", "restaurants"];
...
export default function ToolHome() {
  ...
  const result = useMemo(
    () =>
      planDressCode({
        ...
        venueIds,
        duringRamadan: form.duringRamadan,
      }),
    [form, venueIds],
  );
  ...
  const summary = useMemo(() => {
    if (hasError) return "";
    return [
      `UAE packing and dress plan — ${result.month}, ${result.region.label}`,
      `Typical month: ${result.meanTempC} °C mean, ${result.maxTempC} °C daily maximum (${result.band.label})`,
      `Trip: ${result.tripDays} days, laundry every ${result.laundryEveryDays} days${result.duringRamadan ? ", during Ramadan" : ""}`,
      "",
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl leading-tight font-semibold sm:text-4xl">
          UAE Dress Code Planner for Travellers
        </h1>
      ...
      <section className={CARD_CLASS}>
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            />
            <p className={HINT_CLASS}>Hotel and mall laundry services are quick and cheap here.</p>
          </div>
        </div>

        <label className={`mt-4 ${CHECK_ROW}`} htmlFor="in-ramadan">
          <input
            id="in-ramadan"
            type="checkbox"
            className="mt-0.5 h-5 w-5 shrink-0 accent-[var(--primary)]"
            checked={form.duringRamadan}
            onChange={(event) => setField("duringRamadan", event.target.checked)}
          />
          <span>
            <span className="block font-medium">My trip falls during Ramadan</span>
            <span className="mt-0.5 block text-xs text-[var(--muted-foreground)]">
              Ramadan moves about eleven days earlier each year, so check the dates for your travel
              year.
            </span>
          </span>
        </label>
      ...
      <section className={`mt-6 ${CARD_CLASS}`}>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              {hasError
                ...
                : `${result.month} in ${result.region.label} — ${result.band.label.toLowerCase()}`}
            </p>
          </div>
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Typical daily maximum", hasError ? DASH : `${result.maxTempC} °C`],
            ...
            [
              ...
              hasError ? DASH : result.needsHeadCover ? "Yes, for the mosque visit" : "No",
            ],
            [
              "Shoulders and knees covered",
              hasError ? DASH : result.needsShoulders || result.needsKnees ? "Yes, for at least one venue" : "Not for the places you ticked",
            ],
            [
              "Midday outdoors",
              hasError ? DASH : result.indoorMiddayNeeded ? "Not realistic — plan indoors" : "Manageable with shade and water",
            ],
          ].map(([label, value]) => (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Planning aid only, and not legal advice. Temperature and rainfall figures are rounded
        long-run monthly averages rather than a forecast. Dress requirements differ between emirates
        and between individual venues and can change — check current guidance from your embassy and
        from the venue itself before you travel.
      </p>
    </main>
  );
}
