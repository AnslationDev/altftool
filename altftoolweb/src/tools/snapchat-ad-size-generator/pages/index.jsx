...
import { Check, Copy, Download, Ghost, RotateCcw, Upload } from "lucide-react";
...
const DEFAULT_SOURCE = { width: "1440", height: "1800" };
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    ...
    const safeLine = plan.safe.error
      ? plan.safe.error
      : `${plan.safe.width} x ${plan.safe.height} px (${NUM.format(plan.safe.coveragePercent)}% of the frame)`;
    return [
      `Snapchat creative spec — ${preset.label}`,
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = `${imageName || "snap"}-${plan.target.width}x${plan.target.height}.${chosen.extension}`;
  ...
  const hasSafeZone = !plan.error && !plan.safe.error && plan.safe.top + plan.safe.bottom > 0;
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Snapchat Ad Size Generator</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Export Snap Ads, story snaps, brand tiles and geofilters at the exact pixel size, with the
          safe zone drawn on the preview and the file-weight ceiling checked after export. Everything
          runs locally in your browser.
        </p>
      </header>

      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <h2 className="text-base font-semibold">1. Choose the creative</h2>
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <div className="mt-3">
          <label className={LABEL_CLASS} htmlFor="snap-image-file">
          ...
          <p className="mt-2 text-xs leading-5 text-[var(--muted-foreground)]">
            No file yet? The spec figures below work from the source dimensions you type.
          </p>
        </div>

        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="snap-src-width">
              Source width (px)
            </label>
            <input
              id="snap-src-width"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="snap-src-height">
              Source height (px)
            </label>
            <input
              id="snap-src-height"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="snap-fit">
              Fit mode
            </label>
            <select
              id="snap-fit"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="snap-background">
              Bar background
            </label>
            <select
              id="snap-background"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="snap-format">
              Export format
            </label>
            <select
              id="snap-format"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="snap-quality">
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the Snapchat creative specification"
              className={GHOST_BTN}
            ...
            <button
              ...
              aria-label="Export and download the resized creative"
        ...
        {exported && exportedWeight && !exportedWeight.error && (
          ...
          >
            ...
            {exportedWeight.ok ? "." : ". Lower the encoder quality or simplify the artwork."}
          </p>
        )}
      </section>
        ...
        <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
          ...
          <div className="mt-3 flex justify-center">
            ...
            >
              {imageUrl ? (
                ...
                <img
                  src={imageUrl}
                  alt="Preview of how your artwork sits inside the Snapchat creative frame"
          ...
          {hasSafeZone && (
            <p className="mt-3 text-center text-xs leading-5 text-[var(--muted-foreground)]">
              Keep headlines and logos between the dashed lines — {plan.safe.top} px is reserved at
              the top and {plan.safe.bottom} px at the bottom.
            </p>
          )}
        </section>
      )}
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <h2 className="text-base font-semibold">Every Snapchat creative size</h2>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Ad specifications change without notice. Confirm the current creative requirements in
        Snapchat Ads Manager before submitting a campaign.
      </p>
    </main>
  );
}
