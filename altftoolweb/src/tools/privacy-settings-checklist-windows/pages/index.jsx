...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, Monitor } from "lucide-react";
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Windows 11 Privacy Settings Checklist</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Twenty-eight Windows 11 settings, ranked by how much data each one actually stops leaving your machine. Pick the profile that matches the PC you are hardening, tick what you have already changed, and the score shows what is still reporting home.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="win-profile">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="win-target">
              Target score (%)
            </label>
            <input
              id="win-target"
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]" aria-labelledby="win-score">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p id="win-score" className="text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
              Protection score
            </p>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Assuming BitLocker is on because the PC is new. Device encryption depends on the hardware and edition &mdash; check the status page rather than trusting it.</li>
          <li>Turning encryption on without saving the recovery key anywhere. A firmware update that triggers a recovery prompt then destroys the data permanently.</li>
          <li>Turning diagnostics down and never pressing Delete diagnostic data, so everything collected before the change stays associated with the device.</li>
          <li>Not re-checking after a Windows feature update, which can reset privacy toggles to their defaults.</li>
          <li>Leaving OneDrive folder backup on from the setup wizard, so &ldquo;local&rdquo; Desktop and Documents files are in the cloud and on every device you sign into.</li>
          <li>Treating Recall as harmless because the snapshots stay on the device. Local or not, it concentrates every sensitive thing you have looked at into one searchable index.</li>
        </ul>
      </section>
