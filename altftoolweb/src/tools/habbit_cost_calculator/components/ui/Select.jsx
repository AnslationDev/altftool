...
export const Select = React.forwardRef(({ label, error, options, className = '', id, ...props }, ref) => {
  ...
  return (
    <div className="relative">
      {label && (
        <label htmlFor={selectId} className="block text-sm font-medium text-(--secondary) mb-2">
          {label}
        </label>
      )}
      <div className="relative">
        ...
        >
          {options.map((option) => (
            <option
              key={option.value}
              value={option.value}
              className="bg-slate-100 text-slate-900 dark:bg-slate-900 dark:text-slate-100"
            >
              {option.label}
            </option>
          ))}
