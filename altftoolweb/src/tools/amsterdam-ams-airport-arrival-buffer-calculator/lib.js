/**
 * Departure planning for Amsterdam Airport Schiphol (AMS / EHAM).
 *
...
export const AIRPORT = {
  ...
  city: "Amsterdam",
  terminals: [
    "One terminal with Departure Halls 1, 2 and 3 sharing a single airside area",
    "Schengen piers B, C and D-lower; non-Schengen piers D-upper, E, F, G and H/M",
    "Pier H/M and the far end of E add a long walk after passport control",
  ],
};
...
/** Baseline queue times at AMS in normal conditions, before the queue factor. */
...
/**
 * Schiphol advises being at the airport about 2 hours before a European flight and
 * 3 hours before an intercontinental one, which is what the two options use. A
 * Schengen departure has no border check; anything else passes through the Royal
 * Marechaussee passport control. Check-in closing 40 minutes before a European
 * departure and 60 minutes before an intercontinental one, with the gate closing 20
 * minutes before departure, follow the common KLM rules; other carriers differ and
 * several close bag drop earlier.
 */
export const JOURNEY_TYPES = [
  {
    id: "schengen",
    label: "European / Schengen (no passport control)",
    ...
    gateWalkMinutes: 18,
    checkInClosesMinutes: 40,
    gateClosesMinutes: 20,
  },
  {
    id: "nonschengen",
    label: "Intercontinental / non-Schengen",
...
/** Multiplier applied to the free-flow drive time. The A4 and A9 ring roads are the constraint; Dutch peaks are sharp but short, and rain reliably adds a quarter to the journey. */
export const TRAFFIC_LEVELS = [
  ...
  { id: "severe", label: "Severe (file on the A4, roadworks, storm)", multiplier: 1.8 },
];
...
export const QUEUE_LEVELS = [
  ...
  { id: "peak", label: "Peak bank (05:00-09:00 morning wave)", multiplier: 1.5 },
];

/**
 * Amsterdam road rush-hour windows, as minutes from midnight. Used only to SUGGEST a
...
/**
 * Indicative free-flow (no-traffic) road times to AMS from common starting
 * points. They are starting estimates only - replace the minutes with a live maps
 * estimate for your own address.
 */
export const AREA_PRESETS = [
  { id: "hoofddorp", label: "Hoofddorp / Schiphol-Rijk", driveMinutes: 10 },
  { id: "amstelveen", label: "Amstelveen / Aalsmeer", driveMinutes: 15 },
  { id: "zuid", label: "Amsterdam Zuid / Zuidas", driveMinutes: 15 },
  { id: "amsterdamcentre", label: "Amsterdam centre / Centraal", driveMinutes: 25 },
  { id: "haarlem", label: "Haarlem", driveMinutes: 20 },
  { id: "leiden", label: "Leiden", driveMinutes: 30 },
  { id: "almere", label: "Almere", driveMinutes: 40 },
  { id: "utrecht", label: "Utrecht", driveMinutes: 45 },
  { id: "thehague", label: "The Hague", driveMinutes: 50 },
  { id: "rotterdam", label: "Rotterdam", driveMinutes: 55 },
