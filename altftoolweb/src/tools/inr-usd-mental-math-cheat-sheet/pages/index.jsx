...
const usd = (value) => `${CURRENCY.symbol}${NUM2.format(Number.isFinite(value) ? value : 0)}`;
...
const DEFAULTS = {
  ...
  amount: "50",
  tax: String(DEFAULT_SALES_TAX_PERCENT),
  tip: String(DEFAULT_TIP_PERCENT),
};
...
export default function ToolHome() {
  ...
  const sheet = useMemo(
    () => buildCheatSheet({ inrPerUnit: rate, amount, salesTaxPercent: tax, tipPercent: tip }),
    [rate, amount, tax, tip],
  );
  ...
  const summary = useMemo(() => {
    if (!ok) return "";
    const lines = [
      `INR / USD mental math cheat sheet at ₹${sheet.rate} per ${CURRENCY.symbol}1`,
    ...
    if (sheet.worked) {
      lines.push(
        "",
        `${CURRENCY.symbol}${sheet.worked.amount} on the shelf = ${CURRENCY.symbol}${NUM2.format(sheet.worked.till.total)} at the till = ${money(sheet.worked.tillInr)}`,
      );
    }
    return lines.join("\n");
  }, [ok, sheet, forward]);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          ...
          Dollars in your head
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          INR to USD Mental Math Cheat Sheet
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Enter today&apos;s rate and this works out the roundest multiplication that still lands
          close to the real answer, tells you exactly how far off it is, and prints a ladder you can
          keep on your phone. It also adds what American price tags leave out: sales tax at the
          register and the tip.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="usd-rate">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="usd-amount">
              A price on the tag ({CURRENCY.symbol})
            </label>
            <input
              id="usd-amount"
              className={`mt-2 ${INPUT_CLASS}`}
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="usd-tax">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="usd-tip">
        ...
        <p className="mt-3 text-xs leading-5 text-[var(--muted-foreground)]">
          Sales tax is set by state and city, so it varies: {NO_SALES_TAX_STATES.join(", ")} charge
          none at all, while some cities pass 10%. Set the tip to 0 for shops and supermarkets.
        </p>
      </section>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
              {ok && sheet.worked
                ? `${CURRENCY.symbol}${NUM2.format(sheet.worked.amount)} on the tag is`
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              {ok && sheet.worked
                ? `${moneyExact(sheet.worked.tillInr)} once ${NUM2.format(sheet.salesTaxPercent)}% tax and ${NUM2.format(sheet.tipPercent)}% tip are on the bill`
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={copyResult}
              disabled={!ok}
              aria-label="Copy the dollar to rupee cheat sheet"
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            [
              "One rupee is",
              ok ? `${CURRENCY.symbol}${sheet.unitsPerRupee.toFixed(4)}` : DASH,
            ],
            [
              "Tag price at the till",
              ok && sheet.worked ? usd(sheet.worked.till.total) : DASH,
            ],
            [
              "Sales tax and tip added",
              ok && sheet.worked
                ? `${usd(sheet.worked.till.taxAmount)} tax + ${usd(sheet.worked.till.tipAmount)} tip`
                : DASH,
            ],
            [
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-3">
        ...
        {!ok && (
          <p className="text-sm text-[var(--muted-foreground)] sm:col-span-3">{DASH}</p>
        )}
      </section>

      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <h2 className="text-lg font-semibold">Dollars to rupees</h2>
        ...
        <h2 className="mt-6 text-lg font-semibold">Rupees to dollars</h2>
        <div className="mt-3 overflow-x-auto">
            ...
            <tbody>
              {(ok ? sheet.rupeeLadder : []).map((row) => (
                <tr key={row.rupees} className="border-b border-[var(--border)] last:border-0">
                  ...
                  <td className="py-2 pr-3 text-right">{usd(row.exactUnits)}</td>
                  <td className="py-2 text-right text-[var(--muted-foreground)]">
                    {usd(row.ruleUnits)}
                  </td>
                </tr>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        The rupee-dollar rate floats and the rate you get from a card, a forex card or a money
        changer is not the mid-market rate — enter the rate you are actually quoted. Rules here are
        for estimating in a shop, not for accounting.
      </p>
    </main>
  );
}
