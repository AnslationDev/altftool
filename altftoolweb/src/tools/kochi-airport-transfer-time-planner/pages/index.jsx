...
/** Presentation helper: turn a minute value into "23:30" plus a day note. */
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (hasError) return "";
    return [
      `Kochi airport (${AIRPORT.iata}) transfer plan`,
      `Flight: ${formatHhmm(result.flightMinute)} — ${result.journey.label}`,
      `From: ${result.zone.label} by ${result.mode.label}`,
      ...
      result.usesMetro
        ? `Metro to Aluva: ${formatDuration(result.metroRideMin)}, then ${formatDuration(result.interchangeMin)} to change`
        : null,
      `Road leg: ${result.roadKm} km in ${formatDuration(result.driveMinutes)} at about ${result.averageSpeedKmph} km/h (${result.trafficBand})`,
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Work backwards from the scheduled departure to the minute you should set off, with Kochi
          traffic simulated hour by hour and the metro leg to Aluva counted as fixed minutes that
          traffic cannot touch. {AIRPORT.terminalNote}
        </p>
      </header>
      ...
      <section className={`mt-6 ${CARD_CLASS}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the Kochi airport transfer plan"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            [
              "Metro to Aluva",
              hasError || !result.usesMetro
                ? DASH
                : `${formatDuration(result.metroRideMin)} on Line 1, plus ${formatDuration(result.interchangeMin)} to change`,
            ],
            [
              "Road leg",
              hasError
                ? DASH
                : `${formatDuration(result.driveMinutes)} for ${result.roadKm} km — ${result.trafficBand}`,
            ],
            ["Average road speed", hasError ? DASH : `${result.averageSpeedKmph} km/h`],
      ...
      <section className={`mt-6 ${CARD_CLASS}`}>
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
            <h2 className="text-base font-semibold">Kochi&apos;s slow hours</h2>
          ...
          <div>
            <h2 className="text-base font-semibold">Where it actually jams</h2>
            <ul className="mt-3 grid gap-2 text-sm text-[var(--muted-foreground)]">
              {BOTTLENECKS.map((spot) => (
                <li key={spot}>{spot}</li>
              ))}
            </ul>
          </div>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        not live data. Metro timings, check-in cutoffs and gate cutoffs vary and can change without
        notice, so confirm them with KMRL and your airline, and check the live flight status before
        you leave.
      </p>
    </main>
  );
