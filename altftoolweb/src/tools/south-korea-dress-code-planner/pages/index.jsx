...
const DEFAULTS = {
  ...
  tripDays: 8,
  laundryEveryDays: 4,
  venueIds: ["temple","floor-dining","palace-hanbok","shopping"],
  headCover: false,
};
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          South Korea Dress Code Planner for Travellers
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Pick the month and the places you plan to visit. The planner applies Korea&rsquo;s real
          expectations &mdash; shoes off, temple cover-up, nude bathing floors, the hanbok palace rule
          &mdash; alongside KMA climate normals for Seoul, and returns a counted packing list.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="kr-month">
              Month of travel
            </label>
            <select
              id="kr-month"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="kr-days">
              Trip length (days)
            </label>
            <input
              id="kr-days"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="kr-laundry">
              Laundry every (days)
            </label>
            <input
              id="kr-laundry"
            ...
            <p className="mt-1 text-xs text-[var(--muted-foreground)]">
              Coin laundries are common near guesthouses and most hotels have a same-day service.
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the South Korea packing plan"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Climate figures are Korea Meteorological Administration normals for Seoul. Gangwon and the
        mountains run colder, Jeju and Busan milder and windier. Venue expectations are social norms
        rather than law, but they are followed closely.
      </p>
    </main>
  );
}
