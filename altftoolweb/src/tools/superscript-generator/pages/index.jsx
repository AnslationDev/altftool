...
const SUPER_CHARS = Object.keys(SUPER_MAP);

function toSuperscript(text) {
  return text.split("").map((ch) => SUPER_MAP[ch] || ch).join("");
}
...
export default function ToolHome() {
  ...
  const result = useMemo(() => toSuperscript(text), [text]);

  const unsupported = useMemo(() => {
    if (!text) return [];
    return text.split("").filter((ch) => !SUPER_CHARS.includes(ch) && !/[\s]/.test(ch));
  }, [text]);
    ...
    <main className="min-h-screen bg-(--background) px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="rounded-2xl border border-(--border) bg-(--card) p-6">
          <h1 className="text-3xl font-bold text-(--foreground)">Superscript Generator</h1>
          <p className="mt-1 text-(--muted-foreground)">Convert text into Unicode superscript characters</p>
        </div>
        ...
        <div className="rounded-2xl border border-(--border) bg-(--card) p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-(--foreground)">Superscript Output</h2>
          ...
          {result ? (
            ...
            <p className="text-sm text-(--muted-foreground) py-6 text-center">Enter text above to see superscript output</p>
          )}
          ...
          {unsupported.length > 0 && (
            <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
              ...
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                These characters have no superscript equivalent and will appear as-is.
              </p>
            </div>
          )}
