...
export default function ToolHome() {
  ...
  const [passphrase, setPassphrase] = useState("Chandni-42-Bagh");
  const [gpus, setGpus] = useState("1");
  const [copied, setCopied] = useState(false);

  const score = useMemo(() => scoreChecklist(done, profileId), [done, profileId]);
  const strength = useMemo(
    () => wifiPassphraseStrength(passphrase, { gpus: gpus.trim() === "" ? Number.NaN : Number(gpus) }),
    [passphrase, gpus]
  );
  ...
  const summary = useMemo(() => {
    if (!hasScore) return "";
    const lines = [
      "TP-Link Router Hardening Checklist",
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          {CHECKLIST.length} steps for Archer and Deco units, weighted by how much exposure each one
          actually removes. Tick what is already done and the score shows what is still open, with
          the exact menu path for the rest.
        </p>
      </header>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
              ...
              {DEVICE.gateways.join(" / ")} typed into a browser on the Wi-Fi. App:{" "}
              {DEVICE.app}.
            </p>
          </div>
        </div>
        ...
        {hasScore && score.capped ? (
          <p className="mt-4 flex items-start gap-2 rounded-md bg-[var(--warning-soft)] px-3 py-2 text-sm text-[var(--warning)]">
            ...
            <span>
              ...
              an attacker can use today, whatever else you have tightened.
            </span>
          </p>
        ) : null}
      ...
      <section className={`mt-6 ${CARD}`}>
        ...
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">
          A WPA2 handshake can be captured in seconds and attacked offline. This estimates a pure
          brute-force search at published hashcat speeds. Nothing you type leaves this page.
        </p>
      ...
      <section className={`mt-6 ${CARD}`}>
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>
            Hiding the SSID and calling it security &mdash; the name is still broadcast by every
            device that connects to it, and it breaks roaming on some phones.
          </li>
          <li>
            Relying on MAC filtering, which is defeated by copying a MAC address seen in the air in
            about a minute.
          </li>
          <li>
            Changing the Wi-Fi password but leaving the admin password at the factory value, which
            lets anyone already connected read the new one straight off the settings page.
          </li>
          <li>
            Leaving remote management on &ldquo;just for now&rdquo; after a support call and never
            switching it back off.
          </li>
          <li>
            Upgrading firmware for the wrong hardware revision &mdash; V1 firmware on a V2 board can
            brick the unit, so check the label first.
          </li>
        </ul>
      </section>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        asks for your real router password. {DEVICE.note} Menu wording moves between firmware
        releases, so follow the description rather than hunting for an exact label, and check
        TP-Link&rsquo;s own support pages for your model and hardware version. This is general
        security guidance, not a substitute for professional advice on a business network.
      </p>
    </main>
  );
}
