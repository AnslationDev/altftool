...
const DEFAULTS = {
  ...
  tripDays: 8,
  laundryEveryDays: 4,
  venueIds: ["cathedral","bistro","museum"],
  headCover: false,
};
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          France Dress Code Planner for Travellers
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Pick the month and the places you plan to visit. The planner applies the rules that are
          actually enforced &mdash; church door policies, restaurant codes, the public-pool swimwear
          rule &mdash; alongside M&eacute;t&eacute;o-France climate normals, and returns a counted packing list.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="fr-month">
              Month of travel
            </label>
            <select
              id="fr-month"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="fr-days">
              Trip length (days)
            </label>
            <input
              id="fr-days"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="fr-laundry">
              Laundry every (days)
            </label>
            <input
              id="fr-laundry"
            ...
            <p className="mt-1 text-xs text-[var(--muted-foreground)]">
              Coin laveries are common in French towns; hotel laundry is charged per piece.
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the France packing plan"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Climate figures are Paris-Montsouris normals. The Riviera runs several degrees warmer and
        drier in summer, Brittany far wetter, and the Alps are a different climate entirely. This is
        travel guidance, not legal advice &mdash; check municipal by-laws for the commune you visit.
      </p>
    </main>
  );
}
