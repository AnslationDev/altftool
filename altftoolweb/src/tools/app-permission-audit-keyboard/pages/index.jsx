...
import { Keyboard, Check, Copy, RotateCcw } from "lucide-react";
...
export default function ToolHome() {
  const [appName, setAppName] = useState("My keyboard app");
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Keyboard App Permission Audit</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          A keyboard sees every character you type, including passwords typed outside a password field. Tick what yours holds: each item is weighted by sensitivity and by whether typing genuinely needs it.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="kbd-app-name">
        ...
        {mode === "paste" ? (
          <div className="mt-4">
            <label className={LABEL_CLASS} htmlFor="kbd-paste">
              ...
              <ul className="mt-3 space-y-2">
                {PERMISSIONS.map((permission) => (
                  <li key={permission.id}>
                    <label
                      htmlFor={`kbd-perm-${permission.id}`}
