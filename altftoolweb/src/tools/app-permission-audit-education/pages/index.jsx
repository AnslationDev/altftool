...
import { GraduationCap, Check, Copy, RotateCcw } from "lucide-react";
...
export default function ToolHome() {
  const [appName, setAppName] = useState("My learning app");
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Education App Permission Audit</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Live classes need a camera and a microphone. Proctoring tools go much further — screen capture, the list of every installed app, sometimes accessibility control. This audit separates teaching from surveillance, which matters most when the user is a child.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="edu-app-name">
        ...
        {mode === "paste" ? (
          <div className="mt-4">
            <label className={LABEL_CLASS} htmlFor="edu-paste">
              ...
              <ul className="mt-3 space-y-2">
                {PERMISSIONS.map((permission) => (
                  <li key={permission.id}>
                    <label
                      htmlFor={`edu-perm-${permission.id}`}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational only. India's DPDP Act 2023 requires verifiable parental consent for users under 18 and bars behavioural advertising aimed at children; in the US, COPPA covers under-13s. If an app aimed at students carries an advertising ID, raise it with the school.
      </p>
    </main>
  );
}
