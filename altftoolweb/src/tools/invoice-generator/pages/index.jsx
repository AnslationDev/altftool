        ...
        <section className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-6 shadow-[var(--anslation-ds-shadow-sm)] lg:p-8">
          <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <button
              ...
              className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-[var(--primary)] px-4 text-sm font-semibold text-[var(--primary-foreground)]"
            >
            ...
            <div>
              <div className="mb-3 flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={addItem}
                  className="inline-flex h-9 items-center gap-2 rounded-md border border-[var(--border)] px-3 text-sm font-semibold hover:border-[var(--primary)] hover:text-[var(--primary)]"
                >
          ...
          <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-6 shadow-[var(--anslation-ds-shadow-sm)]">
            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[var(--border)] pb-5">
              <div>
                ...
                <h2 className="mt-1 text-2xl font-semibold">{businessName}</h2>
              </div>
            ...
            <div className="mt-5 overflow-hidden rounded-lg border border-[var(--border)]">
              <div className="grid grid-cols-[1fr_80px_110px_120px] bg-[var(--muted)] px-4 py-3 text-xs font-semibold uppercase text-[var(--muted-foreground)]">
                <span>Item</span>
              ...
              {items.map((item) => (
                <div key={item.id} className="grid grid-cols-[1fr_80px_110px_120px] border-t border-[var(--border)] px-4 py-3 text-sm">
                  <span>{item.name}</span>
                  <span className="text-right">{item.quantity}</span>
                  <span className="text-right">{formatMoney(item.rate)}</span>
                  <span className="text-right font-semibold">{formatMoney(item.quantity * item.rate)}</span>
                </div>
              ))}
            </div>
            ...
            <div className="ml-auto mt-5 max-w-sm space-y-2">
              {[
                ...
                <div key={label} className="flex justify-between text-sm text-[var(--muted-foreground)]">
                  <span>{label}</span>
                  <span>{formatMoney(value)}</span>
                </div>
              ))}
              <div className="flex justify-between border-t border-[var(--border)] pt-3 text-xl font-semibold">
                <span>Total</span>
