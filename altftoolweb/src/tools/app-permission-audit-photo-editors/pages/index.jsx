...
import { Image, Check, Copy, RotateCcw } from "lucide-react";
...
export default function ToolHome() {
  const [appName, setAppName] = useState("My photo editor");
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          A photo editor needs the picture you picked and somewhere to save the result. It does not need your whole gallery, the GPS tags inside your photos, or a copy of the original on someone's server. This audit scores the difference.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="photo-app-name">
        ...
        {mode === "paste" ? (
          <div className="mt-4">
            <label className={LABEL_CLASS} htmlFor="photo-paste">
              ...
              <ul className="mt-3 space-y-2">
                {PERMISSIONS.map((permission) => (
                  <li key={permission.id}>
                    <label
                      htmlFor={`photo-perm-${permission.id}`}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational only. Since Android 13 and iOS 14 the system photo picker hands an app one file with no permission at all, so a full-library grant is a choice, not a requirement. Read the app's terms for what rights it claims over uploaded images.
      </p>
    </main>
  );
}
