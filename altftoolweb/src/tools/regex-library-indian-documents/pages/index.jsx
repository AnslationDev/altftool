    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Regex Library for Indian Documents
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Structure-checked patterns for PAN, GSTIN, IFSC, UPI IDs, PIN codes, standard and
          BH-series vehicle numbers and Aadhaar format — with live testing and clear notes on what
          a regex cannot verify.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="rli-pattern">
              Document type
            </label>
            <select
              id="rli-pattern"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="rli-sample">
              Test a value
            </label>
            <input
              id="rli-sample"
            ...
            <p className="mt-1 text-xs text-[var(--muted-foreground)]">
              Use sample values — avoid pasting real identity numbers into any online tool.
            </p>
          </div>
        </div>
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            >
              {hasError ? DASH : result.matched ? "Valid format" : "Invalid format"}
            </p>
            <p className="mt-1 max-w-md text-sm text-[var(--muted-foreground)]">
              {hasError
                ? "Fix the input above to see a result."
                : `“${sample || "(empty)"}” checked against the ${pattern.name} pattern. Format validity does not prove the document exists.`}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyPattern}
              aria-label="Copy the selected document regex pattern"
              className={GHOST_BTN}
            >
              {copied ? (
            ...
            <button
              type="button"
              onClick={reset}
              aria-label="Reset document type and test input to defaults"
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          <div className="flex items-center justify-between gap-4 py-2.5">
            <dt className="text-[var(--muted-foreground)]">Check depth</dt>
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <h2 className="text-base font-semibold">What this pattern cannot check</h2>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        These patterns check structure only — checksums (PAN check letter, GSTIN mod-36, Aadhaar
        Verhoeff) and actual issuance need the issuing authority&apos;s verification service.
        Identity numbers are sensitive personal data under Indian law; collect them only with a
        lawful basis and mask them at rest. Informational, not legal advice.
      </p>
    </main>
  );
}
