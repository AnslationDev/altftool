...
const PRESETS = [
  ...
  { label: "Travel kettle", minV: "220", maxV: "240", freq: "50", watts: "2000" },
];
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (hasError) return "";
    return [
      "Plug and Voltage Guide for UK",
      `UK mains: ${MAINS_VOLTAGE_V} V, ${MAINS_FREQUENCY_HZ} Hz, BS 1363 type G sockets`,
      ...
      `Plug fuse: ${result.fuseA ? `${result.fuseA} A BS 1362` : "no standard fuse covers this load"}`,
  ...
  const rows = [
    ...
    [
      "BS 1362 plug fuse to fit",
      hasError ? DASH : result.fuseA ? `${result.fuseA} A` : "None — load exceeds 13 A",
    ],
    [
      `Fits a ${STANDARD_SOCKET_A} A socket`,
      hasError ? DASH : result.fitsStandardSocket ? "Yes" : "No — needs a dedicated circuit",
    ],
    [
      "Safe on a bathroom shaver socket",
      hasError ? DASH : result.shaverSocketOk ? "Yes" : `No — that outlet is only ${SHAVER_SOCKET_VA} VA`,
    ],
    [
      `Ceiling on one ${STANDARD_SOCKET_A} A plug`,
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Plug and Voltage Guide for UK</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          The UK runs {MAINS_VOLTAGE_V} V at {MAINS_FREQUENCY_HZ} Hz on the shuttered BS 1363 type G socket, and every
          British plug carries its own fuse. Enter what you are travelling with to see whether an adapter is enough
          and which fuse belongs in the plug.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="uk-plug">
              Plug on your device
            </label>
            <select
              id="uk-plug"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="uk-minv">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="uk-maxv">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="uk-freq">
              Label frequency
            </label>
            <select
              id="uk-freq"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="uk-watts">
              Rated power (W)
            </label>
            <input
              id="uk-watts"
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={copyResult}
              disabled={hasError}
              aria-label="Copy the UK plug and voltage verdict"
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <h2 className="text-lg font-semibold">Outlets you will meet in the UK</h2>
              ...
              {SOCKET_TYPES.map((socket) => (
                <tr key={socket.code} className="border-b border-[var(--border)] align-top last:border-0">
                  ...
                  <td className="py-2.5 pr-4">
                    {socket.code === "SHAVER" ? `${SHAVER_SOCKET_VA} VA` : `${socket.ratedCurrentA} A`}
                  </td>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        draw a little more. UK supply is allowed to sit anywhere between about {NUM0.format(SUPPLY_MIN_V)} V and{" "}
        {NUM0.format(SUPPLY_MAX_V)} V, and socket outlets are usually on a ring final circuit protected at{" "}
        {RING_CIRCUIT_A} A, so the plug fuse is what protects your lead. This is general travel guidance, not an
        electrical inspection — ask a qualified electrician about anything unusual.
      </p>
    </main>
  );
}
