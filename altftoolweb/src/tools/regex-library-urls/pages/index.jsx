...
const DEFAULTS = {
  patternId: PATTERNS[0].id, // full http(s) URL
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Regex Library for URLs</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Copy-ready patterns for full http(s) URLs, any-scheme URIs, strict RFC 1035 domains,
          protocol-optional website fields and range-checked IPv4 hosts — with live testing.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="rlu-pattern">
              Pattern
            </label>
            <select
              id="rlu-pattern"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="rlu-sample">
              Test a URL or domain
            </label>
            <input
              id="rlu-sample"
              className={`mt-2 ${INPUT_CLASS}`}
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyPattern}
              aria-label="Copy the selected URL regex pattern"
              className={GHOST_BTN}
            >
              {copied ? (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        In JavaScript, prefer <code>new URL(value)</code> in a try/catch for full parsing — it
        implements the WHATWG URL Standard. Use these patterns where a regex is the only option:
        form validation attributes, log grepping, database CHECK constraints and quick filters.
      </p>
    </main>
  );
}
