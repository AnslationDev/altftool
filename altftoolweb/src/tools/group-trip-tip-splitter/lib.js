...
/** Major units -> integer minor units, rounded half-up at the cent. */
 ...
 */
export function splitGroupTips({ people, items }) {
  ...
  const perPerson = people.map((name, index) => ({
    ...
    owesToPayers: fromCents(owedToPayers[index]),
    owesToPot: fromCents(owedToPot[index]),
    totalShare: fromCents(owedToPayers[index] + owedToPot[index]),
    paid: fromCents(paid[index]),
    net: fromCents(netCents[index]),
  }));
  ...
  const settlements = transfers.map((transfer) => ({
    ...
    amount: fromCents(transfer.cents),
  }));
  ...
  return {
    ...
    potTotal: fromCents(potCents),
    equalSplit: fromCents(Math.round(totalCents / count)),
