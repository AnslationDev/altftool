...
export default function ToolHome() {
  ...
  const result = useMemo(() => {
    const weightValue = Number(String(weight).trim());
    const parsed = {};
    for (const stage of COOKING_STAGES) {
      const value = toNumber(minutes[stage.id]);
      if (Number.isNaN(value)) return { error: "Enter minutes as plain numbers." };
      parsed[stage.id] = value;
    }
    return computeCookingCalories({
      weight: Number.isFinite(weightValue) ? weightValue : null,
      weightUnit,
      minutes: parsed,
    });
  }, [weight, weightUnit, minutes]);
  ...
  const summary = useMemo(() => {
    ...
    const lines = [
      ...
      `Kitchen time: ${NUM0.format(result.totalMinutes)} minutes`,
      `Calories burned (gross): ${NUM0.format(result.grossKcal)} kcal`,
      `Calories above resting (net): ${NUM0.format(result.netKcal)} kcal`,
      `Average intensity: ${NUM2.format(result.averageMet)} METs`,
      `Equivalent to about ${NUM0.format(result.briskWalkMinutes)} minutes of brisk walking`,
      "",
      "Breakdown:",
    ];
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Add up the calories you spend standing, chopping, cooking and clearing up, using the
          published MET values for kitchen work from the 2011 Compendium of Physical Activities.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="cook-weight">
              Body weight
            </label>
            <input
              id="cook-weight"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="cook-unit">
              Weight unit
            </label>
            <select
              id="cook-unit"
        ...
        <h2 className="mt-6 text-base font-semibold">Minutes spent on each stage</h2>
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          {COOKING_STAGES.map((stage) => (
            <div key={stage.id}>
              <label className={LABEL_CLASS} htmlFor={`cook-${stage.id}`}>
                {stage.label}
              </label>
              <input
                id={`cook-${stage.id}`}
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div aria-live="polite" role="status">
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              {hasError
                ...
                : `over ${NUM0.format(result.totalMinutes)} minutes in the kitchen`}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={copyResult}
              disabled={hasError}
              aria-label="Copy cooking calorie result"
        ...
        >
          {[
            ...
            [
              "Same as brisk walking for",
              hasError ? DASH : `${NUM0.format(result.briskWalkMinutes)} minutes`,
            ],
          ].map(([label, value]) => (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        MET values describe an average adult, so your real burn varies with body composition,
        fitness and how briskly you actually move. This is informational only and is not a substitute
        for advice from a doctor or a registered dietitian.
      </p>
    </main>
  );
}
