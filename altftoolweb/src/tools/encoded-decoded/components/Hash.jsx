  ...
  const md5 = (str) => {
    ...
    for (i = 0; i < m.length; i += 16) {
      ...
      for (j = 0; j < 64; j++) {
        ...
        let tmp = h[3];
        h[3] = h[2]; h[2] = h[1];
        const sum = (h[0] + f + k[j] + (m[i + (j < 16 ? j : j < 32 ? (5 * j + 1) % 16 : j < 48 ? (3 * j + 5) % 16 : (7 * j) % 16)] | 0)) | 0;
        const rot = [7, 12, 17, 22, 5, 9, 14, 20, 4, 11, 16, 23, 6, 10, 15, 21][(j >> 4 << 2) | (j & 3)];
        h[1] = h[1] + ((sum << rot) | (sum >>> (32 - rot)));
        h[0] = tmp;
      }
      for (j = 0; j < 4; j++) h[j] = (h[j] + a[j]) | 0;
    }
    let res = "";
    for (i = 0; i < 16; i++) res += ((h[i >> 2] >> ((i % 4) * 8)) & 0xff).toString(16).padStart(2, "0");
    return res;
  };
