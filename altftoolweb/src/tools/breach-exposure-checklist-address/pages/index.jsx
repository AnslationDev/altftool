...
import { Check, Clock, Copy, MapPinHouse, RotateCcw, ShieldCheck, TriangleAlert } from "lucide-react";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (!hasScore) return "";
    const lines = [
      "Home Address Exposure Checklist",
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          You cannot change a home address the way you change a password, so the response is to make
          the household safe and to stop the address counting as proof of who you are. Tick each step
          as you finish it &mdash; the score weights the ones that actually reduce harm.
        </p>
      </header>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the address exposure response plan"
              className={GHOST_BTN}
            >
              {copied ? (
        ...
        {hasScore && score.capped ? (
          <p className="mt-4 flex items-start gap-2 rounded-md bg-[var(--warning-soft)] px-3 py-2 text-sm text-[var(--warning)]">
            ...
            <span>
              ...
              for an address that still works as a verification answer.
            </span>
          </p>
        ) : null}
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>
            Requesting a takedown before screenshotting the page, which removes the evidence a police
            complaint needs.
          </li>
          <li>
            Opting out of one people-search site and stopping &mdash; they source from the same public
            records and relist within months.
          </li>
          <li>
            Treating a caller who knows your address as verified. That is now public information, not
            a secret.
          </li>
          <li>
            Arguing with whoever posted it. Engagement keeps the post visible and invites others to
            repost the address elsewhere.
          </li>
        </ul>
      </section>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        page never asks for your address. If the exposure came with threats or harassment, contact the
        police and a lawyer rather than handling it alone.
      </p>
    </main>
  );
}
