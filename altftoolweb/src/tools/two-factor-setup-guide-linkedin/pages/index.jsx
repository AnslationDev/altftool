  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">LinkedIn 2FA Setup Guide</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Tick off each control as you set it. The score is weighted by how much each one actually blocks an account takeover, and it will not go past 69% while a critical control is still open.
        </p>
      </header>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Enabling two-step verification but leaving SMS as the method — a SIM swap at the mobile operator defeats it entirely.</li>
          <li>Closing the recovery-code screen without saving the codes, then discovering it only appears once.</li>
          <li>Leaving an old employer&rsquo;s email on the account, which lets whoever inherits that mailbox reset the password.</li>
          <li>Accepting connection requests from &ldquo;recruiters&rdquo; with brand-new profiles, which is the usual first step of a LinkedIn job scam.</li>
        </ul>
      </section>
