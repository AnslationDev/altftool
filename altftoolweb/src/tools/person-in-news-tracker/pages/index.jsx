...
import { Check, Copy, Plus, RotateCcw, Trash2, UserRound } from "lucide-react";
...
const STORAGE_KEY = "altft-person-in-news-tracker-v1";
...
const EMPTY_FORM = { person: "", role: "", organisation: "", date: "", why: "" };
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Person In News Tracker</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Log every &quot;who was appointed as what, and when&quot; fact in one revisable table. Rows
          are saved in your browser and export as CSV.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <form onSubmit={addRow} className="mt-3">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className={LABEL_CLASS} htmlFor="pn-person">
                Person
              </label>
              <input
                id="pn-person"
            ...
            <div>
              <label className={LABEL_CLASS} htmlFor="pn-role">
                Role / appointment
              </label>
              <input
                id="pn-role"
                ...
                placeholder="e.g. RBI Governor, Army Chief"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="pn-org">
              ...
              <input
                ...
                placeholder="e.g. RBI, ISRO, UN"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="pn-date">
                Date
              </label>
              <input
                id="pn-date"
            ...
            <div className="sm:col-span-2">
              <label className={LABEL_CLASS} htmlFor="pn-why">
                Why in news (optional)
              </label>
              <input
                id="pn-why"
                className={`mt-2 ${INPUT_CLASS}`}
                ...
                placeholder="One-line reason — appointment, award, record…"
              />
            </div>
          </div>
          {formError ? (
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="w-full sm:max-w-xs">
            <label className={LABEL_CLASS} htmlFor="pn-search">
              Search the table
            </label>
            <input
              id="pn-search"
              ...
              {visible.length === 0 ? (
                ...
                visible.map((row) => (
                  <tr key={row.id} className="border-b border-[var(--border)] last:border-0">
                    <td className="py-2.5 pr-3 font-semibold">{row.person}</td>
                    ...
                    <td className="py-2.5 pr-3 text-[var(--muted-foreground)]">{row.why || "—"}</td>
                    <td className="py-2.5 text-right">
                      <button
                        type="button"
                        onClick={() => deleteRow(row.id)}
                        aria-label={`Delete ${row.person}`}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        Verify appointments against official notifications before the exam; starter rows are
