...
export default function ToolHome() {
  ...
  const [visaStatus, setVisaStatus] = useState(DEFAULTS.visaStatus);
  ...
  const result = useMemo(
    ...
    [licenceOrigin, idpHeld, visaStatus, arrivalDate, departureDate, ageYears],
  );
  ...
  const summary = useMemo(() => {
    ...
    return [
      ...
      `Visa status: ${result.visaLabel}`,
      `Permit held: ${result.idpLabel}`,
      `Arrival: ${prettyDate(arrivalDate)}`,
      `How long the foreign licence works: ${result.windowLabel}`,
      `Recognised for a visitor: ${result.touristRecognised ? "yes" : "no"}`,
      `Convertible without a test: ${result.residentConvertible ? "yes" : "no"}`,
  ...
  const reset = () => {
    ...
    setVisaStatus(DEFAULTS.visaStatus);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          In the UAE the visa decides more than the permit does. A visitor can often hire on the
          licence alone; the day a residence visa is issued, the foreign licence and the permit both
          stop working. This checker sorts out which side of that line you are on.
        </p>
      </header>
      ...
      <section className={CARD}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="ae-visa">
          ...
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="ae-idp">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ae-arrival">
              Date you arrive
            </label>
            <input
              id="ae-arrival"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ae-departure">
      ...
      <section className={`mt-6 ${CARD}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the IDP verdict for the UAE"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Visa status", hasError ? DASH : result.visaLabel],
            ["How long your licence works here", hasError ? DASH : result.windowLabel],
            ["Recognised for a visitor on its own", hasError ? DASH : YES_NO(result.touristRecognised)],
            [
              "Convertible without a test",
              hasError ? DASH : YES_NO(result.residentConvertible),
            ],
            [
              "Length of stay entered",
              hasError || result.stayDays === null ? DASH : `${result.stayDays} days`,
            ],
            ["Minimum licence age", hasError ? DASH : `${result.minimumAge} years`],
            ["IDP formats accepted here", hasError ? DASH : result.acceptedFormats],
            ["Traffic drives on the", hasError ? DASH : result.drivesOn],
            [
              "Black points that cost the licence",
              hasError ? DASH : `${result.blackPointLimit} in 12 months`,
            ],
          ].map(([label, value]) => (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        recognised-country list is revised - confirm with the RTA, Abu Dhabi Police or your emirate's
        licensing authority before you drive.
      </p>
    </main>
  );
}
