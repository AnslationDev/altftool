...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    ...
    return [
      ...
      result.windowEndDate ? `Last day it is valid: ${prettyDate(result.windowEndDate)}` : null,
      `Minimum driving age here: ${result.minimumAge}`,
      `Drives on the ${result.drivesOn}`,
      `IDP formats accepted: ${result.acceptedFormats}`,
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Answer six questions and see whether France expects an International Driving Permit
          alongside your own licence, how long that licence keeps working, and what has to be in the
          car. Based on Article R222-1 of the Code de la route and the arrete du 12 janvier 2012.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="fr-origin">
          ...
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="fr-idp">
          ...
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="fr-purpose">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="fr-arrival">
              Date you arrive
            </label>
            <input
              id="fr-arrival"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="fr-departure">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="fr-age">
              Your age (years)
            </label>
            <input
              id="fr-age"
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the IDP verdict for France"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
        ...
        {hasError ? null : (
          <p className={`mt-4 rounded-md px-3 py-2 text-sm leading-6 ${toneClass}`}>{result.reason}</p>
        )}
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Length of stay entered", hasError || result.stayDays === null ? DASH : `${result.stayDays} days`],
            ["Minimum age to drive a car", hasError ? DASH : `${result.minimumAge} years`],
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        country that issued your licence and cannot be bought online from a third party. Confirm your
        own case with your prefecture, the ANTS licence service or your embassy before you drive.
      </p>
    </main>
  );
}
