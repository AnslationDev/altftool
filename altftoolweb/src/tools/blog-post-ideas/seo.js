const seo = {
  ...
  benefits: [
    ["Formats, not word salad", "Every suggestion is built on an article shape that reliably works — comparison, mistakes list, budget angle — so each idea already implies a structure you can write to."],
    ["Reshuffles for a fresh five", "Regenerating draws a new random subset from the eight templates, so you can keep pulling until an angle clicks rather than accepting the first list."],
    ["Instant and repeatable", "Ideas come from templates rather than a model call, so results appear with no wait and no dependence on an external service."],
  ],
  faqs: [
    [
