  ...
  return (
    <div className="rounded-xl border border-(--border) bg-(--background) p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <input value={sourceFps} onChange={(e) => setSourceFps(e.target.value)} type="number" step="0.001" placeholder="Source FPS" className="px-3 py-2 rounded-lg border border-(--border) bg-(--card) focus:outline-none focus:ring-2 focus:ring-(--primary)" />
        <input value={targetFps} onChange={(e) => setTargetFps(e.target.value)} type="number" step="0.001" placeholder="Target FPS" className="px-3 py-2 rounded-lg border border-(--border) bg-(--card) focus:outline-none focus:ring-2 focus:ring-(--primary)" />
        <input value={minGapMs} onChange={(e) => setMinGapMs(e.target.value)} type="number" placeholder="Minimum gap ms" className="px-3 py-2 rounded-lg border border-(--border) bg-(--card) focus:outline-none focus:ring-2 focus:ring-(--primary)" />
        <button
