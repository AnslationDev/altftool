...
import { Check, Copy, RotateCcw, Router, ShieldCheck, TriangleAlert, RefreshCw } from "lucide-react";
...
export default function ToolHome() {
  ...
  const [lastChecked, setLastChecked] = useState(today);
  const [copied, setCopied] = useState(false);

  const score = useMemo(() => scoreChecklist(done, profileId), [done, profileId]);
  const firmware = useMemo(
    () => firmwareAgeRisk({ lastCheckedISO: lastChecked, todayISO: today }),
    [lastChecked, today]
  );

  const hasScore = !score.error;
  const hasFirmware = !firmware.error;
  ...
  const summary = useMemo(() => {
    if (!hasScore) return "";
    const lines = [
      "Netgear Router Hardening Checklist",
      ...
      hasFirmware ? `Firmware last checked: ${firmware.days} days ago — ${firmware.bandLabel}` : "",
      "",
    ...
    return lines.join("\n");
  }, [score, hasScore, firmware, hasFirmware]);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          {CHECKLIST.length} steps for Nighthawk and R-series routers, weighted by how much exposure
          each one removes. Tick what is done, and the score shows what is still open with the exact
          menu path for the rest.
        </p>
      </header>
      ...
      <section className={`mt-6 ${CARD}`}>
        <h2 className="flex items-center gap-2 text-base font-semibold">
          <RefreshCw className="h-4 w-4 text-[var(--primary)]" aria-hidden="true" />
          How stale is your firmware?
        </h2>
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">
          Netgear ships security fixes only as firmware, and publishes advisories continuously. The
          working rule here is a check every {CHECK_INTERVAL_DAYS} days.
        </p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="rh-last-checked">
              Last firmware check
            </label>
            <input
              id="rh-last-checked"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="rh-today">
              Today
            </label>
            <input
              id="rh-today"
        ...
        <p className="mt-4 text-3xl font-semibold text-[var(--primary)]">
          {hasFirmware ? `${NUM.format(firmware.days)} days` : DASH}
        </p>
        <p className="mt-1 text-sm font-semibold">{hasFirmware ? firmware.bandLabel : DASH}</p>
        <dl className="mt-4 divide-y divide-[var(--border)] text-sm">
          {[
            ["Since last check", hasFirmware ? `${NUM.format(firmware.months)} months` : DASH],
            ["Quarterly checks missed", hasFirmware ? `${firmware.missedChecks}` : DASH],
            [
              "Next check due in",
              hasFirmware ? (firmware.overdue ? "Now" : `${firmware.daysUntilDue} days`) : DASH,
            ],
            ["What to do", hasFirmware ? firmware.advice : DASH],
          ].map(([label, value]) => (
            <div key={label} className="flex items-start justify-between gap-4 py-2.5">
              <dt className="text-[var(--muted-foreground)]">{label}</dt>
      ...
      <section className={`mt-6 ${CARD}`}>
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>
            Leaving the guest network&rsquo;s &ldquo;access my local network&rsquo; box ticked, which
            makes the guest SSID a second door into the same LAN.
          </li>
          <li>
            Treating NETGEAR Armor as the whole answer &mdash; a subscription filter does not close a
            WAN-facing admin page or a default password.
          </li>
          <li>
            Downloading firmware from a search result instead of the official support page for the
            exact model number printed on the label.
          </li>
          <li>
            Plugging a backup drive into the USB port and forgetting that ReadySHARE publishes it to
            the whole network by default.
          </li>
          <li>
            Enabling Remote Management for one support call and never turning it off again.
          </li>
        </ul>
      </section>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        asks for your real router password. {DEVICE.note} Menu wording changes between firmware
        releases, so follow the description rather than an exact label, and check Netgear&rsquo;s own
        support and security advisory pages for your model. This is general security guidance, not a
        substitute for professional advice on a business network.
      </p>
    </main>
  );
}
