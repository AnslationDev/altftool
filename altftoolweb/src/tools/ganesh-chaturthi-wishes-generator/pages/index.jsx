...
export default function ToolHome() {
  ...
  const result = useMemo(
    ...
    [language, audience, recipient, sender, count, seed],
  );

  const featured = result.error ? null : result.messages[0];
  const rest = result.error ? [] : result.messages.slice(1);
  const totalCharacters = result.error
    ? 0
    : result.messages.reduce((sum, item) => sum + item.characters, 0);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Pick a language and an audience, add the name you are writing to, and get ready-to-send
          greetings for the {FESTIVAL_FACTS.maxUtsavDays}-day utsav that begins on{" "}
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="gc-language">
              Language
            </label>
            <select
              id="gc-language"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="gc-audience">
              Who is it for?
            </label>
            <select
              id="gc-audience"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="gc-recipient">
              Their name (optional)
            </label>
            <input
              id="gc-recipient"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="gc-sender">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="gc-count">
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Characters in this greeting", result.error ? "—" : NUM.format(featured.characters)],
            [
              "SMS parts for this greeting",
              result.error ? "—" : `${NUM.format(featured.smsSegments)} (${featured.encoding})`,
            ],
            ["Characters across all messages", result.error ? "—" : NUM.format(totalCharacters)],
          ].map(([label, value]) => (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Ganesh Chaturthi usually falls in {FESTIVAL_FACTS.gregorianWindow}; the exact date follows
        the luni-solar calendar, so check a panchang for the year you are greeting.
      </p>
    </main>
  );
}
