...
export default function ToolHome() {
  ...
  const trip = useMemo(
    () => assessTrip({ ageYears, licenceOrigin, stayDays }),
    [ageYears, licenceOrigin, stayDays],
  );
  ...
  const summary = useMemo(() => {
    ...
    return [
      ...
      `Drink-drive threshold: ${BAC.format(trip.alcohol.limitBacPercent)}% blood / ${trip.alcohol.breathMgPerL} mg per litre of breath.`,
      `Document you need: ${trip.permit.document}.`,
      `Accepted for a visitor: ${trip.permit.accepted ? "yes" : "no"}.`,
      "",
      "Speed limits:",
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Japan drives on the left and accepts only the 1949 Geneva Convention permit — a 1968
          Vienna permit is worthless here. Check which document you actually need, the 0.03%
          alcohol threshold and the speed limits in {COUNTRY.localName} before you book a car.
        </p>
      </header>

      <section className={CARD} aria-labelledby="jp-details">
        <h2 id="jp-details" className="text-base font-semibold">
          Your details
        </h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-stay">
              Length of stay (days)
            </label>
            <input
              id="jp-stay"
      ...
      <section className={`mt-6 ${CARD}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
              Japan&apos;s drink-driving threshold
            </p>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={copyResult}
              disabled={!ok}
              aria-label="Copy the Japanese driving rules summary"
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Typical car-hire minimum age", `${COUNTRY.typicalRentalMinimumAgeYears} years, often with a one-year licence-held rule`],
            ["Legal drinking age", `${COUNTRY.drinkingAgeYears} years`],
            ["Document you must carry", ok ? trip.permit.document : DASH],
            ["Accepted for a visitor", ok ? (trip.permit.accepted ? "Yes, with your home licence and passport" : "No — see the warning below") : DASH],
            ["Valid in Japan for", ok ? (trip.permit.validityDays ? `${trip.permit.validityDays} days` : "Until its own expiry date") : DASH],
            ["Why this alcohol threshold", ok ? trip.alcohol.reason : DASH],
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="jp-liability">
        <h2 id="jp-liability" className="text-base font-semibold">
          Who is liable for drink-driving in Japan
        </h2>
        <p className="mt-1 text-sm leading-6 text-[var(--muted-foreground)]">
          The 2007 amendment to the Road Traffic Act extended criminal liability well past the
          person behind the wheel. Each of these is a separate offence.
        </p>
        <div className="mt-3 overflow-x-auto">
          <table className="w-full min-w-[340px] text-left text-sm">
            ...
            <tbody>
              {ok
                ? trip.alcohol.roles.map((role) => (
                    <tr key={role.id} className="border-b border-[var(--border)] align-top last:border-0">
                      <td className="py-2.5 pr-3 font-semibold">{role.label}</td>
                      <td className="py-2.5 text-[var(--muted-foreground)]">{role.penalty}</td>
                    </tr>
                  ))
                : null}
            </tbody>
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="jp-speeds">
        <h2 id="jp-speeds" className="text-base font-semibold">
          Speed limits
        </h2>
        <div className="mt-3 overflow-x-auto">
          <table className="w-full min-w-[340px] text-left text-sm">
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="jp-convert">
        <h2 id="jp-convert" className="text-base font-semibold">
          Convert a speed
        </h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-speed-value">
              Speed
            </label>
            <input
              id="jp-speed-value"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-speed-unit">
              Entered in
            </label>
            <select
              id="jp-speed-unit"
            ...
            >
              <option value="kmh">km/h (Japanese signs)</option>
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="jp-alcohol">
        <h2 id="jp-alcohol" className="flex items-center gap-2 text-base font-semibold">
          <Wine className="h-4 w-4 text-[var(--primary)]" aria-hidden="true" />
          Morning-after estimate
        </h2>
        <p className="mt-1 text-sm leading-6 text-[var(--muted-foreground)]">
          A Widmark-equation estimate of where your blood and breath alcohol sit. Japan&apos;s
          threshold is low enough that a single beer can put a light person over it, which is why
          the local norm is not to drink at all if you are driving.
        </p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-drinks">
              Number of drinks
            </label>
            <input
              id="jp-drinks"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-volume">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-abv">
              Strength (% ABV)
            </label>
            <input
              id="jp-abv"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-weight">
              Body weight (kg)
            </label>
            <input
              id="jp-weight"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-sex">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="jp-hours">
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            [
              ...
              bac.error ? DASH : `${NUM.format(bac.gramsAlcohol)} g (${NUM.format(bac.standardDrinks)} units)`,
            ],
            ...
            [
              "Against the Japanese threshold",
              !legal || bac.error || trip.error
                ? DASH
                : legal.alreadyUnder
                  ? "Below the threshold on this estimate — still not proof of fitness to drive"
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="jp-equipment">
        <h2 id="jp-equipment" className="text-base font-semibold">
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="jp-rules">
        <h2 id="jp-rules" className="text-base font-semibold">
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational summary only, not legal advice. Japanese traffic law and penalties change,
        and a posted sign always overrides the defaults shown here. Confirm your permit
        requirements with JAF or the Japanese embassy before you fly, because a rental desk will
        refuse the wrong document and driving on it is an unlicensed-driving offence. The alcohol
        figure is a statistical model with wide individual variation — the only safe amount before
        driving is none.
      </p>
    </main>
  );
