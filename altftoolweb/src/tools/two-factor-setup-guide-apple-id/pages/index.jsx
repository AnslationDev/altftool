...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, Apple } from "lucide-react";
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Apple ID 2FA Setup Guide</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Two-factor authentication on an Apple Account is only as strong as the device passcode that receives the code. Tick each control as you set it; the score weights recovery paths as heavily as the login itself, because Apple lockouts are permanent.
        </p>
      </header>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Creating a recovery key, storing it in a note on the same iPhone, and losing both together &mdash; that combination makes the account unrecoverable.</li>
          <li>Leaving a 4-digit passcode on the phone that displays your verification codes, which someone can shoulder-surf in a queue.</li>
          <li>Having only one trusted phone number, on the same phone that holds every trusted device.</li>
          <li>Reading a 6-digit code to a caller claiming to be Apple Support about &ldquo;suspicious iCloud activity&rdquo;.</li>
        </ul>
      </section>
