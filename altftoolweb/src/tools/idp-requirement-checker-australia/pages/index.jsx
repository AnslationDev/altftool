...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    ...
    return [
      ...
      `How long the overseas licence works: ${result.windowLabel}`,
      result.windowEndDate ? `Local licence needed by: ${prettyDate(result.windowEndDate)}` : null,
      `Traffic drives on the ${result.drivesOn}`,
      `Blood-alcohol limit: ${result.bacLimit.toFixed(2)} on a full licence, zero on a provisional`,
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Australian licensing is state law, and every state asks the same first question: is your
          licence in English? This checker answers that, then dates the deadline a permanent resident
          gets before the local licence becomes compulsory.
        </p>
      </header>
      ...
      <section className={CARD}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="au-idp">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-purpose">
              What is your status?
            </label>
            <select
              id="au-purpose"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-arrival">
              {residentMode ? "Date permanent residency started" : "Date you arrive"}
            </label>
            <input
              id="au-arrival"
              className={`mt-2 ${INPUT_CLASS}`}
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-departure">
      ...
      <section className={`mt-6 ${CARD}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy the IDP verdict for Australia"
              className={GHOST_BTN}
              disabled={hasError}
            >
              {copied ? (
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Lowest licensing age", hasError ? DASH : `${result.minimumAge} years`],
            ["IDP formats accepted here", hasError ? DASH : result.acceptedFormats],
            ["Traffic drives on the", hasError ? DASH : result.drivesOn],
            [
              "Blood-alcohol limit",
              hasError ? DASH : `${result.bacLimit.toFixed(2)} full licence, 0.00 provisional`,
            ],
          ].map(([label, value]) => (
      ...
      {hasError ? null : (
        <section className={`mt-6 ${CARD}`}>
          <h2 className="text-base font-semibold">
            How long each state gives a new permanent resident
          </h2>
          ...
          <p className="mt-3 text-xs leading-5 text-[var(--muted-foreground)]">
            The clock starts on the day permanent residency starts, not on arrival. Temporary
            visitors are not on this clock at all. Confirm with the state road authority.
          </p>
        </section>
      )}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        deadlines and accepted translators change - confirm with the road authority for the state you
        will drive in.
      </p>
    </main>
  );
