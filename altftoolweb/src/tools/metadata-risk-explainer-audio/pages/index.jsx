...
export default function ToolHome() {
  ...
  const result = useMemo(
    () => assessAudioMetadataRisk({ selectedIds: selected, channelId }),
    [selected, channelId],
  );
    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">
          What Your Audio Metadata Reveals
        </h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--muted-foreground)] sm:text-base">
          Model the hidden and audible traces in MP3, M4A voice notes and WAV files:
          recorder app, timestamps, GPS/location titles, filenames, cover art, voices and ambient sounds.
        </p>
      </header>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <div className={CARD}>
          <label>
            ...
            <select className={`${SELECT_CLASS} mt-2`} value={channelId} onChange={(event) => setChannelId(event.target.value)}>
              ...
              <fieldset key={group.name} className="rounded-lg border border-[var(--border)] p-4">
                ...
                <div className="mt-3 grid gap-3">
                  {group.items.map((signal) => (
                    <label key={signal.id} className="flex cursor-pointer gap-3 rounded-lg border border-[var(--border)] bg-[var(--background)] p-3 transition hover:border-[var(--primary)]">
                      <input
                        type="checkbox"
            ...
            <>
              ...
              <div className="mt-5 grid gap-3 text-sm">
                <div className="rounded-lg bg-[var(--background)] p-3 ring-1 ring-[var(--border)]">
                  ...
                  selected signals survive this channel.
                </div>
