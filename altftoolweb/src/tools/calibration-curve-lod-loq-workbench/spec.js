...
export const spec = {
  ...
  compute: (values) => {
      const points = String(values.points || "").split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => line.split("|").map((cell) => cell.trim())).filter((cells) => cells.length >= 2 && cells[0] !== "" && cells[1] !== "").map((cells) => cells.map(Number)).filter((row) => row.length >= 2 && row.every(Number.isFinite));
      ...
      const limitsText = slope !== 0 ? "LOD " + (Number(values.lod_factor) * sigma / Math.abs(slope)).toFixed(8) + " · LOQ " + (Number(values.loq_factor) * sigma / Math.abs(slope)).toFixed(8) : "LOD/LOQ undefined — slope is 0 (flat calibration curve has no sensitivity)";
      const interceptText = intercept < 0 ? "- " + Math.abs(intercept).toFixed(8) : "+ " + intercept.toFixed(8);
      return { result: limitsText, caption: "y = " + slope.toFixed(8) + "x " + interceptText, rows: [["Slope", slope.toFixed(8)], ["Intercept", intercept.toFixed(8)], ["Residual SD", sigma.toFixed(8)], ["R²", r2.toFixed(8)], ["Points", n]], table: { headers: ["x", "y", "Fitted y", "Residual"], rows: points.map((row, index) => [row[0], row[1], (intercept + slope * row[0]).toFixed(8), residuals[index].toFixed(8)]) } };
    },
};
