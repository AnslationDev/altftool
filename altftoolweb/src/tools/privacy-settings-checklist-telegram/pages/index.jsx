...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, Send } from "lucide-react";
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          ...
          Telegram privacy audit
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Telegram Privacy Settings Checklist</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Twenty-six Telegram controls, ranked by how much exposure each one actually closes — not by how easy it is to find. Pick the profile that matches why you are doing this, tick what you have already changed, and the score shows where you are still open.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="tg-profile">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="tg-target">
              Target score (%)
            </label>
            <input
              id="tg-target"
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Hiding your phone number but leaving &ldquo;Who can find me by my number&rdquo; on Everybody &mdash; they are two separate settings and only one of them stops a stranger opening your profile.</li>
          <li>Assuming ordinary Telegram chats are end-to-end encrypted. Cloud chats and every group chat are not; only a manually started Secret Chat is.</li>
          <li>Accepting calls with peer-to-peer left on Default, which hands your IP address to whoever is on the other end.</li>
          <li>Turning off contact sync without pressing the separate &ldquo;Delete Synced Contacts&rdquo; button, so the phone book you already uploaded stays uploaded.</li>
          <li>Relying on the SMS code alone. Without a cloud password, a SIM swap is the whole attack.</li>
          <li>Leaving old sessions alive. A terminated device stops receiving messages instantly; an untouched one keeps reading them for years.</li>
        </ul>
      </section>
