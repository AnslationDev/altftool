...
export default function PrivateRecordWorkspace({ title, description, fields = ["Title", "Notes"], exportName = "record" }) {
  ...
  useEffect(() => {
    if (!hasLoaded) return;
    if (rows.length) window.localStorage.setItem(storageKey, JSON.stringify(rows)); else window.localStorage.removeItem(storageKey);
  }, [rows, storageKey, hasLoaded]);
  ...
  const importFile = (event) => {
    ...
    reader.onload = () => { try { const imported = JSON.parse(String(reader.result)); if (Array.isArray(imported)) setRows(imported.map((row) => ({ ...row, id: row.id || crypto.randomUUID() }))); } catch { /* ignore malformed local import */ } };
