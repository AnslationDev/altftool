...
export default function PrivateRecordWorkspace({ title, description, fields = ["Title", "Notes"], exportName = "record" }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // extension, or a future schema change reusing this key). Without this
      // check `rows` would become a non-array, `filtered.filter(...)` would
      // throw on every render, and the only way out would be clearing all
      // site storage. Treat it the same as a parse failure instead.
