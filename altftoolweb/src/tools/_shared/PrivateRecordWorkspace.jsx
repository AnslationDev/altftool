...
export default function PrivateRecordWorkspace({ title, description, fields = ["Title", "Notes"], exportName = "record" }) {
  ...
  const add = () => { if (!Object.values(values).some(Boolean)) return; setLoadError(false); setRows((current) => [...current, { ...values, id: crypto.randomUUID() }]); setValues(Object.fromEntries(fields.map((field) => [field, ""]))); };
