...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    if (!ok) return "";
    const lines = [
      "Courier delivery phishing check",
    ...
    lines.push("", "If a card was used, block it on the number printed on the card, call 1930 and file at cybercrime.gov.in.");
    return lines.join("\n");
  }, [ok, result]);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Courier Delivery Phishing Anatomy
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          A fake &ldquo;your parcel is on hold, pay the Rs.25 redelivery fee&rdquo; SMS taken
          apart line by line, with a scanner that weighs the same tells against any message you
          paste and reads the real domain behind every link. Everything runs in this tab &mdash;
      ...
      <section className={CARD} aria-labelledby="pa-scan">
        ...
        <div className="mt-3">
          ...
          <p className="mt-2 text-xs leading-5 text-[var(--muted-foreground)]">
            ...
            paste your own card number, UPI PIN or OTP anywhere, including here.
          </p>
        </div>
      ...
      <section className={`mt-6 ${CARD}`}>
        ...
        <p className="mt-3 text-xs leading-5 text-[var(--muted-foreground)]">
          Addresses above are written with [.] so nothing here is clickable, and the tracking
          number is fictional. This is a composite of reported messages, not a copy of one real
          operator's SMS.
        </p>
      </section>

      <section className={`mt-6 ${CARD}`}>
        <h2 className="text-base font-semibold">What a real courier actually does</h2>
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        The scanner looks for documented markers of the fake-delivery family and weighs them. A
        ...
        Check any parcel on the carrier's own tracking page, reached by typing the address
        yourself, and treat this page as general security education rather than legal or financial
        advice.
      </p>
    </main>
  );
