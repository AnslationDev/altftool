...
import { Share2, FileDown, FileText, Link2, Copy, Check, Printer } from "lucide-react";
import { SectionCard } from "./ui";

const btn =
  "inline-flex items-center justify-center gap-2 rounded-lg border border-(--border) bg-(--card) px-3 py-2.5 text-sm font-bold text-(--foreground) transition hover:border-(--primary) hover:text-(--primary)";
...
export default function ShareResults({ summary = "" }) {
  ...
  const socials = [
    ...
    { label: "X (Twitter)", href: `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(shareUrl)}` },
  ];
    ...
    <SectionCard title="Share Your Results" icon={Share2}>
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        ...
        <button type="button" className={btn} onClick={() => copy("link", shareUrl)}>
          {copied === "link" ? <Check className="h-4 w-4 text-(--primary)" /> : <Link2 className="h-4 w-4" />} Copy Link
        </button>
        <button type="button" className={btn} onClick={() => copy("result", text)}>
          {copied === "result" ? <Check className="h-4 w-4 text-(--primary)" /> : <Copy className="h-4 w-4" />} Copy Result
        </button>
      </div>
