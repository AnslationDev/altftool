...
import FlagCard, { FlagOption } from "./FlagCard";
...
export default function QuizPlay({ questions, config, onFinish }) {
  ...
  const skip = useCallback(() => {
    ...
    setSkipped((s) => s + 1);
    setStreak(0);
    const skippedAnswer = { question: current, selected: "__skipped__", correct: false, timeTaken: 0 };
    const newAnswers = [...answers, skippedAnswer];
    setAnswers(newAnswers);
    if (idx + 1 >= total) {
      onFinish({ answers: newAnswers, questions, bestStreak });
    } else {
      setIdx((i) => i + 1);
      setSelected(null);
  ...
  const renderQuestionArea = () => {
    if (current.flagOptions) {
      return (
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm">
          <p className="mb-4 text-base font-semibold text-[var(--foreground)] leading-relaxed">
            {current.question}
          </p>
          <div className="grid grid-cols-2 gap-3">
            {current.flagOptions.map((fo) => {
              const isCorrectOpt = fo.name === current.answer;
              const isSelectedOpt = fo.name === selected;
              let cls =
              ...
              return (
                <button
                  key={fo.name}
                  className={cls}
                  onClick={() => choose(fo.name)}
                  disabled={!!selected}
                >
                  <img
                    src={fo.flag}
                    alt={`${fo.name} flag`}
                    className="h-16 w-auto rounded-lg border border-[var(--border)] object-contain"
                    loading="eager"
                    onError={(e) => { e.target.style.display = "none"; }}
                  />
                  <span
                    className={`text-xs font-semibold ${
                      selected && isCorrectOpt ? "text-emerald-700"
                      : isSelectedOpt ? "text-rose-600"
                      : "text-[var(--foreground)]"
                    }`}
                  >
                    {fo.name}
                  </span>
                  {selected && isCorrectOpt && <CheckCircle className="h-4 w-4 text-emerald-600" />}
                  {selected && isSelectedOpt && !isCorrectOpt && <XCircle className="h-4 w-4 text-rose-500" />}
                </button>
              );
            })}
          </div>
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm">
        {current.showFlag && (
          <div className="mb-4 flex justify-center">
            <FlagCard flag={current.showFlag} flagSvg={current.showFlagSvg} size="xl" country={current.country} />
          </div>
        )}
  ...
  return (
    <div className="space-y-5">
      ...
      {!current.flagOptions && current.options && (
