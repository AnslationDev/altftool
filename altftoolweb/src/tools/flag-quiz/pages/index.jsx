...
import { Flag, Zap, Globe, BarChart3, Star, Clock } from "lucide-react";
...
export default function FlagQuizPage() {
  ...
  const handleStart = useCallback(async (cfg) => {
    ...
    try {
      ...
      setError("Failed to load flag data. Please check your connection and try again.");
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <div className="mx-auto max-w-3xl">
        ...
        {screen === "home" && (
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            {[
              { icon: Globe, label: "200+ Flags" },
              { icon: Zap, label: "Real-Time Data" },
              { icon: Flag, label: "9 Quiz Modes" },
        ...
        <p className="mt-6 text-center text-xs text-[var(--muted-foreground)]">
          Flag images and country data from mledoze/countries dataset and flagcdn.com. No data is stored on servers.
        </p>
      </div>
    </div>
  );
