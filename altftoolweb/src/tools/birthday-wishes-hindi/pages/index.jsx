    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Pick the relationship and the tone. Every wording is written out separately for आप and
          तुम, so the possessive and the verb ending are right — with romanisation, an English
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="hi-relationship">
              Who is it for?
            </label>
            <select
              id="hi-relationship"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="hi-register">
              Politeness
            </label>
            <select
              id="hi-register"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="hi-tone">
              Tone
            </label>
            <select
              id="hi-tone"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="hi-count">
              How many messages
            </label>
            <select
              id="hi-count"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="hi-sender">
      ...
      <section className="mt-6 rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button
            ...
            aria-label="Copy every generated Hindi birthday message"
        ...
        {!hasError && result.registerOverridden ? (
          <p className="mt-4 rounded-md bg-[var(--muted)] px-3 py-2 text-sm text-[var(--muted-foreground)]">
            For a {result.relationship.label.toLowerCase()}, Hindi speakers would usually use the{" "}
            {result.autoRegister === "respectful" ? "आप" : "तुम"} form.
          </p>
        ) : null}
      </section>
          ...
          {messages.map((item) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <button
                  type="button"
                  onClick={() => copy(showRoman ? `${item.full}\n\n(${item.fullRoman})` : item.full, item.id)}
                  aria-label="Copy this Hindi birthday message"
                  className={GHOST_BTN}
                >
                  {copiedId === item.id ? (
                    <Check className="h-4 w-4" aria-hidden="true" />
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        ...
        and 67 per part after that — so a long Hindi wish can cost two or three SMS. WhatsApp and
