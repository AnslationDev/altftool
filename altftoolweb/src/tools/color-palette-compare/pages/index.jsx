...
import { Palette, Columns, Shuffle, RotateCcw, Copy, Check } from "lucide-react";
...
const getContrastRating = (ratio) => {
  ...
  if (ratio >= 3) return { label: "AA (Large Text)", color: "text-amber-500" };
...
export default function ColorPaletteCompare() {
  ...
  const PaletteEditor = ({ title, palette, setPalette, id }) => (
        ...
        {Object.entries(palette).map(([key, value]) => (
          <div key={`${id}-${key}`} className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-xs font-semibold uppercase text-[var(--muted-foreground)]">{key}</p>
              <div className="flex items-center gap-2">
                ...
                >
                  {copiedColor === value ? <Check className="h-3.5 w-3.5 text-green-500" /> : <Copy className="h-3.5 w-3.5" />}
                </button>
              </div>
            </div>
          </div>
