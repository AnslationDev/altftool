...
export const spec = {
  ...
  compute: (values) => {
      const ratings=new Map(String(values.ratings||"").split(/\r?\n/).map((line)=>{const [name,rating]=line.split("|").map((x)=>x.trim());const parsedRating=rating===undefined||rating===""?NaN:Number(rating);return [name,Number.isFinite(parsedRating)?parsedRating:1200];}).filter((x)=>x[0])), k=Math.max(1,Number(values.k)||32), history=[];
      String(values.matches||"").split(/\r?\n/).forEach((line,index)=>{const [a,b,scoreRaw]=line.split("|").map((x)=>x.trim());const scoreNum=scoreRaw===undefined||scoreRaw===""?NaN:Number(scoreRaw);if(!a||!b||a===b||!Number.isFinite(scoreNum))return;const score=Math.max(0,Math.min(1,scoreNum));if(!ratings.has(a))ratings.set(a,1200);if(!ratings.has(b))ratings.set(b,1200);const ra=ratings.get(a),rb=ratings.get(b),expected=1/(1+Math.pow(10,(rb-ra)/400)),newA=ra+k*(score-expected),newB=rb+k*((1-score)-(1-expected));ratings.set(a,newA);ratings.set(b,newB);history.push([index+1,a,b,score,newA.toFixed(1),newB.toFixed(1)]);});
