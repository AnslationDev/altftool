...
const BAC = new Intl.NumberFormat("en-AU", {
...
export default function ToolHome() {
  ...
  const [licenceClass, setLicenceClass] = useState(DEFAULTS.licenceClass);
  ...
  const trip = useMemo(
    () => assessTrip({ ageYears, stateId, licenceClass, licenceOrigin, stayDays }),
    [ageYears, stateId, licenceClass, licenceOrigin, stayDays],
  );
  ...
  const summary = useMemo(() => {
    if (!ok) return "";
    return [
      `Driving in ${COUNTRY.name} ${DASH} rules summary (${trip.state.label})`,
      ...
      `English translation or IDP needed: ${trip.permit.needsTranslation ? "yes" : "no"}.`,
      "",
      "Speed limits:",
  ...
  const reset = () => {
    ...
    setLicenceClass(DEFAULTS.licenceClass);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          You drive on the left in a right-hand-drive car, and every sign is in km/h. Pick the state
          or territory and see the speed limits, the alcohol limit for your licence class, and what
          paperwork your overseas licence needs.
        </p>
      </header>

      <section className={CARD} aria-labelledby="au-details">
        <h2 id="au-details" className="text-base font-semibold">
          Your details
        </h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="au-state">
              State or territory you are driving in
            </label>
            <select
              id="au-state"
          ...
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="au-class">
              Your licence class
            </label>
            <select
              id="au-class"
          ...
          <div className="sm:col-span-2">
            <label className={LABEL_CLASS} htmlFor="au-origin">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-age">
              Your age (years)
            </label>
            <input
              id="au-age"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-stay">
              Length of stay (days)
            </label>
            <input
              id="au-stay"
      ...
      <section className={`mt-6 ${CARD}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
              Your legal blood-alcohol limit in Australia
            </p>
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              {ok
                ? `${trip.alcohol.category} · ${trip.alcohol.licenceClass}`
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={copyResult}
              disabled={!ok}
              aria-label="Copy the Australian driving rules summary"
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            ["Highest limit in this state", ok ? `${trip.state.maxKmh} km/h` : DASH],
            ["Default open-road limit here", ok ? `${trip.state.defaultRuralKmh} km/h where nothing is signed` : DASH],
            ["Minimum age for a full licence", `${COUNTRY.minimumDrivingAgeYears} to 18, set by each state`],
            ...
            ["English translation or IDP", ok ? (trip.permit.needsTranslation ? "Required with your licence" : "Not needed") : DASH],
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="au-speeds">
        <h2 id="au-speeds" className="text-base font-semibold">
          Speed limits in {ok ? trip.state.label : "this state"}
        </h2>
        ...
        <p className="mt-3 text-xs leading-5 text-[var(--muted-foreground)]">
          Australian enforcement tolerance is narrow — Victoria in particular is measured in single
          km/h — and fixed, mobile, point-to-point and phone-detection cameras are all in routine
          use. A posted sign always overrides these defaults.
        </p>
      </section>

      <section className={`mt-6 ${CARD}`} aria-labelledby="au-convert">
        <h2 id="au-convert" className="text-base font-semibold">
          Convert a speed
        </h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="au-speed-value">
              Speed
            </label>
            <input
              id="au-speed-value"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-speed-unit">
              Entered in
            </label>
            <select
              id="au-speed-unit"
            ...
            >
              <option value="kmh">km/h (Australian signs)</option>
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="au-alcohol">
        <h2 id="au-alcohol" className="flex items-center gap-2 text-base font-semibold">
          <Wine className="h-4 w-4 text-[var(--primary)]" aria-hidden="true" />
          Morning-after estimate
        </h2>
        <p className="mt-1 text-sm leading-6 text-[var(--muted-foreground)]">
          A Widmark-equation estimate of where your blood alcohol sits. An Australian standard
          drink is 10 g of pure alcohol — roughly a 285 ml middy of full-strength beer. It is a
          rough model, never a green light to drive.
        </p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="au-drinks">
              Number of drinks
            </label>
            <input
              id="au-drinks"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-volume">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-abv">
              Strength (% ABV)
            </label>
            <input
              id="au-abv"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-weight">
              Body weight (kg)
            </label>
            <input
              id="au-weight"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-sex">
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="au-hours">
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="au-equipment">
        <h2 id="au-equipment" className="text-base font-semibold">
      ...
      <section className={`mt-6 ${CARD}`} aria-labelledby="au-rules">
        <h2 id="au-rules" className="text-base font-semibold">
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Informational summary only, not legal advice. Australian road rules are made by each state
        and territory and change regularly, and a posted sign always overrides the defaults shown
        here. Confirm the current rules with your rental company, your insurer and the state road
        authority before you travel. The alcohol figure is a statistical model with wide individual
        variation — the only safe amount before driving is none.
      </p>
    </main>
  );
}
