...
import { BookCheck, Check, Copy, RotateCcw } from "lucide-react";
...
const DEFAULT_SUBJECTS = ["physics", "chemistry", "maths", "english"];
const DEFAULT_DAYS_LEFT = "90";
...
export default function ToolHome() {
  ...
  const summary = useMemo(() => {
    ...
    const lines = [
      "CBSE Class 12 syllabus progress",
    ...
    overall.subjects.forEach((subject) => {
      lines.push(
        `${subject.name}: ${pct(subject.percent)} — ${subject.done}/${subject.total} chapters mastered`,
      );
    });
    if (!pace.error && !pace.finished) {
      lines.push(
        `Pace needed: ${NUM2.format(pace.perWeek)} chapters a week for the next ${daysLeft} days`,
      );
    }
    return lines.join("\n");
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Mark every chapter of your Class 12 subjects as read, revised or fully mastered, and see
          the one number that matters — how much of the syllabus is genuinely exam-ready, and how
          many chapters a week you now have to cover.
        </p>
      </header>

      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <p className={LABEL_CLASS} id="subject-picker-label">
          Subjects you are taking
        </p>
        <div className="mt-3 flex flex-wrap gap-2" role="group" aria-labelledby="subject-picker-label">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="days-left">
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              {hasResult
                ? `${overall.masteredChapters} of ${overall.totalChapters} chapters revised with PYQs solved`
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyResult}
              aria-label="Copy syllabus progress summary"
              className={GHOST_BTN}
            >
              {copied ? (
        ...
        <dl className="mt-5 divide-y divide-[var(--border)] text-sm">
          {[
            ...
            [
              "Days you can give each remaining chapter",
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                      {subject.total} chapters · {subject.theory} marks theory + {subject.practical}{" "}
                      marks {subject.practicalLabel.toLowerCase()}
                    </p>
                  </div>
                  <button
                    type="button"
                    ...
                    <ul className="mt-4 space-y-2">
                      {full.chapters.map((chapter, index) => {
                        ...
                        return (
                          ...
                          >
                            <label className="text-sm font-medium" htmlFor={`status-${key}`}>
                              {index + 1}. {chapter}
                            </label>
                            <select
                              id={`status-${key}`}
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Chapter lists follow the rationalised NCERT books CBSE currently prescribes. The board
        issues a fresh curriculum circular every session — check your school&apos;s copy for
        deletions, and remember that progress here is a planning aid, not a predicted score.
      </p>
    </main>
  );
}
