...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, UserLock } from "lucide-react";
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          <UserLock className="h-4 w-4" aria-hidden="true" />
          Microsoft account security
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Microsoft Account 2FA Guide</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          A Microsoft account usually carries your Outlook mail, OneDrive files, Xbox library and Windows sign-in at once. Tick each control as you set it and the score weights them by how much of that a takeover would reach.
        </p>
      </header>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Generating a new recovery code and forgetting that it silently replaced the one already printed and filed away.</li>
          <li>Approving a push notification without checking the number, which is exactly what a prompt-bombing attack relies on.</li>
          <li>Changing the password after a compromise but leaving the attacker&rsquo;s mail forwarding rule in place.</li>
          <li>Using an old work or school address as the alternate email, then losing access to it when you leave.</li>
        </ul>
      </section>
