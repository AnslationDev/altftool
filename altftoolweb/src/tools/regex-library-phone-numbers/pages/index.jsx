...
import { Check, Copy, Phone, RotateCcw } from "lucide-react";
...
const DEFAULTS = {
  patternId: PATTERNS[0].id, // E.164 — the storage standard
  sample: "+919876543210",
};
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
          Regex Library for Phone Numbers
        </h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Copy-ready phone patterns for E.164 storage, Indian mobiles, US/Canada NANP, UK numbers
          and a loose international catch-all — with live testing against your own numbers.
        </p>
      </header>
      ...
      <section className="rounded-xl ring-1 ring-[var(--border)] bg-[var(--card)] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="rlp-pattern">
              Region / format
            </label>
            <select
              id="rlp-pattern"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="rlp-sample">
              Test a phone number
            </label>
            <input
              id="rlp-sample"
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={copyPattern}
              aria-label="Copy the selected phone number regex pattern"
              className={GHOST_BTN}
            >
              {copied ? (
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Regex checks format, not reachability or assignment. Store numbers in E.164, validate
        country rules with a numbering-plan library where correctness matters, and remember that
        national numbering plans change over time.
      </p>
    </main>
  );
}
