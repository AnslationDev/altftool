/**
 * Turkish Airlines (TK) baggage allowance checker.
 *
 * Turkish Airlines uses a weight concept across the bulk of its network — one
 * allowance in kilograms covering every checked piece together — and switches to
 * IATA piece rules on journeys to and from the Americas, where the allowance
 * becomes a counted number of bags with a ceiling on each.
 *
 *   • Weight concept: 15 kg on domestic Türkiye sectors, commonly 20 kg, 25 kg or
 *     30 kg on international Economy depending on the route and fare family, and
 *     30 kg or 40 kg in Business.
 *   • Piece concept, Americas: 2 × 23 kg in Economy, 2 × 32 kg in Business.
 *
 * Cabin baggage is 8 kg and 55 × 40 × 23 cm in Economy — a kilogram more generous
 * than the 7 kg most carriers allow — plus a small accessory bag such as a handbag
 * or laptop bag of up to 40 × 30 × 15 cm. Business carries two cabin pieces of
 * 8 kg each rather than one.
 *
 * These are the published standard allowances. Miles&Smiles elite tiers, Star
 * Alliance status, codeshare sectors, infants and sports equipment follow separate
 * rules, so confirm what is printed on your own booking.
 *
...
export const AIRLINE = { name: "Turkish Airlines", code: "TK", country: "Türkiye" };

/** Economy cabin bag: one piece, 8 kg, 55 × 40 × 23 cm. */
export const CABIN_BAG_KG = 8;
export const CABIN_DIM_RULE = { type: "each", maxEachCm: [55, 40, 23] };

/** Accessory bag — a handbag or laptop bag that fits under the seat in front. */
export const PERSONAL_DIM_RULE = { type: "each", maxEachCm: [40, 30, 15] };

/** Business carries two cabin pieces, each on the same 8 kg and size rule. */
...
/**
 * No default excess rate ships with this tool. Turkish prices excess per kilogram
 * by route band and sells prepaid weight more cheaply than the airport desk, so a
 * stored average would mislead. Enter the rate quoted for your own booking.
 */
...
const BUSINESS_CABIN = {
  pieces: PREMIUM_CABIN_PIECES,
  kg: CABIN_BAG_KG,
  combinedKg: null,
  dimRule: CABIN_DIM_RULE,
};
/** No published weight for the accessory bag — it is judged on size and fit. */
const ACCESSORY_BAG = { kg: null, dimRule: PERSONAL_DIM_RULE };
/** The second Business cabin piece is measured on the cabin bag rule. */
...
export const FARE_OPTIONS = [
  {
    key: "eco-30",
    label: "International Economy — 30 kg",
    cabin: ECONOMY_CABIN,
    personal: ACCESSORY_BAG,
    checked: { type: "weight", allowanceKg: 30 },
  },
  {
    key: "eco-25",
    label: "International Economy — 25 kg",
    cabin: ECONOMY_CABIN,
    personal: ACCESSORY_BAG,
  ...
  {
    key: "eco-20",
    label: "International Economy — 20 kg",
    cabin: ECONOMY_CABIN,
    personal: ACCESSORY_BAG,
  ...
  {
    key: "dom-15",
    label: "Domestic Türkiye Economy — 15 kg",
    cabin: ECONOMY_CABIN,
    personal: ACCESSORY_BAG,
  ...
  {
    key: "biz-30",
    label: "Business — 30 kg",
    cabin: BUSINESS_CABIN,
    personal: SECOND_CABIN_PIECE,
    checked: { type: "weight", allowanceKg: 30 },
  },
  {
    key: "biz-40",
