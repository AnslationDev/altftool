...
import { Check, Copy, RotateCcw, ShieldCheck, TriangleAlert, Wallet } from "lucide-react";
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">
          ...
          Wallet and UPI security
        </div>
        <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">Paytm Security Setup Guide</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Paytm bundles a wallet, UPI, saved cards and often a Postpaid credit line behind one login. Tick each control as you set it; the score weights each by how much money it keeps out of someone else's reach.
        </p>
      </header>
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--muted-foreground)]">
          <li>Leaving auto add money on, which refills the wallet for whoever is spending from it.</li>
          <li>Ignoring a Paytm Postpaid balance because it does not touch the bank account until the due date.</li>
          <li>Reading out an OTP to someone who called claiming a wallet KYC or account-block problem.</li>
          <li>Assuming a small unexplained debit is a glitch &mdash; test debits usually come before the large one.</li>
        </ul>
      </section>
