...
import {
  ...
  RELATIONSHIPS,
  TONES,
  generateMalayalamBirthdayWishes,
  nextSeed,
} from "../lib";
...
export default function ToolHome() {
  ...
  const [showEnglish, setShowEnglish] = useState(DEFAULTS.showEnglish);
  ...
  const blockFor = (item) => {
    ...
    if (showRoman) parts.push(item.fullRoman);
    if (showEnglish) parts.push(item.fullEnglish);
    return parts.join("\n\n");
  };
  ...
  const allText = useMemo(() => {
    ...
    return messages
      ...
      .join("\n\n———\n\n");
  }, [hasError, messages, showRoman, showEnglish]);
  ...
  const reset = () => {
    ...
    setShowEnglish(DEFAULTS.showEnglish);
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 text-[var(--foreground)] sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
          Pick who it is for and the tone you want. Each wording is written out separately for നീ and
          നിങ്ങൾ so the possessive is right, താങ്കൾ is there for formal messages, and every message
          comes with a romanisation, an English meaning and its SMS length.
        </p>
      </header>
      ...
      <section className="rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={LABEL_CLASS} htmlFor="ml-name">
              Their name (optional)
            </label>
            <input
              id="ml-name"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ml-relationship">
              Who is it for?
            </label>
            <select
              id="ml-relationship"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ml-register">
              Politeness
            </label>
            <select
              id="ml-register"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ml-tone">
              Tone
            </label>
            <select
              id="ml-tone"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ml-count">
              How many messages
            </label>
            <select
              id="ml-count"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="ml-sender">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <label className={CHECK_LABEL} htmlFor="ml-roman">
            <input
              id="ml-roman"
              type="checkbox"
              checked={showRoman}
          ...
          <label className={CHECK_LABEL} htmlFor="ml-english">
            <input
              id="ml-english"
              type="checkbox"
              checked={showEnglish}
      ...
      <section className="mt-6 rounded-xl bg-[var(--card)] p-5 ring-1 ring-[var(--border)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button
            ...
            aria-label="Copy every generated Malayalam birthday message"
        ...
        {!hasError ? (
          <p className="mt-4 rounded-md bg-[var(--muted)] px-3 py-2 text-sm text-[var(--muted-foreground)]">
            {result.registerOverridden
              ? `For a ${result.relationship.label.toLowerCase()}, Malayalam speakers would normally use ${
                  REGISTERS.find((item) => item.id === result.autoRegister)?.pronoun
                }. ${result.registerInfo.note}`
              : result.registerInfo.note}
          </p>
        ) : null}
      </section>
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="rounded-full bg-[var(--muted)] px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
                  ...
                  {item.tailored
                    ? ` · written for a ${result.relationship.label.toLowerCase()}`
                    : ""}
                </span>
                <button
                  type="button"
                  onClick={() => copy(blockFor(item), item.id)}
                  aria-label="Copy this Malayalam birthday message"
                  className={GHOST_BTN}
                >
                  {copiedId === item.id ? (
                    <Check className="h-4 w-4" aria-hidden="true" />
      ...
      <p className="mt-6 text-xs leading-5 text-[var(--muted-foreground)]">
        Malayalam is sent over SMS as UCS-2, which allows 70 characters in a single message and 67
        per part after that, so a long wish can cost two or three SMS. WhatsApp and other data
        messaging apps have no such limit.
      </p>
    </main>
  );
}
