...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="pt-4 pb-12 lg:pt-6 lg:pb-16 bg-white relative overflow-hidden">
      <div className="absolute top-20 left-10 w-72 h-72 bg-[#00AEEF]/10 rounded-full blur-3xl" />
      <div className="absolute bottom-20 right-10 w-80 h-80 bg-[#0D3B66]/10 rounded-full blur-3xl" />
      ...
      <div className="relative max-w-6xl mx-auto px-5 lg:px-8">
        ...
        <div
          ...
          className="relative bg-gradient-to-br from-[#F8FAFC] to-white rounded-3xl border border-slate-100 shadow-premium p-8 lg:p-14"
        >
          <Quote className="absolute top-6 right-6 lg:top-10 lg:right-10 w-16 h-16 text-[#00AEEF]/15" />
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <blockquote className="font-display text-xl lg:text-2xl text-[#0D3B66] leading-relaxed font-medium">
                "{cur.review}"
              </blockquote>
          ...
          <div className="mt-8 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={() => setIdx((i) => (i - 1 + items.length) % items.length)}
                className="w-11 h-11 rounded-full bg-white border border-slate-200 hover:border-[#00AEEF] text-[#0D3B66] flex items-center justify-center transition">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={() => setIdx((i) => (i + 1) % items.length)}
                className="w-11 h-11 rounded-full bg-gradient-to-r from-[#0D3B66] to-[#1E5AA8] text-white flex items-center justify-center hover:scale-105 transition">
