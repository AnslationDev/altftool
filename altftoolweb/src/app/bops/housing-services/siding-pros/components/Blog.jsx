...
export default function Blog() {
  return (
    <section id="blog" data-siding-blog className="pt-4 pb-12 lg:pt-6 lg:pb-16 bg-white relative overflow-hidden">
      <div className="absolute top-20 right-0 w-[36rem] h-[36rem] bg-[#00AEEF]/5 rounded-full blur-3xl" />
      <div className="absolute bottom-20 left-0 w-[30rem] h-[30rem] bg-[#0D3B66]/5 rounded-full blur-3xl" />
      ...
      <div className="relative max-w-7xl mx-auto px-5 lg:px-8">
        ...
        <motion.article
          ...
          className="group grid lg:grid-cols-2 gap-0 rounded-3xl overflow-hidden bg-white border border-slate-100 shadow-premium mb-14 lg:mb-16"
        >
          <a href={`#blog/${featuredPost.slug}`} className="relative h-72 lg:h-auto overflow-hidden block">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[#0D3B66]/60 via-transparent to-transparent lg:bg-gradient-to-r" />
            <div className="absolute top-5 left-5 flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-[#0D3B66] to-[#00AEEF] text-white text-[10px] font-bold tracking-wider uppercase shadow-lg">
          ...
          <div className="p-8 lg:p-12 flex flex-col justify-center">
            ...
            <a href={`#blog/${featuredPost.slug}`} className="block">
              <h3 className="font-display font-extrabold text-2xl lg:text-3xl text-[#0D3B66] leading-tight group-hover:text-[#1E5AA8] transition-colors">
                {featuredPost.title}
              </h3>
            </a>
            <p className="mt-4 text-slate-600 leading-relaxed">{featuredPost.excerpt}</p>
            <a href={`#blog/${featuredPost.slug}`} className="mt-7 inline-flex items-center gap-2 self-start px-6 py-3 rounded-full bg-gradient-to-r from-[#0D3B66] to-[#1E5AA8] text-white font-semibold shadow-premium hover:shadow-glow hover:scale-[1.03] transition-all">
              Read Full Article
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {regularPosts.map((p, i) => (
            <motion.article
              ...
              whileHover={{ y: -6 }}
              className="group rounded-2xl overflow-hidden bg-white border border-slate-100 shadow-[0_4px_20px_rgba(13,59,102,0.06)] hover:shadow-premium transition-all flex flex-col"
            >
              <a href={`#blog/${p.slug}`} className="relative h-52 overflow-hidden block">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#0D3B66]/40 via-transparent to-transparent" />
              ...
              <div className="p-6 flex flex-col flex-1">
                ...
                <a href={`#blog/${p.slug}`} className="block">
                  <h3 className="font-display font-bold text-lg text-[#0D3B66] leading-snug group-hover:text-[#1E5AA8] transition-colors">
                    {p.title}
                  </h3>
                </a>
                <p className="mt-3 text-sm text-slate-600 leading-relaxed flex-1">{p.excerpt}</p>
                <div className="mt-5 pt-5 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-500 flex items-center gap-1.5"><User className="w-3.5 h-3.5" /> {p.author}</span>
                  <a href={`#blog/${p.slug}`} className="inline-flex items-center gap-1 text-sm font-bold text-[#00AEEF] group/btn">
                    Read
