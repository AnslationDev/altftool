...
export default function Projects() {
  ...
  return (
    <section id="projects" className="pb-6 pt-4 lg:pb-8 lg:pt-6 bg-white">
          ...
          {filters.map((f) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all ${
                filter === f
                  ? "bg-gradient-to-r from-[#0D3B66] to-[#1E5AA8] text-white shadow-premium"
          ...
          <AnimatePresence>
            {filtered.map((p) => (
              <motion.article
                ...
                className="group relative rounded-2xl overflow-hidden cursor-pointer aspect-[4/3] shadow-[0_4px_20px_rgba(13,59,102,0.08)] hover:shadow-premium"
              >
                <img
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0D3B66]/95 via-[#0D3B66]/30 to-transparent opacity-80 group-hover:opacity-100 transition-opacity" />
                <div className="absolute top-4 left-4">
                  <span className="px-3 py-1 rounded-full bg-white/90 text-[10px] font-bold tracking-wider uppercase text-[#0D3B66]">
                    {p.category}
                  </span>
                </div>
                <div className="absolute top-4 right-4 w-9 h-9 rounded-full glass flex items-center justify-center text-[#0D3B66] opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <AnimatePresence>
        {lightbox && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-[#0D3B66]/90 backdrop-blur-md flex items-center justify-center p-5"
          >
            <motion.div
            ...
            >
              <button
                onClick={() => setLightbox(null)}
                className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-white/90 flex items-center justify-center text-[#0D3B66] hover:bg-white"
              >
                <X className="w-5 h-5" />
              </button>
              <img
              ...
              <div className="p-6 lg:p-8">
                <div className="text-xs font-bold tracking-wider uppercase text-[#00AEEF]">{lightbox.category}</div>
                <h3 className="mt-1 font-display font-extrabold text-2xl text-[#0D3B66]">{lightbox.title}</h3>
