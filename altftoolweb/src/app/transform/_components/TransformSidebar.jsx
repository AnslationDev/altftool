...
export default function TransformSidebar({ activeSlug }) {
  ...
  return (
    <aside className="flex h-full w-full flex-col rounded-2xl border border-border bg-surface p-4 shadow-sm">
      {/* Search Header */}
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full rounded-xl border border-border bg-surface-soft py-2 pl-9 pr-4 text-xs font-semibold text-foreground outline-none transition placeholder:text-muted-foreground focus:border-primary focus:bg-surface focus:ring-2 focus:ring-primary/25"
        />
      </div>
                    ...
                    <li key={tool.slug}>
                      <Link
                        ...
                        className={`flex items-center justify-between rounded-lg px-2.5 py-1.5 text-xs font-medium transition-all ${
                          isActive
                            ? "bg-primary-soft text-primary-text border-l-2 border-primary font-semibold"
