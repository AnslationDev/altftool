...
export default function TransformShell({ tool }) {
  ...
  // The dot carries the status colour — it is a graphical object, so the 3:1
  // bar applies and --danger/--success clear it. The Done/Error labels fall
  // back to --foreground: those same tokens are only 3.8:1 and 3.3:1 on the
  // light surface, which fails AA for 12px copy.
  const statusPill = useMemo(() => {
    if (status === "loading") return { dot: "bg-(--primary) animate-pulse", label: "Converting…", tone: "text-(--primary-text)" };
    if (status === "error") return { dot: "bg-(--danger)", label: "Error", tone: "text-(--foreground)" };
    if (status === "ok") return { dot: "bg-(--success)", label: "Done", tone: "text-(--foreground)" };
    return { dot: "bg-(--muted-foreground)", label: "Ready", tone: "text-(--muted-foreground)" };
  }, [status]);

  const engineBadge =
    tool.engine === "browser"
      ? { icon: Cpu, label: "Runs in your browser", tone: "text-(--foreground)", iconTone: "text-(--success)" }
      : { icon: Server, label: "Runs on server", tone: "text-(--foreground)", iconTone: "text-(--primary)" };
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-wide text-(--muted-foreground)">
            <span className="rounded-md bg-(--muted) px-2 py-0.5 text-(--foreground)">{labelFor(tool.from)}</span>
            <ArrowRight className="h-3.5 w-3.5" />
            <span className="rounded-md bg-(--muted) px-2 py-0.5 text-(--primary-text)">{labelFor(tool.to)}</span>
          </div>
        ...
        <div className={`flex shrink-0 items-center gap-2 rounded-full border border-(--border) bg-(--card) px-3 py-1.5 text-xs font-semibold shadow-sm ${engineBadge.tone}`}>
          <EngineIcon className={`h-4 w-4 ${engineBadge.iconTone}`} />
          {engineBadge.label}
        </div>
      </div>
            ...
            <div className="flex items-center gap-1">
              ...
              {optionsSchema.length > 0 && (
                <GhostButton
                  ...
                  className={showSettings ? "bg-(--muted) text-(--primary-text)" : ""}
                />
              )}
      ...
      {/* error + warnings */}
      {error ? (
        <div className="mt-4 flex items-start gap-2 rounded-xl border border-(--danger)/30 bg-(--danger-soft) px-4 py-3 text-sm text-(--foreground)">
          <TriangleAlert className="mt-0.5 h-4 w-4 shrink-0 text-(--danger)" />
          <span className="whitespace-pre-wrap">{error}</span>
        </div>
      ) : null}
      {warnings.length > 0 ? (
        <div className="mt-3 rounded-xl border border-(--warning)/30 bg-(--warning-soft) px-4 py-3 text-sm text-(--foreground)">
          {warnings.map((w, i) => (
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-4">
        <StatTile icon={FileText} tint="bg-(--primary)/10" iconTone="text-(--primary)" label="Input" value={`${input.length.toLocaleString()} chars`} />
        <StatTile icon={FileText} tint="bg-(--success)/10" iconTone="text-(--success)" label="Output" value={`${output.length.toLocaleString()} chars`} />
        <StatTile icon={ShieldCheck} tint="bg-(--accent)/10" iconTone="text-(--accent)" label="Status" value={statusPill.label} />
        <StatTile icon={Timer} tint="bg-(--warning)/10" iconTone="text-(--warning)" label="Time" value={`${elapsed} ms`} />
      </div>

      {/* privacy note */}
      <p className="mt-6 flex items-center gap-2 rounded-xl bg-(--muted) px-4 py-3 text-xs leading-relaxed text-(--muted-foreground)">
