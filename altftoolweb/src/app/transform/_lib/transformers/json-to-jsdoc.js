...
function build(name, obj, defs) {
  const props = Object.keys(obj).map((k) => ` * @property {${typeFor(k, obj[k], defs)}} ${k}`);
  defs.set(name, `/**\n * @typedef {Object} ${name}\n${props.join("\n")}\n */`);
}
...
export function transform(input) {
  ...
  if (!root || typeof root !== "object" || Array.isArray(root)) {
    return err("JSDoc typedefs are generated from a JSON object (or an array of objects).");
  }
  try {
    const defs = new Map();
