  ...
  return (
    <main className="page">
      ...
      {a === b ? (
        <section className="wrap">
          <div className="cmp-error-card">
            <GitCompareArrows size={36} className="cmp-error-icon" />
            <h3>Choose different rankings</h3>
            <p>You are comparing the same list in both columns. Select a different ranking in either dropdown to see a head-to-head comparison.</p>
          </div>
        </section>
      ) : (
      ...
      <section className="wrap pad">
        ...
        <div className="rowlist">{[ra, rb].map((r, i) => <RowCard key={`${r.slug}-${i}`} r={r} i={i} />)}</div>
      </section>
      <Newsletter />
    </main>
  );
