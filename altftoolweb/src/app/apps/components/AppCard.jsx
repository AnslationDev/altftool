...
export default function AppCard({ app, compact = false, descriptionFullWidth = false }) {
  const showDownload = !compact && Boolean(app.apkUrl);

  return (
    <article className={`flex flex-col rounded-lg border border-[var(--home-border)] bg-[var(--card)] shadow-[var(--anslation-ds-shadow-sm)] transition hover:-translate-y-1 hover:shadow-[var(--anslation-ds-shadow-md)] ${compact ? "min-h-[270px] p-5" : "min-h-[244px] p-4"}`}>
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1.5 text-[13px] font-medium text-[var(--primary-text)]">{app.category}</p>
      ...
      <div className={`mt-auto flex items-center gap-3 text-sm ${compact ? "pt-6" : "pt-4"}`}>
        <span className="text-xs font-semibold text-[var(--muted-foreground)]">{app.apkSize}</span>
      </div>

      <div className={`${compact ? "mt-5" : "mt-4"} ${showDownload ? "grid-cols-2" : "grid-cols-1"} grid gap-3`}>
        <Link
          href={`/apps/${app.slug}`}
        ...
        {showDownload ? <DownloadButton href={app.apkUrl} label="Download" className="min-h-11 px-4 py-2 text-sm" /> : null}
      </div>
    </article>
  );
}
