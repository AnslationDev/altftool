...
const CATEGORY_STYLES = {
  Extensions:  "bg-(--anslation-ds-primary-soft) text-(--primary) border-(--border)",
  Design:      "bg-(--anslation-ds-primary-soft) text-(--primary) border-(--border)",
  Performance: "bg-(--anslation-ds-primary-soft) text-(--primary) border-(--border)",
  Tutorials:   "bg-(--anslation-ds-primary-soft) text-(--primary) border-(--border)",
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col overflow-hidden rounded-2xl bg-(--card) border border-(--border)
                 shadow-sm hover:shadow-xl hover:shadow-(--anslation-ds-primary-soft)/60 hover:-translate-y-1
                 transition-all duration-300 h-full"
      style={{ animationDelay: `${index * 60}ms` }}
    >
      {/* Image */}
      <div className="relative overflow-hidden h-44 bg-gradient-to-br from-(--anslation-ds-primary-soft) to-(--muted) flex-shrink-0">
        {!imgLoaded && (
          <div className="absolute inset-0 animate-pulse bg-gradient-to-r from-(--muted) via-(--anslation-ds-primary-soft) to-(--muted) bg-[length:200%_100%]" />
        )}
        <ManagedImage
          src={post.image}
        ...
        {index < 3 && (
          <div className="absolute top-3 left-3 flex items-center gap-1 bg-(--primary) text-(--primary-foreground)
        ...
        <div className="absolute bottom-3 right-3 flex items-center gap-1.5 bg-white/90 backdrop-blur-sm
                        text-(--foreground) text-[10px] font-semibold px-2 py-0.5 rounded-full shadow-sm
      ...
      <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-(--primary) to-(--anslation-ds-accent)
      ...
      <div className="flex flex-col items-center gap-2 mb-8 text-center">
        <div className="flex items-center w-full gap-4">
          ...
          <div className="flex flex-col items-center gap-1.5">
            {/* Eyebrow */}
            <div className="flex items-center gap-2 bg-(--anslation-ds-primary-soft) border border-(--border) text-(--primary)
