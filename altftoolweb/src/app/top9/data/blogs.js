export const blogs = [
  {
    title: "Affordable SEO Tools in 2026",

    slug: "affordable-seo-tools-2026",

    img: "https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=1200&auto=format&fit=crop",

    desc: "In 2026, SEO is no longer optional for businesses and creators looking to grow online visibility and organic traffic. Affordable SEO tools now provide powerful keyword research, backlink tracking, rank monitoring, content optimization, competitor analysis, and AI-powered insights without requiring expensive enterprise subscriptions. These tools help bloggers, startups, marketers, and agencies improve search rankings efficiently while staying within budget. They also simplify technical SEO tasks and help websites stay competitive in constantly changing search engine algorithms.",

    cat: "Digital Tools",

    date: "May 11, 2026",
  },

  {
    title: "Best Call of Duty Games",

    slug: "best-call-of-duty-games",

    img: "https://images.unsplash.com/photo-1542751371-adc38448a05e?q=80&w=1200&auto=format&fit=crop",

    desc: "The Call of Duty franchise has delivered some of the most iconic first-person shooter experiences in gaming history. From intense modern warfare campaigns to fast-paced multiplayer battles and unforgettable zombie modes, these games shaped competitive online gaming for millions of players worldwide. Fans continue to debate which entries offered the best storytelling, maps, mechanics, graphics, and replay value across different gaming generations. Each title introduced unique innovations that kept the franchise fresh and highly popular among gamers.",

    cat: "Gaming",

    date: "May 11, 2026",
  },

  {
    title: "Best Dog Breeds for First-Time Owners in 2026",

    slug: "top-9-dog-breeds",

    img: "https://images.unsplash.com/photo-1516734212186-a967f81ad0d7",

    desc: "Choosing the right dog breed is important for first-time pet owners who want a friendly, easy-to-train, and low-maintenance companion. Some breeds are naturally more adaptable, affectionate, and suitable for apartment living or family environments. This guide highlights the best dog breeds for beginners based on temperament, grooming needs, intelligence, exercise requirements, and compatibility with children and other pets. These breeds are also known for building strong bonds with owners and adjusting well to different lifestyles.",

    cat: "Pets",

    date: "May 11, 2026",
  },

  {
    title: "Scariest Movies That Will Keep You Awake",

    slug: "scariest-movies",

    img: "https://images.unsplash.com/photo-1509248961158-e54f6934749c",

    desc: "Horror movies have evolved into terrifying cinematic experiences filled with psychological fear, supernatural creatures, disturbing visuals, and suspenseful storytelling. Some films are so intense and unsettling that they stay in viewers’ minds long after the credits roll. From classic horror masterpieces to modern psychological thrillers, these movies are known for delivering unforgettable scares and sleepless nights. Their chilling sound design and unpredictable scenes make them favorites among horror fans worldwide.",

    cat: "Movies",

    date: "May 11, 2026",
  },

  {
    title: "Best Clothing Brands",

    slug: "best-clothing-brands",

    img: "https://images.unsplash.com/photo-1441986300917-64674bd600d8",

    desc: "Fashion trends continue to evolve in 2026, and the world’s best clothing brands are combining style, comfort, sustainability, and innovation like never before. From luxury labels and streetwear giants to affordable everyday fashion companies, these brands influence global fashion culture and consumer preferences. They are recognized for quality fabrics, modern designs, brand reputation, and strong customer loyalty worldwide. Many brands are also focusing heavily on eco-friendly production and sustainable fashion practices. Their collections continue to inspire millions of fashion enthusiasts across different age groups and lifestyles. Fashion brands today are also expanding into smart fabrics, gender-neutral collections, and influencer-driven collaborations that dominate social media trends worldwide. With global online shopping growth increasing rapidly, these companies continue to shape the future of fashion through creativity, premium craftsmanship, and digital-first experiences for modern consumers.",

    cat: "Fashion",

    date: "May 11, 2026",
  },

  {
    title: "Best Workout Songs",

    slug: "best-workout-songs",

    img: "https://images.unsplash.com/photo-1534438327276-14e5300c3a48",

    desc: "Music plays a huge role in boosting workout motivation, focus, and energy during gym sessions, running, cardio, and strength training. The best workout songs feature powerful beats, motivating lyrics, and high-energy rhythms that help maintain momentum and improve performance. From hip-hop and EDM to rock and pop anthems, these tracks create the perfect fitness playlist for intense training sessions. A strong workout playlist can improve consistency, mood, and overall exercise performance for fitness enthusiasts. High-tempo tracks also help people stay mentally engaged and push harder during difficult workouts. Many popular workout songs are designed with energetic drops, fast BPMs, and uplifting hooks that naturally increase endurance and excitement during exercise routines. Whether someone is lifting weights, training for a marathon, or simply staying active at home, the right music playlist can completely transform the workout experience and keep motivation levels consistently high.",

    cat: "Fitness",

    date: "May 11, 2026",
  },
];
