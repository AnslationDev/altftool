...
const faqs = [
  ["How does Top9 choose what makes each list?", "Every list starts with independent research, hands-on testing where possible, and verified reader feedback. We never accept payment for placement the nine picks are the nine we'd actually recommend."],
  ...
  return <main className="top9-page t9-home">
    <section className="t9-hero">
      <div className="t9-container t9-hero-copy">
        <div className="t9-hero-badge"><i />150+ categories updated daily</div>
      ...
      <div className="t9-container t9-hero-grid">
        {heroItems.map((item, index) => <Link href={`/top9/${item.slug}`} key={item.slug} className={`t9-hero-story t9-hero-story-${index + 1}`}>
          <img src={item.img} alt={item.title} /><b>#{index + 1}</b><span className="t9-story-overlay"><small>{index === 0 ? "Editor's pick Technology" : index === 1 ? "Food" : "Travel"}</small><strong>{index === 0 ? "Top 9 AI Tools Actually Worth Paying For in 2026" : `Top 9 ${item.title}`}</strong>{index === 0 && <em>12 min read 40K reads this week</em>}</span>
        </Link>)}
        <Link className="t9-trending-float" href="/top9"><Flame size={15} /> Trending now</Link>
      </div>
      <div className="t9-container"><div className="t9-statbar">{[["227K+", "Top 9 lists"], ["150+", "Categories"], ["Daily", "Fresh rankings"], ["3.4M", "Readers worldwide"]].map(([number, text]) => <div key={text}><strong>{number}</strong><span>{text}</span></div>)}</div></div>
    </section>
    ...
    <section className="t9-container t9-trending-section"><SectionHeading eyebrow="Right now" title={<><Flame /> Trending Today</>} /><div className="t9-trending-viewport" ref={trendViewportRef}><div className="t9-trending-track" ref={trendTrackRef} style={{ transform: `translateX(${trackTranslateX}px)` }}>{allItems.slice(0, 5).map((item, index) => <Link key={`${item.slug}-${index}`} href={`/top9/${item.slug}`} ref={(el) => { trendCardRefs.current[index] = el; }} className={`t9-trend-card ${index === selectedTrend ? "is-selected" : ""}`} onClick={(event) => handleTrendClick(event, index)}><img src={item.img} alt={item.title} /><b>0{index + 1}</b><div><small>{["AI Tools", "Netflix", "Travel", "Technology", "Culture"][index]}</small><strong>{index === 0 ? "The 9 AI Tools Productivity Teams Actually Rely On" : `Top 9 ${item.title}`}</strong><p>{(2.3 - index * 0.2).toFixed(1)}K views <span /> {5 + index} min read</p></div></Link>)}</div></div></section>
    ...
    <section className="t9-container t9-updated"><SectionHeading eyebrow="Fresh off the desk" title="Recently Updated" /><div className="t9-timeline">{allItems.slice(4, 8).map((item, index) => <Link href={`/top9/${item.slug}`} key={item.slug}><i>{2 + index * 2}h</i><img src={item.img} alt={item.title} /><div><small>{["Travel", "Gaming", "Food", "Fitness"][index]}</small><strong>{`Top 9 ${item.title}`}</strong></div><em>{842 + index * 90} views</em></Link>)}</div></section>

    <section className="t9-featured-section"><div className="t9-container"><SectionHeading eyebrow="The long read" title="Featured Rankings" /><div className="t9-featured-grid"><Link href={`/top9/${allItems[1].slug}`} className="t9-feature-main"><img src={allItems[1].img} alt="Featured ranking" /><div><small>Editor&apos;s feature</small><strong>Top 9 Documentaries That Will Change the Way You See the World</strong><p>Two months of watching, comparing and debating this is the shortlist our editors care about the most.</p><em>4.9 rating 10.2K views</em></div></Link><div className="t9-feature-list">{allItems.slice(2, 6).map((item, index) => <Link href={`/top9/${item.slug}`} key={item.slug}><b>0{index + 1}</b><img src={item.img} alt="" /><div><small>{["Movies", "Sports", "Travel", "Technology"][index]}</small><strong>{`Top 9 ${item.title}`}</strong><p>{(4.8 - index / 10).toFixed(1)} rating {25 - index * 3}K views 5 min read</p></div></Link>)}</div></div></div></section>
