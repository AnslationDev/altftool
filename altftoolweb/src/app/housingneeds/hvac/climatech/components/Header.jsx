...
export default function Header() {
  ...
  const isHome = pathname === "/housingneeds/hvac/climatech";
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <Link
            href="/housingneeds/hvac/climatech"
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {["Services", "About", "Portfolio", "Testimonials", "Contact"].map(
              (item) => (
                <Link
                  key={item}
                  href={isHome ? `#${item.toLowerCase()}` : `/housingneeds/hvac/climatech#${item.toLowerCase()}`}
