    ...
    <section id="faq" data-siding-faq className="pt-12 pb-4 lg:pt-16 lg:pb-6 bg-gradient-soft">
      <div className="max-w-4xl mx-auto px-5 lg:px-8">
        ...
        >
          ...
          <p className="mt-5 text-lg text-foreground/75">Everything you need to know before booking your free estimate.</p>
        </motion.div>
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl border transition-all overflow-hidden ${
                  isOpen ? "bg-surface border-secondary/40 shadow-premium" : "bg-surface border-border hover:border-border"
                }`}
              >
                <button
                ...
                >
                  <span className="font-display font-bold text-primary text-base lg:text-lg">{f.q}</span>
                  <span className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center transition-all ${isOpen ? "bg-secondary text-primary-foreground rotate-45" : "bg-surface-soft text-primary"}`}>
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="px-6 lg:px-8 pb-6 text-foreground/75 leading-relaxed">{f.a}</div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
