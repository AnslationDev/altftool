...
const ratingColor = (v) => {
  ...
  if (/Good|Very Good|Versatile|Low/i.test(v)) return "text-primary-hover bg-primary-hover/10";
  if (/Moderate/i.test(v)) return "text-amber-600 bg-amber-50";
  if (/High/i.test(v)) return "text-rose-600 bg-rose-50";
  return "text-foreground/75 bg-surface-soft";
};
    ...
    <section id="comparison" className="py-24 lg:py-32 bg-gradient-soft">
      <div className="max-w-7xl mx-auto px-5 lg:px-8">
          ...
          {tabs.map((t, i) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition ${
                active === i ? "bg-primary text-primary-foreground" : "bg-surface text-foreground/75 border border-border"
              }`}
            >
              {t}
            </button>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="rounded-3xl bg-surface shadow-premium border border-border overflow-hidden"
        >
          {/* Desktop */}
          <div className="hidden lg:block">
            <table className="w-full">
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.attribute} className={i % 2 === 0 ? "bg-surface" : "bg-light"}>
                    <td className="p-6 font-semibold text-primary">{r.attribute}</td>
                    {keys.map((k) => (
                ...
                <tr className="bg-surface">
                  ...
                  {keys.map((k) => (
                    <td key={k} className="p-6 text-center">
                      <a href="#contact" className="inline-flex items-center gap-1.5 text-sm font-bold text-secondary hover:underline">
          ...
          <div className="lg:hidden p-5 space-y-3">
            {rows.map((r) => (
              <div key={r.attribute} className="flex items-center justify-between border-b border-border pb-3 last:border-0">
                <span className="font-semibold text-primary">{r.attribute}</span>
