...
export default function WhyChooseUs() {
  return (
    <section className="relative pb-4 pt-12 lg:pb-6 lg:pt-16 bg-gradient-soft overflow-hidden">
      <div className="absolute top-0 right-0 w-[40rem] h-[40rem] bg-secondary/5 rounded-full blur-3xl" />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 items-stretch">
          {features.map((f, i) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="group relative bg-surface rounded-3xl overflow-hidden border border-border shadow-premium hover:shadow-premium transition-all flex flex-col justify-between h-full"
            >
              <div className="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-primary to-secondary z-20 scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-300" />
              ...
              <div className="p-6 lg:p-8 flex-1">
                <h3 className="font-display font-bold text-xl text-primary mb-3 group-hover:text-primary-hover transition-colors duration-300">{f.title}</h3>
