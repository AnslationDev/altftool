...
import { bestCouponfirebase } from "../service/firebasebrand";
import { PopularSalesSkeleton } from "../DealsPageSkeleton";
import Link from "next/link";

function PopularSales({ data }) {
  const [coupon, setCoupon] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const unsubscribe = bestCouponfirebase((data) => {
      setCoupon(data);
            ...
            {coupon.map((_, i) => (
              <div
                ...
                style={{
                  ...
                  transition:
                    "width 0.4s cubic-bezier(0.33,1,0.68,1), background 0.3s ease",
                }}
              />
            ))}
          </div>
