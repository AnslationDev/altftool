...
`node scripts/run-unit-tests.mjs` → **907 tests, 907 pass, 0 fail** at the original
audited commit. The follow-up release repair adds route/discovery regressions and passes
**917/917**.

## Duplicate-title follow-up (rechecked 2026-07-31)

The original 15-pair snapshot became stale after the registry updates landed. A fresh
check finds one shared display name (`animation-generator` and
`animation-generator-tool`), but their effective SEO titles are distinct. The current
default metadata therefore has **zero duplicate title pairs**. Recompute this result
from the generated registry and per-tool SEO sources before treating any future
display-name match as search cannibalisation.
