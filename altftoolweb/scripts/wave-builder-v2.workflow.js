export const meta = {
  name: 'build-tools-wave-500-v2',
  description: 'Build a 500-tool wave: 50 agents x 10 tools, logic split from UI, GEO-ready SEO, fix-in-place for partial tools',
...
const SPEC = `HOW AN ALTFTOOL TOOL IS BUILT — five files per tool. Study ${WEB}/src/tools/section-234f-late-fee-calculator/ first; it is a recent tool built to this exact spec (note how lib.js cites the governing rule for every constant).
...
EXISTING DIRECTORIES:
 - If your entry has "fix": true, the directory exists but is INCOMPLETE (its "have" array lists the
   files already present). Do NOT skip it: read the existing files, verify they meet this spec (fix
   them if they are broken or violate the colour/logic-split rules), and write the missing files so
   all five exist. This is completion work, not duplication.
 - If your entry has no "fix" flag but the directory already exists anyway, SKIP it and say so.`
  ...
  groups.map(({ start, end }, gi) => () =>
    agent(
      `Your work list is in the JSON array at ${waveFile}. Read ONLY entries at index ${start} through ${end - 1} (0-based, ${end - start} tools) — for example:\n` +
        ...
        `Each entry has { slug, name, category, desc, sub } and possibly { fix, have }. Build those ${end - start} tools, one at a time, completely. Ignore every other entry in the file.\n\n` +
        `${SPEC}\n\n${RULES}\n\nReturn a compact report, one block per slug: built | completed (fix) | skipped (+reason) — then the rule/formula implemented | the numeric case you verified (inputs -> output) | parse-check result.`,
